"use strict"; return ((function() {
  ...
  const { ArrayBufferPrototypeGetByteLength, ArrayIsArray, ArrayPrototypeFind, ArrayPrototypeIncludes, DataViewPrototypeGetBuffer, DataViewPrototypeGetByteLength, DataViewPrototypeGetByteOffset, Error, ObjectPrototypeIsPrototypeOf, String, StringPrototypeIncludes, TypeError, TypeErrorPrototype, TypedArrayPrototypeGetBuffer, TypedArrayPrototypeGetByteLength, TypedArrayPrototypeGetByteOffset } = primordials;
  const { op_node_dh_check, op_node_dh_compute_secret, op_node_dh_keys_generate_and_export, op_node_diffie_hellman, op_node_ecdh_compute_public_key, op_node_ecdh_compute_secret, op_node_ecdh_encode_pubkey, op_node_ecdh_generate_keys, op_node_ecdh_validate_private_key, op_node_ecdh_validate_public_key, op_node_gen_prime } = core.ops;
  const { isAnyArrayBuffer, isArrayBufferView, isTypedArray } = core.loadExtScript("ext:deno_node/internal/util/types.ts");
  const { ERR_CRYPTO_ECDH_INVALID_FORMAT, ERR_CRYPTO_ECDH_INVALID_PUBLIC_KEY, ERR_CRYPTO_INCOMPATIBLE_KEY, ERR_CRYPTO_UNKNOWN_DH_GROUP, ERR_INVALID_ARG_TYPE, ERR_INVALID_ARG_VALUE, NodeError } = core.loadExtScript("ext:deno_node/internal/errors.ts");
  const { validateInt32, validateString } = core.loadExtScript("ext:deno_node/internal/validators.mjs");
  const { Buffer } = core.loadExtScript("ext:deno_node/internal/buffer.mjs");
  const { deprecate } = core.loadExtScript("ext:deno_node/util.ts");
  const { ellipticCurves, getDefaultEncoding, toBuf } = core.loadExtScript("ext:deno_node/internal/crypto/util.ts");
  const { getArrayBufferOrView, getKeyObjectHandle, kConsumePrivate, kConsumePublic } = core.loadExtScript("ext:deno_node/internal/crypto/keys.ts");
  ...
  function getViewParts(view) {
    if (isTypedArray(view)) {
      return {
        ab: TypedArrayPrototypeGetBuffer(view),
        off: TypedArrayPrototypeGetByteOffset(view),
        len: TypedArrayPrototypeGetByteLength(view)
      };
    }
    return {
      ab: DataViewPrototypeGetBuffer(view),
      off: DataViewPrototypeGetByteOffset(view),
      len: DataViewPrototypeGetByteLength(view)
    };
  }
  function DiffieHellman(sizeOrKey, keyEncoding, generator, genEncoding) {
    return new DiffieHellmanImpl(sizeOrKey, keyEncoding, generator, genEncoding);
  }
  class DiffieHellmanImpl {
      ...
      if (typeof sizeOrKey !== "number") {
        if (isArrayBufferView(sizeOrKey) || isAnyArrayBuffer(sizeOrKey)) {
          if (isArrayBufferView(sizeOrKey)) {
            const parts = getViewParts(sizeOrKey);
            this.#prime = Buffer.from(parts.ab, parts.off, parts.len);
          } else {
            this.#prime = Buffer.from(sizeOrKey, 0, ArrayBufferPrototypeGetByteLength(sizeOrKey));
          }
        } else {
          this.#prime = toBuf(sizeOrKey, keyEncoding);
        }
      } else {
        // The supplied parameter is our primeLength, generate a suitable prime.
        this.#primeLength = sizeOrKey;
        if (this.#primeLength < 2) {
          throw new NodeError("ERR_OSSL_DH_MODULUS_TOO_SMALL", "modulus too small");
        }
        this.#prime = Buffer.from(TypedArrayPrototypeGetBuffer(op_node_gen_prime(this.#primeLength, false, null, null)));
      }
      if (!generator) {
    ...
    computeSecret(otherPublicKey, inputEncoding, outputEncoding) {
      ...
      if (buf.length === 0) {
        throw new NodeError("ERR_CRYPTO_INVALID_KEYLEN", "Unspecified validation error");
      }
      const sharedSecret = op_node_dh_compute_secret(this.#prime, this.#privateKey, buf);
    ...
    generateKeys(_encoding) {
      ...
      if (this.#privateKey) {
        ...
        const publicKey = op_node_dh_compute_secret(this.#prime, this.#privateKey, this.#generator);
        this.#publicKey = Buffer.from(TypedArrayPrototypeGetBuffer(publicKey));
        this.#publicKeyNeedsUpdate = false;
      } else {
        // Generate both keys
        const keys = op_node_dh_keys_generate_and_export(this.#prime, this.#primeLength ?? 0, generator);
  ...
  class DiffieHellmanGroupImpl {
    ...
    constructor(name){
      ...
      this.#diffiehellman = new DiffieHellmanImpl(buf, DH_GROUPS[name].generator);
      this.verifyError = 0;
    }
    computeSecret(otherPublicKey, inputEncoding, outputEncoding) {
      return this.#diffiehellman.computeSecret(otherPublicKey, inputEncoding, outputEncoding);
    }
  ...
  function validateEcdhFormat(format) {
    if (format !== "compressed" && format !== "uncompressed" && format !== "hybrid") {
      throw new ERR_CRYPTO_ECDH_INVALID_FORMAT(String(format));
    }
  }
  function ecdhEncode(buffer, encoding) {
    if (encoding === undefined || encoding === "buffer") {
      return buffer;
    }
    // deno-lint-ignore deno-internal/prefer-primordials -- Buffer.prototype.toString(encoding) has no primordial
  ...
  class ECDHImpl {
    ...
    static convertKey(key, curve, inputEncoding, outputEncoding, format) {
      ...
      let result;
      try {
        result = Buffer.from(op_node_ecdh_encode_pubkey(curve, buf, compress));
      } catch (e) {
        if (ObjectPrototypeIsPrototypeOf(TypeErrorPrototype, e) && e.message === "Unsupported curve") {
      ...
      if (format === "hybrid") {
        ...
        const compressedBuf = Buffer.from(op_node_ecdh_encode_pubkey(curve, buf, true));
    ...
    computeSecret(otherPublicKey, inputEncoding, outputEncoding) {
      ...
      try {
        op_node_ecdh_compute_secret(this.#curve.name, this.#privbuf, this.#pubbuf, otherBuf, secretBuf);
      } catch (e) {
        const err = e;
    ...
    generateKeys(encoding, format = "uncompressed") {
      ...
      const pubbuf = Buffer.alloc(format == "compressed" ? this.#curve.publicKeySizeCompressed : this.#curve.publicKeySize);
      const privbuf = Buffer.alloc(this.#curve.privateKeySize);
      op_node_ecdh_generate_keys(this.#curve.name, pubbuf, privbuf, format);
      this.#pubbuf = pubbuf;
      this.#privbuf = privbuf;
      if (format === "hybrid") {
        const compressedBuf = Buffer.from(op_node_ecdh_encode_pubkey(this.#curve.name, pubbuf, true));
    ...
    getPublicKey(encoding, format = "uncompressed") {
      ...
      validateEcdhFormat(format);
      const pubbuf = Buffer.from(op_node_ecdh_encode_pubkey(this.#curve.name, this.#pubbuf, format === "compressed"));
      if (format === "hybrid") {
        const compressedBuf = Buffer.from(op_node_ecdh_encode_pubkey(this.#curve.name, this.#pubbuf, true));
        pubbuf[0] = compressedBuf[0] + 4;
      }
      return ecdhEncode(pubbuf, encoding ?? "buffer");
    }
  ...
  ECDH.prototype.setPublicKey = deprecate(ECDHImpl.prototype.setPublicKey, "ecdh.setPublicKey() is deprecated.", "DEP0031");
  function statelessDH(privateKeyObject, publicKeyObject) {
    ...
    if (privType !== undefined && pubType !== undefined && privType !== pubType) {
      throw new ERR_CRYPTO_INCOMPATIBLE_KEY("key types for Diffie-Hellman", `${privType} and ${pubType}`);
    }
    try {
      ...
      if (e && typeof e.message === "string") {
        if (StringPrototypeIncludes(e.message, "mismatching domain parameters")) {
          e.code = "ERR_OSSL_MISMATCHING_DOMAIN_PARAMETERS";
        } else if (StringPrototypeIncludes(e.message, "failed during derivation")) {
