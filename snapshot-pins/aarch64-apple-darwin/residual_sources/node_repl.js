...
let _prevCaptureCallback = null;
function _replCaptureCallback(err) {
  ...
  if (repl) {
    ...
    const dom = repl._domain;
    if (dom && typeof dom.listenerCount === "function" && dom.listenerCount("error") > 0) {
      dom.emit("error", err);
    }
    return;
  }
...
function _installCaptureHook() {
  ...
  const p = process;
  if (typeof p.hasUncaughtExceptionCaptureCallback === "function" && p.hasUncaughtExceptionCaptureCallback()) {
    _prevCaptureCallback = p._uncaughtExceptionCaptureFn ?? null;
...
function _newListenerGuard(event, listener) {
  if (event === "uncaughtException" && listener && listener.name !== "_replUncaughtHandler") {
...
function filterReplStack(stack) {
...
function _cursorTo(stream, x) {
  stream.write(`\x1b[${x + 1}G`);
}
function _moveCursor(stream, dx, dy) {
  let data = "";
...
function _clearLine(stream, dir) {
...
function _getPropertyNames(obj) {
  ...
  try {
    ...
    const tooBigIndexed = typeof len === "number" && len > 100_000 && (ArrayIsArray(obj) || ArrayBufferIsView(obj));
    if (tooBigIndexed) {
      const symbolKeyed = ArrayPrototypeFilter(ObjectKeys(obj), (k)=>!RegExpPrototypeTest(reDigits, k));
      return ArrayPrototypeFilter(symbolKeyed, (name)=>{
        if (SetPrototypeHas(_replHiddenProtoNames, name)) return false;
        return RegExpPrototypeTest(reIdentifier, name);
      });
    }
    return ArrayPrototypeFilter(ObjectGetOwnPropertyNames(obj), (name)=>{
      if (SetPrototypeHas(_replHiddenProtoNames, name)) return false;
      return RegExpPrototypeTest(reIdentifier, name);
    });
  } catch  {
 ...
 */ function isObjectLiteral(code) {
  ...
  return StringPrototypeStartsWith(trimmed, "{") && !StringPrototypeStartsWith(trimmed, "{\\");
}
...
const { createPreviewSession: _createPreviewSession } = core.loadExtScript("ext:deno_node/_repl_preview.js");
...
const reVarDeclaration = new SafeRegExp(/^\s*(?:let|const|var)\s+[a-zA-Z_$][\w$]*$/);
const reRequireImport = new SafeRegExp(/(?:^|[\s;({,])(require|import)\s*\(\s*(['"`])((?:(?!\2|\\).)*)$/);
const reExprChain = new SafeRegExp(/(?:^|[\s;({\[,!~+\-*/&|^%<>?:=])((?:[a-zA-Z_$][\w$]*\??\.)*(?:[a-zA-Z_$][\w$]*)?)\.?$/);
...
const reUnexpectedTokenName = new SafeRegExp(/Unexpected (?:token|identifier|string|number|keyword|reserved\s+word) '(.+?)'/);
 ...
 */ function isRecoverableError(e, code) {
  if (!e || typeof e !== "object" || e.name !== "SyntaxError") {
    return false;
  }
...
function defineDefaultCommands(repl) {
  ...
  repl.defineCommand("help", {
    ...
    action: function() {
      ...
      const longestNameLength = MathMax(...new SafeArrayIterator(ArrayPrototypeMap(names, (name)=>name.length)));
...
export class REPLServer extends Interface {
  constructor(prompt, stream, eval_, useGlobal, ignoreUndefined, replMode){
    ...
    const previewRequested = !!options.terminal && (options.preview !== undefined ? !!options.preview : !eval_);
    ...
    function defaultEval(code, context, _file, cb) {
      let result;
      let err = null;
      ...
      if (isObjectLiteral(code) && !RegExpPrototypeTest(reTrailingSemicolon, StringPrototypeTrim(code))) {
      ...
      if (err === null) {
        ...
        while(true){
          try {
            if (self.replMode === REPL_MODE_STRICT && !RegExpPrototypeTest(reBlankString, code)) {
            ...
            if (isRecoverableError(error, code)) {
              ...
              if (error != null && typeof error === "object" && error.name === "SyntaxError") {
                error._replSourceCode = StringPrototypeReplace(input, reTrailingNewline, "");
              }
              err = error;
            }
          }
    ...
    function _doComplete(line, callback) {
      ...
      if (modMatch) {
        ...
        const fromModule = ArrayIsArray(Module.builtinModules) ? Module.builtinModules : [];
        const fromModulePublic = ArrayPrototypeFilter(fromModule, (m)=>!StringPrototypeStartsWith(m, "_"));
        const unprefixed = ArrayPrototypeFilter(fromModulePublic, (m)=>!StringPrototypeStartsWith(m, "node:"));
        const alreadyPrefixed = ArrayPrototypeFilter(fromModulePublic, (m)=>StringPrototypeStartsWith(m, "node:"));
        const knownInModule = new SafeSet(fromModule);
        const replExtras = ArrayPrototypeFilter(builtinModules, (m)=>!SetPrototypeHas(knownInModule, m) && !StringPrototypeStartsWith(m, "_"));
        ...
        ArrayPrototypePush(completionGroups, [
          ...new SafeArrayIterator(ArrayPrototypeMap(unprefixed, (m)=>`node:${m}`)),
      ...
      if (expr) {
        ...
        self.eval(wrappedExpr, self.context, "repl", (_e, obj)=>{
          if (obj != null) {
            const memberGroups = [];
            try {
              let p;
            ...
            if (memberGroups.length) {
              ...
              for (const group of new SafeArrayIterator(memberGroups)){
                ArrayPrototypePush(completionGroups, ArrayPrototypeMap(group, (m)=>`${prefix}${m}`));
              }
              if (filter) {
      ...
      function completionGroupsLoaded() {
        ...
        if (completionGroups.length && filter) {
          ...
          for (const group of new SafeArrayIterator(completionGroups)){
            const fg = ArrayPrototypeFilter(group, (str)=>StringPrototypeStartsWith(StringPrototypeToLowerCase(str), lowerFilter));
            if (fg.length) ArrayPrototypePush(filtered, fg);
          }
          completionGroups.length = 0;
          ArrayPrototypePush(completionGroups, ...new SafeArrayIterator(filtered));
        }
    ...
    let escaped = null;
    function _getDisplayPosHelper(str) {
    ...
    function getPreviewPos() {
      const displayPos = _getDisplayPosHelper(`${self.getPrompt()}${self.line}`);
      const cursorPos = self.line.length !== self.cursor ? typeof self.getCursorPos === "function" ? self.getCursorPos() : {
        rows: 0,
        cols: self.getPrompt().length + self.cursor
      } : displayPos;
      return {
        displayPos,
        cursorPos
    ...
    function showCompletionPreview(line) {
      previewCompletionCounter++;
      const count = previewCompletionCounter;
      self.completer(line, (err, data)=>{
        ...
        const result = self.useColors ? `\x1b[90m${suffix}\x1b[39m` : ` // ${suffix}`;
    ...
    function showPreview(showCompletion = true) {
      ...
      if (completionPreview !== null && isCursorAtInputEnd() && escaped !== self.line) {
        previewLine += completionPreview;
      }
      let previewCode = previewLine + "\n";
      // Apply object literal wrapping for preview too
      if (isObjectLiteral(previewCode) && !RegExpPrototypeTest(reTrailingSemicolon, StringPrototypeTrim(previewCode))) {
      ...
      let result;
      try {
        const script = new vm.Script(previewCode, {
          filename: "repl"
        });
        if (self.useGlobal) {
          result = script.runInThisContext({
      ...
      // Limit length
      const maxCols = MathMin(self.columns || 80, 250);
      if (inspected.length > maxCols) {
        inspected = StringPrototypeSlice(inspected, 0, maxCols - 4) + "...";
      }
      inputPreview = inspected;
      const preview = self.useColors ? `\x1b[90m${inspected}\x1b[39m` : `// ${inspected}`;
      const { cursorPos, displayPos } = getPreviewPos();
    ...
    function clearPreview(key) {
      ...
      if (completionPreview !== null) {
        ...
        const newPos = _getDisplayPosHelper(totalLine);
        if (newPos.rows === 0 || move && pos.displayPos.rows === newPos.rows) {
        ...
        if (key && !key.ctrl && !key.shift) {
          ...
          } else if ((key.name === "return" || key.name === "enter") && !key.meta && escaped !== self.line && isCursorAtInputEnd()) {
    ...
    self.on("line", function onLine(cmd) {
      ...
      if (self.editorMode) {
        ...
        // code alignment
        const matches = self._sawKeyPress && !self[kLoadingSymbol] ? RegExpPrototypeExec(reLeadingWhitespace, cmd) : null;
        if (matches) {
      ...
      if (trimmedCmd) {
        if (StringPrototypeCharAt(trimmedCmd, 0) === "." && StringPrototypeCharAt(trimmedCmd, 1) !== "." && NumberIsNaN(NumberParseFloat(trimmedCmd))) {
          ...
          if (keyword && FunctionPrototypeCall(_parseREPLKeyword, self, keyword, rest || "") === true) {
            return;
          }
      ...
      function finish(e, ret) {
        FunctionPrototypeCall(_memory, self, cmd);
        if (e !== null && e !== undefined && !self[kBufferedCommandSymbol] && StringPrototypeStartsWith(StringPrototypeTrim(cmd), "npm ") && !ObjectPrototypeIsPrototypeOf(Recoverable.prototype, e)) {
          self.output.write("npm should be run outside of the " + "Node.js REPL, in your normal shell.\n" + "(Press Ctrl+D to exit.)\n");
          self.displayPrompt();
          return;
        }
        // If error was SyntaxError and not JSON.parse error
        if (ObjectPrototypeIsPrototypeOf(Recoverable.prototype, e) && !sawCtrlD) {
          // Start multiline
          self[kBufferedCommandSymbol] += cmd + "\n";
          self.displayPrompt();
          return;
        ...
        if ((e === null || e === undefined) && arguments.length === 2 && (!self.ignoreUndefined || ret !== undefined)) {
  ...
  _handleError(e) {
    ...
    if (typeof e === "object" && e !== null) {
      const isError = ObjectPrototypeIsPrototypeOf(ErrorPrototype, e) || typeof e.name === "string" && typeof e.stack === "string";
      if (isError && e.stack) {
        if (e.name === "SyntaxError") {
          // Remove stack trace
          errStack = StringPrototypeReplace(StringPrototypeReplace(StringPrototypeReplace(e.stack, reStackFrames, ""), reReplFilename, ""), reReplLowerFilename, "");
          ...
          if (e._replSourceCode) {
            const srcLine = e._replSourceCode;
            // Try to determine caret position from the error message.
            // Node.js uses V8's Message.GetStartColumn() which we don't have.
            let col = 0;
            const tokenMatch = RegExpPrototypeExec(reUnexpectedTokenName, e.message);
            if (tokenMatch) {
          ...
          e.stack = filterReplStack(e.stack);
        }
        if (e.name !== "SyntaxError") {
    ...
    ArrayPrototypeForEach(lines, (line)=>{
      if (!matched && RegExpPrototypeTest(reErrorName, line)) {
  ...
  createContext() {
    ...
    if (this.useGlobal) {
      ...
      const builtinNames = new SafeSet(vm.runInNewContext?.("Object.getOwnPropertyNames(globalThis)") ?? []);
      for (const name of new SafeArrayIterator(ObjectGetOwnPropertyNames(globalThis))){
    ...
    const ctxRequire = context.require;
    if (typeof ctxRequire === "function") {
      for (const name of new SafeArrayIterator(builtinModules)){
        if (name[0] === "_" || StringPrototypeIncludes(name, "/") || ObjectPrototypeHasOwnProperty(context, name)) {
          continue;
        }
        const setReal = (val)=>{
          delete context[name];
