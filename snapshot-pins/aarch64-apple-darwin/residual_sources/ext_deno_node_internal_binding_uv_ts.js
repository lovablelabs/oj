"use strict"; return ((function() {
  ...
  const { ArrayPrototypeMap, Error, MapPrototypeGet } = primordials;
  ...
  const errorToCodeWindows = ArrayPrototypeMap(codeToErrorWindows, (entry)=>[
      entry[1][0],
      entry[0]
    ]);
  ...
  const errorToCodeDarwin = ArrayPrototypeMap(codeToErrorDarwin, (entry)=>[
      entry[1][0],
      entry[0]
    ]);
  ...
  const errorToCodeLinux = ArrayPrototypeMap(codeToErrorLinux, (entry)=>[
      entry[1][0],
      entry[0]
    ]);
  ...
  const errorToCodeFreebsd = ArrayPrototypeMap(codeToErrorFreebsd, (entry)=>[
      entry[1][0],
      entry[0]
    ]);
  const codeToErrorOpenBSD = [
    [
      -7,
      [
        "E2BIG",
  ...
  const errorToCodeOpenBSD = ArrayPrototypeMap(codeToErrorOpenBSD, (entry)=>[
      entry[1][0],
      entry[0]
    ]);
  const unreachable = ()=>{
  ...
  const errorMap = new Map(osType === "windows" ? codeToErrorWindows : osType === "darwin" ? codeToErrorDarwin : osType === "linux" ? codeToErrorLinux : osType === "android" ? codeToErrorLinux : osType === "freebsd" ? codeToErrorFreebsd : osType === "openbsd" ? codeToErrorOpenBSD : unreachable());
  // Real Map (not SafeMap): returned to userland via getCodeMap() /
  // process.binding("uv").getCodeMap(); must pass `instanceof Map`.
  // deno-lint-ignore deno-internal/prefer-primordials
  const codeMap = new Map(osType === "windows" ? errorToCodeWindows : osType === "darwin" ? errorToCodeDarwin : osType === "linux" ? errorToCodeLinux : osType === "android" ? errorToCodeLinux : osType === "freebsd" ? errorToCodeFreebsd : osType === "openbsd" ? errorToCodeOpenBSD : unreachable());
