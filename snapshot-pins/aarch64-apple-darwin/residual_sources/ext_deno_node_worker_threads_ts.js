"use strict"; return ((function() {
  ...
  const { op_create_worker, op_host_get_worker_cpu_usage, op_host_post_message, op_host_post_message_raw, op_host_recv_ctrl, op_host_recv_message, op_host_recv_message_sync, op_host_terminate_worker, op_mark_as_untransferable, op_message_port_post_message_raw, op_message_port_recv_message, op_message_port_recv_message_sync, op_node_worker_thread_post_message, op_node_worker_thread_recv_message, op_node_worker_thread_register, op_node_worker_thread_set_listener_count, op_worker_get_resource_limits, op_worker_threads_filename } = core.ops;
  const { deserializeJsMessageData, deserializeMessageData, isUncloneable, markAsUncloneable: webMarkAsUncloneable, MessageChannel, MessagePort, getMessagePortId, MessagePortPrototype, MessagePortReceiveMessageOnPortSymbol, nodeWorkerThreadCloseCb, refMessagePort, serializeJsMessageData, serializeMessageData, setMessagePortId, unrefParentPort } = core.loadExtScript("ext:deno_web/13_message_port.js");
  const webidl = core.loadExtScript("ext:deno_webidl/00_webidl.js");
  const { notImplemented } = core.loadExtScript("ext:deno_node/_utils.ts");
  const { ERR_CLOSED_MESSAGE_PORT, ERR_CONSTRUCT_CALL_REQUIRED, ERR_INVALID_ARG_TYPE, ERR_INVALID_URL_SCHEME, ERR_OUT_OF_RANGE, ERR_WORKER_INVALID_EXEC_ARGV, ERR_WORKER_MESSAGING_ERRORED, ERR_WORKER_MESSAGING_FAILED, ERR_WORKER_MESSAGING_SAME_THREAD, ERR_WORKER_MESSAGING_TIMEOUT, ERR_WORKER_NOT_RUNNING, ERR_WORKER_PATH } = core.loadExtScript("ext:deno_node/internal/errors.ts");
  const { validateArray, validateInteger, validateObject } = core.loadExtScript("ext:deno_node/internal/validators.mjs");
  const { EventEmitter } = core.loadExtScript("ext:deno_node/_events.mjs");
  const { channel: createDiagnosticsChannel } = core.loadExtScript("ext:deno_node/diagnostics_channel.js");
  const workerThreadsChannel = createDiagnosticsChannel("worker_threads");
  const lazyStream = core.createLazyLoader("node:stream");
  const { BroadcastChannel: WebBroadcastChannel, refBroadcastChannel } = core.loadExtScript("ext:deno_web/01_broadcast_channel.js");
  const { untransferableSymbol } = core.loadExtScript("ext:deno_node/internal_binding/util.ts");
  ...
  // loaders.
  const { ArrayBufferIsView, ArrayIsArray, ArrayPrototypeIndexOf, ArrayPrototypePush, ArrayPrototypeSlice, ArrayPrototypeSplice, Error, EvalError, FunctionPrototypeApply, FunctionPrototypeBind, FunctionPrototypeCall, NumberIsFinite, NumberIsNaN, ObjectAssign, ObjectCreate, ObjectDefineProperty, ObjectHasOwn, ObjectKeys, ObjectPrototypeIsPrototypeOf, Promise, PromisePrototypeThen, PromiseReject, PromiseResolve, queueMicrotask, RangeError, ReferenceError, SafeMap, SafeRegExp, SafeSet, String, StringPrototypeIndexOf, StringPrototypeSlice, StringPrototypeSplit, StringPrototypeStartsWith, StringPrototypeTrim, Symbol, SymbolAsyncDispose, SymbolFor, SymbolIterator, SyntaxError, TypeError, URIError, Float64Array } = primordials;
  ...
  function isWorkerOnlineMsg(data) {
    return typeof data === "object" && data !== null && ObjectHasOwn(data, "type") && data["type"] === "WORKER_ONLINE";
  }
  function isWorkerStdinMsg(data) {
    return typeof data === "object" && data !== null && ObjectHasOwn(data, "type") && data["type"] === "WORKER_STDIN";
  }
  function isWorkerStdinEndMsg(data) {
    return typeof data === "object" && data !== null && ObjectHasOwn(data, "type") && data["type"] === "WORKER_STDIN_END";
  }
  function isWorkerStderrMsg(data) {
    return typeof data === "object" && data !== null && ObjectHasOwn(data, "type") && data["type"] === "WORKER_STDERR";
  }
  function isWorkerStdoutMsg(data) {
    return typeof data === "object" && data !== null && ObjectHasOwn(data, "type") && data["type"] === "WORKER_STDOUT";
  }
  ...
  class NodeWorker extends EventEmitter {
    ...
    // deno-lint-ignore no-explicit-any
    stdout = new (lazyStream()).Readable({
      read () {}
    });
    // https://nodejs.org/api/worker_threads.html#workerstderr
    // deno-lint-ignore no-explicit-any
    stderr = new (lazyStream()).Readable({
      read () {}
    });
    constructor(specifier, options){
      ...
      if (options?.execArgv) {
        ...
        if (options.execArgv.length > 0) {
          ...
          for(let i = 0; i < options.execArgv.length; i++){
            ...
            const flagName = eqIdx === -1 ? flag : StringPrototypeSlice(flag, 0, eqIdx);
      ...
      if (options?.env) {
        ...
        if (typeof nodeOptions === "string" && nodeOptions.length > 0) {
          ...
          const parts = StringPrototypeSplit(StringPrototypeTrim(nodeOptions), new SafeRegExp("\\s+"));
          let hasInvalid = false;
          for(let i = 0; i < parts.length; i++){
            const part = parts[i];
            if (StringPrototypeStartsWith(part, "-")) {
              const eqIdx = StringPrototypeIndexOf(part, "=");
              const partName = eqIdx === -1 ? part : StringPrototypeSlice(part, 0, eqIdx);
              if (workerSilentlyIgnoredFlags.has(partName)) {
                continue;
              }
              if (!lazyProcess().default.allowedNodeEnvironmentFlags.has(part) || workerDisallowedFlags.has(partName)) {
                hasInvalid = true;
                break;
              }
            }
      ...
      if (typeof specifier === "object") {
        if (!(specifier.protocol === "data:" || specifier.protocol === "file:")) {
        ...
        if (StringPrototypeStartsWith(specifier, "file://") || StringPrototypeStartsWith(specifier, "data:") || StringPrototypeStartsWith(specifier, "http://") || StringPrototypeStartsWith(specifier, "https://")) {
          throw new ERR_WORKER_PATH(specifier);
        }
        const path = specifier;
        if (!StringPrototypeStartsWith(path, "/") && !StringPrototypeStartsWith(path, "./") && !StringPrototypeStartsWith(path, "../") && !StringPrototypeStartsWith(path, ".\\") && !StringPrototypeStartsWith(path, "..\\")) {
          // On Windows, also allow drive-letter absolute paths (e.g. C:\...)
          const isWindowsAbsolute = path.length >= 3 && path[1] === ":" && (path[2] === "\\" || path[2] === "/");
      ...
      if (envOpt != null && envOpt !== SHARE_ENV) {
        if (typeof envOpt !== "object") {
          throw new ERR_INVALID_ARG_TYPE("options.env", [
            "object",
            "undefined",
            "null",
      ...
      if (options?.eval) {
        ...
        sourceCode = `var __filename = ${// deno-lint-ignore deno-internal/prefer-primordials
        JSON.stringify(lazyProcess().default.cwd() + "/[worker eval]")};\n` + `var __dirname = ${// deno-lint-ignore deno-internal/prefer-primordials
      ...
      lazyProcess().default.nextTick(()=>lazyProcess().default.emit("worker", this));
    ...
    #closeStdio() {
      if (!this.stdout.readableEnded) {
        FunctionPrototypeCall(lazyStream().Readable.prototype.push, this.stdout, null);
      }
      if (!this.stderr.readableEnded) {
        FunctionPrototypeCall(lazyStream().Readable.prototype.push, this.stderr, null);
      }
    }
    #pollControl = async ()=>{
    ...
    #dispatchWorkerThreadMessage(data) {
      ...
      if (// only emit "online" event once, and since the message
      ...
      !this.#workerOnline && isWorkerOnlineMsg(message)) {
        ...
        FunctionPrototypeCall(lazyStream().Readable.prototype.push, this.stdout, message.data);
      } else if (isWorkerStderrMsg(message)) {
        FunctionPrototypeCall(lazyStream().Readable.prototype.push, this.stderr, message.data);
      } else {
    ...
    postMessage(message, transferOrOptions = {
      ...
      if (transferOrOptions === undefined || transferOrOptions === null || arguments.length <= 1) {
        ...
        if (isUncloneable(message)) {
          throw new DOMException("Cannot clone object of unsupported type.", "DataCloneError");
        }
        op_host_post_message_raw(this.#id, serializeMessageData(message));
        return;
      }
      message = webidl.converters.any(message);
      let options;
      if (webidl.type(transferOrOptions) === "Object" && transferOrOptions !== undefined && transferOrOptions[SymbolIterator] !== undefined) {
        const transfer = webidl.converters["sequence<object>"](transferOrOptions, prefix, "Argument 2");
        options = {
          transfer
        };
      } else {
        options = webidl.converters.StructuredSerializeOptions(transferOrOptions, prefix, "Argument 2");
      }
    ...
    cpuUsage(prevValue) {
      if (prevValue != null && !NumberIsNaN(prevValue)) {
        ...
        if (typeof prevValue.user !== "number") {
          throw new ERR_INVALID_ARG_TYPE("prevValue.user", "number", prevValue.user);
        }
        if (!NumberIsFinite(prevValue.user) || prevValue.user < 0) {
          throw new ERR_OUT_OF_RANGE("prevValue.user", ">= 0 && <= 2^53", prevValue.user);
        }
        if (typeof prevValue.system !== "number") {
          throw new ERR_INVALID_ARG_TYPE("prevValue.system", "number", prevValue.system);
        }
        if (!NumberIsFinite(prevValue.system) || prevValue.system < 0) {
          throw new ERR_OUT_OF_RANGE("prevValue.system", ">= 0 && <= 2^53", prevValue.system);
        }
      }
  ...
  internals.__initWorkerThreads = (runningOnMainThread, workerId, maybeWorkerMetadata, moduleSpecifier)=>{
    ...
    if (!isMainThread) {
      ...
      // CJS/ESM loading
      if (moduleSpecifier) {
        globalThis.require = lazyModule().createRequire(StringPrototypeStartsWith(moduleSpecifier, "data:") ? `${Deno.cwd()}/[worker eval]` : moduleSpecifier);
      }
      const parentPortListeners = ObjectCreate(null);
      const getParentPortListenerMap = (name)=>{
      ...
      const nativePostMessage = FunctionPrototypeBind(globalThis.postMessage, globalThis);
      const nativeAddEventListener = FunctionPrototypeBind(globalThis.addEventListener, globalThis);
      const nativeRemoveEventListener = FunctionPrototypeBind(globalThis.removeEventListener, globalThis);
      ...
      if (maybeWorkerMetadata) {
        ...
        if (isWorkerThread) {
          ...
          } else if (moduleSpecifier && StringPrototypeStartsWith(moduleSpecifier, "file:")) {
          ...
          if (metadata.execArgv) {
            ...
            core.loadExtScript("ext:deno_node/internal_binding/node_options.ts").setOptionSourceExecArgv(metadata.execArgv);
          ...
          if (metadata.hasStdin) {
            const workerStdin = new (lazyStream()).Readable({
              read () {}
            });
          ...
          const origStdoutWrite = FunctionPrototypeBind(lazyProcess().default.stdout.write, lazyProcess().default.stdout);
          lazyProcess().default.stdout.write = function(chunk, encoding, callback) {
            parentPort.postMessage({
              type: "WORKER_STDOUT",
              data: chunk
            });
            return FunctionPrototypeCall(origStdoutWrite, lazyProcess().default.stdout, chunk, encoding, callback);
          };
          // Forward stderr writes to the parent so worker.stderr
          // is readable from the host side.
          const origStderrWrite = FunctionPrototypeBind(lazyProcess().default.stderr.write, lazyProcess().default.stderr);
          lazyProcess().default.stderr.write = function(chunk, encoding, callback) {
            parentPort.postMessage({
              type: "WORKER_STDERR",
              data: chunk
            });
            return FunctionPrototypeCall(origStderrWrite, lazyProcess().default.stderr, chunk, encoding, callback);
          };
        }
      }
  ...
  function isCrossThreadEnvelope(v) {
    return typeof v === "object" && v !== null && ObjectHasOwn(v, "__nodeWorkerMessage") && v.__nodeWorkerMessage === true;
  }
  function deliverThreadMessageAck(id, status) {
  ...
  function sendAck(senderThreadId, id, status) {
    try {
      const envelope = {
  ...
  async function pollCrossThreadMessages() {
    ...
    while(threadMessageRid !== undefined){
      ...
      try {
        lazyProcess().default.emit("workerMessage", envelope.payload, envelope.sender);
      } catch  {
  ...
  function setupCrossThreadMessaging() {
    ...
    workerMessageListenerCount = lazyProcess().default.listenerCount?.("workerMessage") ?? 0;
    if (workerMessageListenerCount > 0) {
      ensureCrossThreadMessaging();
    }
    lazyProcess().default.on("newListener", (eventName)=>{
      if (eventName === "workerMessage") {
        workerMessageListenerCount++;
        ensureCrossThreadMessaging();
        op_node_worker_thread_set_listener_count(threadId, workerMessageListenerCount);
      }
    });
    lazyProcess().default.on("removeListener", (eventName)=>{
      if (eventName === "workerMessage") {
  ...
  function postMessageToThread(targetThreadId, value, transferListOrTimeout, maybeTimeout) {
    let transferList;
    let timeout;
    try {
      ...
      if (ArrayIsArray(transferListOrTimeout)) {
        ...
        throw new ERR_INVALID_ARG_TYPE("transferList", "Array", transferListOrTimeout);
      }
  ...
  function isMarkedAsUntransferable(obj) {
    ...
    return ObjectHasOwn(obj, untransferableSymbol) && obj[untransferableSymbol] === true;
  }
  ...
  function moveMessagePortToContext(port, context) {
    ...
    let enabled = false;
    let closed = false;
    const dispatchMessageError = (err)=>{
      ...
      (async ()=>{
        while(!closed){
          ...
          try {
            ...
            message = deserializeMessageData(ArrayBufferIsView(data) ? data : data.data, false);
          } catch  {
            const err = new Error("Message could not be deserialized in the target context");
            // deno-lint-ignore no-explicit-any
 ...
 */ function receiveMessageOnPort(port) {
  ...
  // deno-lint-ignore no-explicit-any
  NodeMessageChannel.prototype = MessageChannel.prototype;
  function webMessagePortToNodeMessagePort(port) {
    ...
    // deno-lint-ignore no-explicit-any
    if (port[nodePortListenersSymbol]) return port;
    const portListeners = ObjectCreate(null);
    const getListenerMap = (name)=>{
      let map = portListeners[name];
      if (map === undefined) {
        map = new SafeMap();
        portListeners[name] = map;
      }
      return map;
    };
    ...
    const isWebEvent = (name)=>name === "message" || name === "messageerror" || name === "close";
    ...
    port.off = port.removeListener = function(name, listener) {
      if (!isWebEvent(name)) {
    ...
    port.postMessage = function postMessage(message, transferOrOptions) {
      ...
      if (arguments.length >= 2 && transferOrOptions !== undefined) {
        ...
        // null is allowed; treated as no transferables.
        } else if (typeof transferOrOptions !== "object" && typeof transferOrOptions !== "function") {
          ...
          if (ArrayIsArray(transferOrOptions)) {
            for(let i = 0; i < transferOrOptions.length; i++){
              ...
              if (item !== null && typeof item === "object" && item[untransferableSymbol] === true) {
        ...
        // serializer below sees the original iterator.
          ...
          if (t !== undefined) {
            if (t === null || typeof t !== "object" && typeof t !== "function" || t[SymbolIterator] === undefined || typeof t[SymbolIterator] === "function" && typeof t[SymbolIterator]().next !== "function") {
              const e = new TypeError("Optional options.transfer argument must be an iterable");
              // deno-lint-ignore no-explicit-any
              e.code = "ERR_INVALID_ARG_TYPE";
              throw e;
            }
      ...
      return FunctionPrototypeCall(webPostMessage, port, message, normalized);
    };
    port.once = (name, listener)=>{
      const fn = (event)=>{
        port.off(name, fn);
  ...
  // deno-lint-ignore no-explicit-any
  function patchMessagePortIfFound(data, seen = new SafeSet()) {
  ...
  // deno-lint-ignore no-explicit-any
  const NodeMessagePort = {
    MessagePort: function MessagePort() {
      const err = new TypeError("Constructor for class MessagePort cannot be invoked");
      // deno-lint-ignore no-explicit-any
