"use strict"; return ((function() {
  ...
  const { op_cpus, op_homedir, op_node_os_get_priority, op_node_os_set_priority, op_node_os_user_info } = core.ops;
  ...
  const { validateInt32 } = core.loadExtScript("ext:deno_node/internal/validators.mjs");
  const { denoErrorToNodeSystemError } = core.loadExtScript("ext:deno_node/internal/errors.ts");
  const { ArrayBuffer, ArrayPrototypePush, DataView, DataViewPrototypeSetInt16, Error, Int16Array, ObjectDefineProperties, SafeArrayIterator, StringPrototypeEndsWith, StringPrototypeSlice, StringPrototypeStartsWith, SymbolToPrimitive } = primordials;
  const constants = os;
  function arch() {
    return process.arch;
  }
  ...
  function endianness() {
    ...
    DataViewPrototypeSetInt16(new DataView(buffer), 0, 256, true);
  ...
  function networkInterfaces() {
    const interfaces = {};
    for (const { name, address, netmask, family, mac, scopeid, cidr } of new SafeArrayIterator(Deno.networkInterfaces())){
  ...
  function tmpdir() {
    if (isWindows) {
      let temp = Deno.env.get("TEMP") || Deno.env.get("TMP") || (Deno.env.get("SystemRoot") || Deno.env.get("windir")) + "\\temp";
      if (temp.length > 1 && StringPrototypeEndsWith(temp, "\\") && !StringPrototypeEndsWith(temp, ":\\")) {
        temp = StringPrototypeSlice(temp, 0, -1);
      }
      return temp;
    } else {
      let temp = Deno.env.get("TMPDIR") || Deno.env.get("TMP") || Deno.env.get("TEMP") || "/tmp";
