"use strict"; return ((function() {
  ...
  const { ERR_UNHANDLED_ERROR } = core.loadExtScript("ext:deno_node/internal/errors.ts");
  const { AsyncHook } = core.loadExtScript("ext:deno_node/internal/async_hooks.ts");
  const { ArrayPrototypeEvery, ArrayPrototypeIndexOf, ArrayPrototypeLastIndexOf, ArrayPrototypePush, ArrayPrototypeSlice, ArrayPrototypeSplice, FunctionPrototypeCall, FunctionPrototypeApply, ObjectDefineProperty, ObjectPrototypeIsPrototypeOf, ReflectApply, SafeMap } = primordials;
  ...
  class Domain extends EventEmitter {
    ...
    bind(fn) {
      ...
      return function() {
        self.enter();
        try {
          const ret = FunctionPrototypeApply(fn, this, ArrayPrototypeSlice(arguments));
          self.exit();
          return ret;
        } catch (e) {
          self.exit();
    ...
    intercept(fn) {
      ...
      return function(e) {
        if (e) {
          ...
          self.enter();
          try {
            const ret = FunctionPrototypeApply(fn, this, ArrayPrototypeSlice(arguments, 1));
            self.exit();
            return ret;
          } catch (e) {
            self.exit();
  ...
  function updateExceptionCapture() {
    const shouldCapture = !ArrayPrototypeEvery(stack, (domain)=>domain.listenerCount("error") === 0);
    if (shouldCapture && !exceptionCaptureActive) {
      exceptionCaptureActive = true;
      process.setUncaughtExceptionCaptureCallback(domainUncaughtExceptionHandler);
 ...
 * of Domain class to best approximate that behavior. */ function patchEventEmitter() {
    if (patched) return;
    patched = true;
