"use strict"; return ((function() {
  ...
  const { ArrayFrom, ArrayIsArray, ArrayPrototypeForEach, ArrayPrototypeMap, ArrayPrototypePush, ArrayPrototypeSort, ArrayPrototypeUnshift, ArrayBuffer, ArrayBufferIsView, ArrayPrototypeSlice, Error, FunctionPrototypeApply, FunctionPrototypeBind, FunctionPrototypeCall, MathMin, Number, ObjectAssign, ObjectDefineProperty, ObjectEntries, ObjectHasOwn, ObjectKeys, ObjectPrototypeIsPrototypeOf, Promise, Proxy, ReflectApply, ReflectGet, ReflectGetPrototypeOf, ReflectSet, SafeArrayIterator, SafeMap, SafeRegExp, SafeSet, StringPrototypeCharCodeAt, StringPrototypeIndexOf, StringPrototypeSlice, StringPrototypeToLowerCase, Symbol, SymbolAsyncDispose, SymbolDispose, TypedArrayPrototypeGetBuffer, Uint32Array, Uint8Array } = primordials;
  const { Http2Session: InternalHttp2Session, op_http2_callbacks } = core.ops;
  const { enqueueNodePerformanceEntry } = core.loadExtScript("ext:deno_node/perf_hooks.js");
  const { performance: webPerformance } = core.loadExtScript("ext:deno_web/15_performance.js");
  ...
  const { _connectionListener: httpConnectionListener, applyAddressOverride, httpServerPreClose, kIncomingMessage, kServerResponse, notifyAddressOverrideServing, SERVER_KIND_NODE_HTTP2, startOverrideListener, Server: HttpServer, setupConnectionsTracking, STATUS_CODES, storeHTTPOptions } = core.createLazyLoader("node:_http_server")();
  ...
  const { kBytesWritten, kLastWriteWasAsync, ShutdownWrap, streamBaseState } = core.loadExtScript("ext:deno_node/internal_binding/stream_wrap.ts");
  const { Http2Session: BindingHttp2Session, Http2Stream: BindingHttp2Stream } = core.loadExtScript("ext:deno_node/internal_binding/http2.ts");
  const { EventEmitter } = core.loadExtScript("ext:deno_node/_events.mjs");
  const { defaultTriggerAsyncIdScope, symbols } = core.loadExtScript("ext:deno_node/internal/async_hooks.ts");
  const { async_id_symbol } = symbols;
  const { kTimeout } = core.loadExtScript("ext:deno_node/internal/timers.mjs");
  ...
  const { addAbortListener } = core.loadExtScript("ext:deno_node/internal/events/abort_listener.mjs");
  const lazyFs = core.createLazyLoader("node:fs");
  const fs = lazyFs().default;
  const { FileHandle: FsFileHandle } = core.createLazyLoader("ext:deno_node/internal/fs/handle.ts")();
  const { JSStreamSocket } = core.loadExtScript("ext:deno_node/internal/js_stream_socket.js");
  const { format, inspect } = core.loadExtScript("ext:deno_node/util.ts");
  const { isUint32, validateAbortSignal, validateArray, validateBoolean, validateBuffer, validateFunction, validateInt32, validateInteger, validateNumber, validateObject, validateString, validateUint32 } = core.loadExtScript("ext:deno_node/internal/validators.mjs");
  const { promisify } = core.loadExtScript("ext:deno_node/internal/util.mjs");
  const { customInspectSymbol: kInspect } = core.loadExtScript("ext:deno_node/internal/util.mjs");
  const { AbortError, aggregateTwoErrors, ERR_HTTP2_ALTSVC_INVALID_ORIGIN, ERR_HTTP2_ALTSVC_LENGTH, ERR_HTTP2_GOAWAY_SESSION, ERR_HTTP2_HEADERS_AFTER_RESPOND, ERR_HTTP2_HEADERS_SENT, ERR_HTTP2_INVALID_INFO_STATUS, ERR_HTTP2_INVALID_ORIGIN, ERR_HTTP2_INVALID_PACKED_SETTINGS_LENGTH, ERR_HTTP2_INVALID_SESSION, ERR_HTTP2_INVALID_SETTING_VALUE, ERR_HTTP2_INVALID_STREAM, ERR_HTTP2_MAX_PENDING_SETTINGS_ACK, ERR_HTTP2_NESTED_PUSH, ERR_HTTP2_NO_SOCKET_MANIPULATION, ERR_HTTP2_ORIGIN_LENGTH, ERR_HTTP2_OUT_OF_STREAMS, ERR_HTTP2_PAYLOAD_FORBIDDEN, ERR_HTTP2_PING_CANCEL, ERR_HTTP2_PING_LENGTH, ERR_HTTP2_PUSH_DISABLED, ERR_HTTP2_SEND_FILE, ERR_HTTP2_SEND_FILE_NOSEEK, ERR_HTTP2_SESSION_ERROR, ERR_HTTP2_SETTINGS_CANCEL, ERR_HTTP2_SOCKET_BOUND, ERR_HTTP2_SOCKET_UNBOUND, ERR_HTTP2_STATUS_101, ERR_HTTP2_STATUS_INVALID, ERR_HTTP2_STREAM_CANCEL, ERR_HTTP2_STREAM_ERROR, ERR_HTTP2_STREAM_SELF_DEPENDENCY, ERR_HTTP2_TOO_MANY_CUSTOM_SETTINGS, ERR_HTTP2_TRAILERS_ALREADY_SENT, ERR_HTTP2_TRAILERS_NOT_READY, ERR_HTTP2_UNSUPPORTED_PROTOCOL, ERR_INVALID_ARG_TYPE, ERR_INVALID_ARG_VALUE, ERR_INVALID_CHAR, ERR_INVALID_URL, ERR_OUT_OF_RANGE, ERR_SOCKET_CLOSED, ERR_TLS_ALPN_CALLBACK_WITH_PROTOCOLS, hideStackFrames } = core.loadExtScript("ext:deno_node/internal/errors.ts");
  const { kAfterAsyncWrite, kBoundSession, kHandle, kMaybeDestroy, kSession, kUpdateTimer, onStreamRead, setStreamTimeout, writeGeneric, writevGeneric } = core.loadExtScript("ext:deno_node/internal/stream_base_commons.ts");
  const { assertIsArray, assertIsObject, assertValidPseudoHeader, assertValidPseudoHeaderResponse, assertValidPseudoHeaderTrailer, assertWithinRange, buildNgHeaderString, getAuthority, getDefaultSettings, getSessionState, getSettings, getStreamState, isPayloadMeaningless, kAuthority, kProtocol, kProxySocket, kRequest, kSensitiveHeaders, kSocket, kStrictSingleValueFields, MAX_ADDITIONAL_SETTINGS, NghttpError, prepareRequestHeadersArray, prepareRequestHeadersObject, remoteCustomSettingsToBuffer, sessionName, toHeaderObject, updateOptionsBuffer, updateSettingsBuffer } = core.loadExtScript("ext:deno_node/internal/http2/util.ts");
  const { ownerSymbol: owner_symbol } = core.loadExtScript("ext:deno_node/internal_binding/symbols.ts");
  const { Http2ServerRequest, Http2ServerResponse, onServerStream } = core.loadExtScript("ext:deno_node/internal/http2/compat.js");
  const { updateSpanFromError } = core.loadExtScript("ext:deno_telemetry/util.ts");
  const { otelState, builtinTracer, ContextManager, SPAN_KEY } = core.loadExtScript("ext:deno_telemetry/telemetry.ts");
  ...
  const { debuglog } = core.loadExtScript("ext:deno_node/internal/util/debuglog.ts");
  ...
  function emitSessionPerfEntry(session) {
    ...
    const framesReceived = handle && typeof handle.framesReceived === "function" ? handle.framesReceived() : 0;
    const framesSent = handle && typeof handle.framesSent === "function" ? handle.framesSent() : 0;
    const streamCount = stats.streamCount;
    const streamAverageDuration = streamCount > 0 ? stats.streamTotalDuration / streamCount : 0;
  ...
  function emitStreamPerfEntry(stream) {
    ...
    const detail = {
      ...
      timeToFirstByteSent: stats.firstByteSent > 0 ? stats.firstByteSent - startTime : 0,
      timeToFirstHeader: stats.firstHeader > 0 ? stats.firstHeader - startTime : 0
    };
    enqueueNodePerformanceEntry({
      name: "Http2Stream",
      entryType: "http2",
  ...
  const { NGHTTP2_CANCEL, NGHTTP2_FLOW_CONTROL_ERROR, NGHTTP2_REFUSED_STREAM, NGHTTP2_DEFAULT_WEIGHT, NGHTTP2_FLAG_END_STREAM, NGHTTP2_HCAT_PUSH_RESPONSE, NGHTTP2_HCAT_RESPONSE, NGHTTP2_INTERNAL_ERROR, NGHTTP2_NO_ERROR, NGHTTP2_SESSION_CLIENT, NGHTTP2_SESSION_SERVER, NGHTTP2_ERR_STREAM_ID_NOT_AVAILABLE, NGHTTP2_ERR_INVALID_ARGUMENT, NGHTTP2_ERR_STREAM_CLOSED, NGHTTP2_ERR_NOMEM, HTTP2_HEADER_AUTHORITY, HTTP2_HEADER_DATE, HTTP2_HEADER_METHOD, HTTP2_HEADER_PATH, HTTP2_HEADER_SCHEME, HTTP2_HEADER_STATUS, HTTP2_HEADER_CONTENT_LENGTH, NGHTTP2_SETTINGS_HEADER_TABLE_SIZE, NGHTTP2_SETTINGS_ENABLE_PUSH, NGHTTP2_SETTINGS_MAX_CONCURRENT_STREAMS, NGHTTP2_SETTINGS_INITIAL_WINDOW_SIZE, NGHTTP2_SETTINGS_MAX_FRAME_SIZE, NGHTTP2_SETTINGS_MAX_HEADER_LIST_SIZE, NGHTTP2_SETTINGS_ENABLE_CONNECT_PROTOCOL, HTTP2_METHOD_GET, HTTP2_METHOD_HEAD, HTTP_STATUS_CONTINUE, HTTP_STATUS_RESET_CONTENT, HTTP_STATUS_OK, HTTP_STATUS_NO_CONTENT, HTTP_STATUS_NOT_MODIFIED, HTTP_STATUS_SWITCHING_PROTOCOLS, HTTP_STATUS_MISDIRECTED_REQUEST, STREAM_OPTION_EMPTY_PAYLOAD, STREAM_OPTION_GET_TRAILERS } = constants;
  ...
  const kQuotedString = new SafeRegExp("^[\\x09\\x20-\\x5b\\x5d-\\x7e\\x80-\\xff]*$");
  ...
  function syncSessionTimeoutInspectLinks(timeout) {
    const list = timeout._idlePrev instanceof TimersList ? timeout._idlePrev : new TimersList(timeout);
  ...
  function onPing(payload, isAck) {
    ...
    const buf = payload && Buffer.from(payload.buffer, payload.byteOffset, payload.byteLength);
    if (isAck) {
  ...
  function doStreamClose(stream, code) {
    ...
    if (!stream.readable || code !== NGHTTP2_NO_ERROR) {
      ...
      if (code === NGHTTP2_NO_ERROR && ws && ws.ending && !ws.finished && !ws.destroyed) {
  ...
  function onGoawayData(code, lastStreamID, buf) {
    ...
    const opaqueData = buf !== undefined ? Buffer.from(buf.buffer, buf.byteOffset, buf.byteLength) : buf;
  ...
  function hasNonZeroContentLength(stream) {
    ...
    if (sent !== undefined) {
      ...
      for(let i = 0; i < keys.length; ++i){
        if (StringPrototypeToLowerCase(keys[i]) === HTTP2_HEADER_CONTENT_LENGTH) {
          const v = sent[keys[i]];
    ...
    if (raw !== undefined) {
      for(let i = 0; i < raw.length; i += 2){
        if (StringPrototypeToLowerCase(raw[i]) === HTTP2_HEADER_CONTENT_LENGTH) {
          const v = raw[i + 1];
          if (v !== undefined && Number(v) !== 0) return true;
        }
      }
  ...
  function requestOnConnect(headersList, options) {
    ...
    const ret = ReflectApply(BindingHttp2Session.prototype.request, session[kHandle], [
      headersList[0],
      headersList[1],
      streamOptions,
      options.parent | 0,
  ...
  const validateSettings = hideStackFrames((settings)=>{
    ...
    if (settings.customSettings) {
      ...
      for (const { 0: key, 1: value } of new SafeArrayIterator(entries)){
        ...
        assertWithinRange("customSettings:value", Number(value), 0, kMaxInt);
      }
    }
    assertWithinRange("headerTableSize", settings.headerTableSize, 0, kMaxInt);
    assertWithinRange("initialWindowSize", settings.initialWindowSize, 0, kMaxInitWindowSize);
  ...
  function emitStreamNT(session, stream, obj, flags, headers) {
    ...
    ReflectApply(session.emit, session, [
      "stream",
      stream,
      obj,
      flags,
  ...
  function callTimeout(self, session) {
    ...
    if (self[kState].writeQueueSize > 0) {
      const handle = session[kHandle];
      const chunksSentSinceLastWrite = handle !== undefined ? handle.chunksSentSinceLastWrite : null;
  ...
  function onSessionHeaders(handle = {
    __proto__: null
  }, id, cat, flags, headers = [], sensitiveHeaders = []) {
    const session = this[kOwner];
    if (session === undefined || session.destroyed) {
      return;
    }
    ...
    if (stream === undefined) {
      ...
      if (type === NGHTTP2_SESSION_SERVER) {
        ...
        if (otelState.TRACING_ENABLED) {
          ...
          const span = builtinTracer().startSpan(reqMethod, {
            kind: 1
          }, ctx);
          ...
          if (reqPath !== undefined) {
            ...
            if (qIdx < 0) {
              ...
              span.setAttribute("url.path", StringPrototypeSlice(reqPath, 0, qIdx));
              span.setAttribute("url.query", StringPrototypeSlice(reqPath, qIdx + 1));
            }
          }
  ...
  function shutdownWritable(callback) {
    ...
    if ((state.endStream || err === 1 && !(state.flags & STREAM_FLAGS_HAS_TRAILERS)) && !(state.flags & STREAM_FLAGS_CLOSED)) {
  ...
  function closeStream(stream, code, rstStreamStatus = kSubmitRstStream) {
    ...
    if (rstStreamStatus === kForceRstStream && code !== NGHTTP2_NO_ERROR && !ending && !stream.pending && stream[kHandle] !== undefined) {
  ...
  class Http2Stream extends Duplex {
    ...
    [kInit](id, handle) {
      ...
      const nativeWriteUtf8String = FunctionPrototypeBind(handle.writeUtf8String, handle);
      ...
      if (reqAsync) {
        handle.onread = function(arrayBuffer, nread) {
          return reqAsync.runInAsyncScope(onStreamRead, handle, arrayBuffer, nread);
        };
      } else {
    ...
    _write(data, encoding, cb) {
      if (this[kSession]?.[kType] === NGHTTP2_SESSION_CLIENT && onClientStreamBodyChunkSentChannel.hasSubscribers) {
    ...
    _writev(data, cb) {
      if (this[kSession]?.[kType] === NGHTTP2_SESSION_CLIENT && onClientStreamBodyChunkSentChannel.hasSubscribers) {
        ...
        // through verbatim.
        const chunkData = data.allBuffers ? ArrayPrototypeMap(data, (c)=>c.chunk) : data;
        onClientStreamBodyChunkSentChannel.publish({
          stream: this,
          writev: true,
          data: chunkData,
      ...
      const hasChunk = typeof chunk === "function" ? false : chunk !== undefined && chunk !== null;
      const handle = this[kHandle];
      const id = this[kID];
      if (handle && typeof handle.markEnding === "function" && !this.pending && !this._writableState.ending && this._writableState.buffered.length === 0 && // For end(chunk): state.writing must be false; another in-flight
      ...
      (hasChunk ? !this._writableState.writing : this._writableState.writing) && !(this[kState].flags & STREAM_FLAGS_HAS_TRAILERS) && typeof id === "number" && id % 2 === 1 && this.headersSent) {
        handle.markEnding();
      }
    ...
    _final(cb) {
      ...
      if (this[kSession]?.[kType] === NGHTTP2_SESSION_CLIENT && onClientStreamBodySentChannel.hasSubscribers) {
      ...
      ReflectApply(shutdownWritable, this, [
        cb
      ]);
    }
    _read(nread) {
    ...
    sendTrailers(headers) {
      ...
      this[kUpdateTimer]();
      const headersList = buildNgHeaderString(headers, assertValidPseudoHeaderTrailer, this.session[kStrictSingleValueFields]);
    ...
    _destroy(err, callback) {
      ...
      // redundant.js).
      if (err == null && code !== NGHTTP2_NO_ERROR && code !== NGHTTP2_CANCEL && sessionState.sentGoawayCode == null) {
      ...
      if (otelSpan !== undefined) {
        if (err != null) {
          ...
          updateSpanFromError(otelSpan, new ERR_HTTP2_STREAM_ERROR(nameForErrorCode[code] || code));
        }
        otelSpan.end();
  ...
  function prepareResponseHeaders(stream, headersParam, options) {
    ...
    if (ArrayIsArray(headersParam)) {
      ({ headers, statusCode } = prepareResponseHeadersArray(headersParam, options));
      stream[kRawHeaders] = headers;
    } else {
      ({ headers, statusCode } = prepareResponseHeadersObject(headersParam, options));
      stream[kSentHeaders] = headers;
    }
    const headersList = buildNgHeaderString(headers, assertValidPseudoHeaderResponse, stream.session[kStrictSingleValueFields]);
    return {
      headers,
      headersList,
      statusCode
  ...
  function processRespondWithFD(self, fd, headers, offset = 0, length = -1, streamOptions = 0) {
    ...
    const ret = ReflectApply(BindingHttp2Stream.prototype.respond, self[kHandle], [
      headersList[0],
      headersList[1],
      streamOptions
    ]);
  ...
  function doSendFD(session, options, fd, headers, streamOptions, err, stat) {
    ...
    if (typeof options.statCheck === "function" && FunctionPrototypeCall(options.statCheck, this, stat, headers, statOptions) === false || this[kState].flags & STREAM_FLAGS_HEADERS_SENT) {
      return;
    }
  ...
  function doSendFileFD(session, options, fd, headers, streamOptions, err, stat) {
    ...
    if (typeof options.statCheck === "function" && FunctionPrototypeCall(options.statCheck, this, stat, headers) === false || this[kState].flags & STREAM_FLAGS_HEADERS_SENT) {
      tryClose(fd);
      return;
    }
    if (stat.isFile()) {
  ...
  class ServerHttp2Stream extends Http2Stream {
    ...
    pushStream(headers, options, callback) {
      ...
      const headersList = buildNgHeaderString(headers, assertValidPseudoHeader, this.session[kStrictSingleValueFields]);
      const streamOptions = options.endStream ? STREAM_OPTION_EMPTY_PAYLOAD : 0;
      const ret = ReflectApply(BindingHttp2Stream.prototype.pushPromise, this[kHandle], [
        headersList[0],
        headersList[1],
        streamOptions
      ]);
    ...
    respond(headersParam, options) {
      ...
      const { headers, headersList, statusCode } = prepareResponseHeaders(this, headersParam, options);
      ...
      const ret = ReflectApply(BindingHttp2Stream.prototype.respond, this[kHandle], [
        headersList[0],
        headersList[1],
        streamOptions
      ]);
    ...
    respondWithFD(fd, headersParam, options) {
      ...
      if (ObjectPrototypeIsPrototypeOf(FsFileHandle.prototype, fd)) {
        ...
        throw new ERR_INVALID_ARG_TYPE("fd", [
          "number",
          "FileHandle"
        ], fd);
      }
      ...
      this[kUpdateTimer]();
      this.ownsFd = false;
      const { headers, statusCode } = prepareResponseHeadersObject(headersParam, options);
      setOtelServerStatus(this, statusCode);
    ...
    additionalHeaders(headers) {
      ...
      this[kUpdateTimer]();
      const headersList = buildNgHeaderString(headers, assertValidPseudoHeaderResponse, this.session[kStrictSingleValueFields]);
  ...
  function setupHandle(socket, type, options) {
    ...
    const handle = new InternalHttp2Session(type, options.strictFieldWhitespaceValidation === false);
    handle[kOwner] = this;
    ...
    handle.ongracefulclosecomplete = FunctionPrototypeBind(this[kMaybeDestroy], this, null);
    ...
    handle.onstreamclose = ()=>{
      ...
      if (!session.destroyed) {
        ...
        const closeCode = goawayCode === NGHTTP2_FLOW_CONTROL_ERROR ? NGHTTP2_FLOW_CONTROL_ERROR : NGHTTP2_CANCEL;
        // deno-lint-ignore deno-internal/prefer-primordials
    ...
    if (isUint32(options.maxSessionInvalidFrames)) {
      const uint32 = new Uint32Array(TypedArrayPrototypeGetBuffer(this[kNativeFields]), kSessionMaxInvalidFrames, 1);
    ...
    if (isUint32(options.maxSessionRejectedStreams)) {
      const uint32 = new Uint32Array(TypedArrayPrototypeGetBuffer(this[kNativeFields]), kSessionMaxRejectedStreams, 1);
  ...
  function closeSession(session, code, error) {
    ...
    if (handle !== undefined && typeof code === "number" && (state.flags & SESSION_FLAGS_CLOSED) === 0 && socket && !socket.destroyed && typeof handle.goaway === "function") {
  ...
  function socketOnError(error) {
    ...
    if (session !== undefined) {
      ...
      if ((error.code === "ECONNRESET" || error.code === "EPIPE") && (session[kState].goawayCode !== null || session.closed || session.destroyed)) {
  ...
  class Http2Session extends EventEmitter {
    constructor(type, options, socket){
      ...
      this[kTimeout] = null;
      this[kHandle] = undefined;
      this[kMaxOutstandingSettings] = MathMin(options.maxOutstandingSettings | 0, 2 ** 31 - 1) || 10;
      this[kMaxOutstandingPings] = MathMin(options.maxOutstandingPings | 0, 2 ** 31 - 1) || 10;
      ...
      if (socket.connecting || socket.secureConnecting) {
        const connectEvent = ObjectPrototypeIsPrototypeOf(tls.TLSSocket.prototype, socket) ? "secureConnect" : "connect";
    ...
    ping(payload, callback) {
      ...
      const cb = (ack, duration, ackPayload)=>{
        try {
          asyncResource.runInAsyncScope(userCb, this, ack, duration, ackPayload);
        } finally{
      ...
      const buf = payload ? new Uint8Array(payload.buffer, payload.byteOffset, payload.byteLength) : Buffer.alloc(8);
  ...
  class ServerHttp2Session extends Http2Session {
    constructor(options, socket, server){
      ...
      this[kNativeFields][kSessionPriorityListenerCount] = server ? server.listenerCount("priority") : 0;
    }
    get server() {
      return this[kServer];
    }
  ...
  class ClientHttp2Session extends Http2Session {
    ...
    request(headersParam, options) {
      ...
      if (otelState.TRACING_ENABLED) {
        ...
        if (ArrayIsArray(headersParam)) {
          for(let i = 0; i < headersParam.length; i += 2){
            if (StringPrototypeToLowerCase(headersParam[i]) === HTTP2_HEADER_METHOD) {
        ...
        span = builtinTracer().startSpan(spanMethod ?? "GET", {
          kind: 2
        });
      ...
      try {
        ...
        let method;
        if (ArrayIsArray(headersParam)) {
          ({ rawHeaders, headersList, scheme, authority, method } = prepareRequestHeadersArray(headersParam, this));
        } else if (!!headersParam && typeof headersParam === "object") {
          ({ headersObject, headersList, scheme, authority, method } = prepareRequestHeadersObject(headersParam, this));
        } else if (headersParam === undefined) {
          ({ headersObject, headersList, scheme, authority, method } = prepareRequestHeadersObject({}, this));
        } else {
          throw new ERR_INVALID_ARG_TYPE("headers", [
            "Object",
            "Array"
          ], headersParam);
        }
        ...
        if (span) {
          ...
          if (headersObject) {
            ...
            for(let i = 0; i < rawHeaders.length; i += 2){
              if (StringPrototypeToLowerCase(rawHeaders[i]) === HTTP2_HEADER_PATH) {
                path = rawHeaders[i + 1];
                break;
              }
            }
        ...
        if (span !== undefined && (stream === undefined || stream[kOtelSpan] === span)) {
  ...
  function initializeOptions(options) {
    ...
    options.Http1IncomingMessage ||= http1Options.IncomingMessage || http.IncomingMessage;
    options.Http1ServerResponse ||= http1Options.ServerResponse || http.ServerResponse;
  ...
  function initializeTLSOptions(options, servername) {
    ...
    if (options.ALPNCallback) {
      ...
      const selected = options.ALPNCallback({
        servername,
        protocols: []
      });
      if (typeof selected !== "string" || selected.length === 0) {
        throw new ERR_INVALID_ARG_VALUE("options.ALPNCallback", selected, "must synchronously return a non-empty protocol string; " + "dynamic per-handshake ALPN selection is not supported");
      }
 ...
 */ function closeAllSessions(server) {
  ...
  function http2ServerOnCaptureRejection(superCtor, self, err, event, args) {
    switch(event){
      ...
      default:
        ...
        ReflectApply(superCtor.prototype[EventEmitter.captureRejectionSymbol], self, args);
    }
  }
      ...
      case "tcp":
        {
          ...
          this.once("listening", ()=>notifyAddressOverrideServing(SERVER_KIND_NODE_HTTP2));
          return FunctionPrototypeApply(net.Server.prototype.listen, this, rewritten);
        }
      case "override-only":
        {
          const last = args[args.length - 1];
  ...
  function connect(authority, options, listener) {
    ...
    assertIsObject(authority, "authority", [
      "string",
      "Object",
      "URL"
    ]);
  ...
  function initCallbacks() {
    if (_init) return;
    op_http2_callbacks(onSessionInternalError, onPriority, onSettings, onPing, onSessionHeaders, onFrameError, onGoawayData, onAltSvc, onOrigin, onStreamTrailers, onStreamClose);
    _init = true;
  }
  ...
  const KNOWN_SETTING_ENTRIES = [
    [
      "headerTableSize",
      SETTING_ID_HEADER_TABLE_SIZE
    ],
    [
      "enablePush",
      SETTING_ID_ENABLE_PUSH
    ],
    [
      "maxConcurrentStreams",
      SETTING_ID_MAX_CONCURRENT_STREAMS
    ],
    [
      "initialWindowSize",
      SETTING_ID_INITIAL_WINDOW_SIZE
    ],
    [
      "maxFrameSize",
      SETTING_ID_MAX_FRAME_SIZE
    ],
    [
      "maxHeaderListSize",
      SETTING_ID_MAX_HEADER_LIST_SIZE
    ],
    [
      "enableConnectProtocol",
      SETTING_ID_ENABLE_CONNECT_PROTOCOL
    ]
  ];
  function getPackedSettings(settings) {
    ...
    validateSettings(settings);
    const entries = [];
    for (const { 0: name, 1: id } of new SafeArrayIterator(KNOWN_SETTING_ENTRIES)){
    ...
    if (settings.customSettings) {
      for (const key of new SafeArrayIterator(ObjectKeys(settings.customSettings))){
        ArrayPrototypePush(entries, [
          Number(key),
  ...
  function getUnpackedSettings(buf) {
    if (// deno-lint-ignore deno-internal/prefer-primordials
    !Buffer.isBuffer(buf) && !(ArrayBufferIsView(buf) && !(buf instanceof DataView))) {
