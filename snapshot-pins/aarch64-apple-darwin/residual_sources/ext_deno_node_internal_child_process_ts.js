"use strict"; return ((function() {
  ...
  const { ArrayBufferIsView, ArrayPrototypeFindIndex, ArrayPrototypeIncludes, ArrayPrototypeIndexOf, ArrayPrototypeMap, DataViewPrototypeGetBuffer, DataViewPrototypeGetByteLength, DataViewPrototypeGetByteOffset, Error, ObjectEntries, ObjectKeys, ObjectPrototype, ObjectPrototypeIsPrototypeOf, ObjectSetPrototypeOf, PromisePrototypeThen, PromiseWithResolvers, ReflectHas, RegExpPrototypeExec, RegExpPrototypeTest, SafeArrayIterator, SafeMap, SafeMapIterator, SafePromiseAll, SafeRegExp, SafeSet, SafeWeakMap, String, StringPrototypeEndsWith, StringPrototypeIndexOf, StringPrototypeMatch, StringPrototypeReplace, StringPrototypeSubstring, StringPrototypeTrimStart, Symbol, SymbolDispose, TypeErrorPrototype, TypedArrayPrototypeGetBuffer, TypedArrayPrototypeGetByteLength, TypedArrayPrototypeGetByteOffset } = primordials;
  const { isTypedArray } = core;
  const { op_node_in_npm_package, op_node_ipc_buffer_constructor, op_node_ipc_read_advanced, op_node_ipc_read_json, op_node_ipc_ref, op_node_ipc_unref, op_node_ipc_write_advanced, op_node_ipc_write_json, op_node_parse_shell_args, op_node_translate_cli_args } = core.ops;
  const { ArrayIsArray, ArrayPrototypeFilter, ArrayPrototypeJoin, ArrayPrototypePush, ArrayPrototypeSlice, ArrayPrototypeSort, ArrayPrototypeUnshift, ObjectHasOwn, StringPrototypeIncludes, StringPrototypeStartsWith, StringPrototypeToUpperCase } = core.loadExtScript("ext:deno_node/internal/primordials.mjs");
  ...
  const { AbortError, ERR_INVALID_ARG_TYPE, ERR_INVALID_ARG_VALUE, ERR_INVALID_HANDLE_TYPE, ERR_INVALID_SYNC_FORK_INPUT, ERR_IPC_CHANNEL_CLOSED, ERR_IPC_ONE_PIPE, ERR_IPC_SYNC_FORK, ERR_MISSING_ARGS, ERR_UNKNOWN_SIGNAL } = core.loadExtScript("ext:deno_node/internal/errors.ts");
  const { Buffer } = core.loadExtScript("ext:deno_node/internal/buffer.mjs");
  const { FastBuffer } = core.loadExtScript("ext:deno_node/internal/buffer.mjs");
  const { ERR_IPC_DISCONNECTED, errnoException } = core.loadExtScript("ext:deno_node/internal/errors.ts");
  const { codeMap } = core.loadExtScript("ext:deno_node/internal_binding/uv.ts");
  const { validateBoolean, validateInt32, validateObject, validateOneOf, validateString } = core.loadExtScript("ext:deno_node/internal/validators.mjs");
  const { kEmptyObject } = core.loadExtScript("ext:deno_node/internal/util.mjs");
  const lazyFsUtils = core.createLazyLoader("ext:deno_node/internal/fs/utils.mjs");
  const lazyProcess = core.createLazyLoader("node:process");
  const { StringPrototypeSlice } = core.loadExtScript("ext:deno_node/internal/primordials.mjs");
  const { Pipe, socketType } = core.loadExtScript("ext:deno_node/internal_binding/pipe_wrap.ts");
  const { socketType: tcpSocketType, TCP } = core.loadExtScript("ext:deno_node/internal_binding/tcp_wrap.ts");
  const lazyNet = core.createLazyLoader("node:net");
  const { channel: createDiagnosticsChannel, tracingChannel } = core.loadExtScript("ext:deno_node/diagnostics_channel.js");
  ...
  const lazyDgramInternal = ()=>core.loadExtScript("ext:deno_node/internal/dgram.ts");
  const lazyUdpWrap = ()=>core.loadExtScript("ext:deno_node/internal_binding/udp_wrap.ts");
  const { kNeedsNpmProcessState, nodeSpawnChild, nodeSpawnSyncChild } = core.loadExtScript("ext:deno_process/40_process.js");
  ...
  const shellVarMatchRe = new SafeRegExp('^(?:"\\$\\{([^}]+)\\}"|"\\$([A-Za-z_][A-Za-z0-9_]*)"|\\$\\{([^}]+)\\}|\\$([A-Za-z_][A-Za-z0-9_]*))');
   ...
   */ spawn(options) {
      ...
      if (childProcessSpawnChannel.hasSubscribers) {
        childProcessSpawnChannel.start.publish({
          process: this,
          options
        });
      }
   ...
   */ #spawnInternal(command, args, options) {
      const { stdio = [
        "pipe",
        "pipe",
        "pipe"
      ], cwd, signal, windowsVerbatimArguments = false, windowsHide = true, detached, envPairs, uid, gid } = options;
      ...
      if (envPairs) {
        for (const pair of new SafeArrayIterator(envPairs)){
          ...
          if (idx !== -1) {
            env[StringPrototypeSubstring(pair, 0, idx)] = StringPrototypeSubstring(pair, idx + 1);
          }
        }
      }
      const serialization = options.serialization || "json";
      const normalizedStdio = normalizeStdioOption(stdio);
      const stdin = normalizedStdio[0] === undefined ? "pipe" : normalizedStdio[0];
      const stdout = normalizedStdio[1] === undefined ? "pipe" : normalizedStdio[1];
      const stderr = normalizedStdio[2] === undefined ? "pipe" : normalizedStdio[2];
      ...
      const builtCommand = buildCommand(command, ArrayPrototypeSlice(args, 1), env);
      ...
      this.spawnargs = [
        cmd,
        ...new SafeArrayIterator(cmdArgs)
      ];
      const ipc = ArrayPrototypeIndexOf(normalizedStdio, "ipc");
      if (ipc !== -1 && ArrayPrototypeIndexOf(normalizedStdio, "ipc", ipc + 1) !== -1) {
      ...
      if (isWindows && (uid != null || gid != null)) {
        throw _createSpawnError("ENOTSUP", command, ArrayPrototypeSlice(args, 1));
      }
      try {
        this.#process = nodeSpawnChild(cmd, {
          ...
          needsNpmProcessState: options[kNeedsNpmProcessState] || includeNpmProcessState,
          ...
          const onAbortListener = ()=>{
            try {
              if (this.kill(killSignal)) {
                this.emit("error", new AbortError(undefined, {
                  cause: signal.reason
                }));
              }
            } catch (err) {
        ...
        (async ()=>{
          ...
          const windowsSignal = status.success ? null : this.#windowsSignalFallback;
        ...
        if (childProcessSpawnChannel.hasSubscribers) {
          childProcessSpawnChannel.end.publish({
            process: this
          });
        }
      } catch (err) {
        ...
        if (ObjectPrototypeIsPrototypeOf(Deno.errors.NotFound.prototype, e)) {
          ...
          e = _createSpawnError("ENOENT", command, ArrayPrototypeSlice(args, 1));
        } else if (ObjectPrototypeIsPrototypeOf(Deno.errors.PermissionDenied.prototype, e)) {
          ...
          if (uid != null || gid != null) {
            if (childProcessSpawnChannel.hasSubscribers) {
              const epermErr = _createSpawnError("EPERM", command, ArrayPrototypeSlice(args, 1));
            ...
            throw _createSpawnError("EPERM", command, ArrayPrototypeSlice(args, 1));
          }
          e = _createSpawnError("EACCES", command, ArrayPrototypeSlice(args, 1));
        }
      ...
      try {
        ...
        if (isWindows) {
          ...
          try {
            ...
            const alreadyClosed = ObjectPrototypeIsPrototypeOf(TypeErrorPrototype, err2) || ObjectPrototypeIsPrototypeOf(Deno.errors.NotFound.prototype, err2) || ObjectPrototypeIsPrototypeOf(Deno.errors.PermissionDenied.prototype, err2);
            if (!alreadyClosed) {
              throw err2;
            }
            return false;
          ...
          const alreadyClosed = ObjectPrototypeIsPrototypeOf(TypeErrorPrototype, err) || ObjectPrototypeIsPrototypeOf(Deno.errors.NotFound.prototype, err) || ObjectPrototypeIsPrototypeOf(Deno.errors.PermissionDenied.prototype, err);
          if (!alreadyClosed) {
            throw err;
          }
    ...
    async #_waitForChildStreamsToClose() {
      ...
      if (this.stdout && !this.stdout.destroyed && ObjectPrototypeIsPrototypeOf(Readable.prototype, this.stdout)) {
        ArrayPrototypePush(promises, waitForReadableToClose(this.stdout));
      }
      // Only readable streams need to be closed
      if (this.stderr && !this.stderr.destroyed && ObjectPrototypeIsPrototypeOf(Readable.prototype, this.stderr)) {
  ...
  function copyProcessEnvToEnv(env, name, optionEnv) {
    if (Deno.env.get(name) && (!optionEnv || !ObjectHasOwn(optionEnv, name))) {
  ...
  function normalizeStdioOption(stdio = [
    ...
    if (ArrayIsArray(stdio)) {
      ...
      if (stdio.length === 3 && stdio[0] === 0 && stdio[1] === 1 && stdio[2] === 2) {
        return [
          "inherit",
          "inherit",
          "inherit"
      ...
      if (stdio.length === 3 && stdio[0] === null && stdio[1] === null && stdio[2] === null) {
        return [
          "pipe",
          "pipe",
          "pipe"
 ...
 */ function getValidStdio(stdio, sync) {
    ...
    for(let i = 0; i < stdio.length; i++){
      ...
      if (value === "ipc") {
        ...
        if (value.fd !== undefined && typeof value.fd === "number") {
          ArrayPrototypePush(result, {
            type: "fd",
            fd: value.fd
          });
    ...
    if (options.shell) {
      ...
      if (args.length > 0) {
        ...
        const escapedParts = [
          ...
          ...new SafeArrayIterator(ArrayPrototypeMap(args, (arg)=>escapeShellArg(stripShellArgQuotes(arg))))
        ];
 ...
 */ function stripShellArgQuotes(arg) {
    ...
    const doubleQuoted = StringPrototypeStartsWith(arg, '"') && StringPrototypeEndsWith(arg, '"');
    // cmd.exe treats single quotes as ordinary characters, while POSIX shells
    // recognize both single and double quotes.
    const singleQuoted = lazyProcess().default.platform !== "win32" && StringPrototypeStartsWith(arg, "'") && StringPrototypeEndsWith(arg, "'");
 ...
 */ function transformDenoShellCommand(command, env, isCmdExe = false) {
    ...
    if (StringPrototypeStartsWith(command, quotedDenoPath)) {
      ...
      if (shellVarMatch) {
        const varName = shellVarMatch[1] || shellVarMatch[2] || shellVarMatch[3] || shellVarMatch[4];
    ...
    if (!startsWithDeno) {
      ...
      const operatorMatch = StringPrototypeMatch(command, shellCompoundOperatorRe);
      if (operatorMatch) {
    ...
    const rest = StringPrototypeTrimStart(StringPrototypeSlice(command, denoPathLength));
    if (rest.length === 0) {
      return command;
    }
    // Parse the command using the shell parser to separate arguments from
    // shell operators (redirections, pipes, etc.).
    const { args, shell_suffix: shellSuffix } = op_node_parse_shell_args(rest, isCmdExe);
    try {
      ...
      const quotedArgs = isWindows ? ArrayPrototypeMap(result.denoArgs, (a)=>{
        ...
        if (RegExpPrototypeTest(winQuoteCharsRe, a)) {
          let escaped = StringPrototypeReplace(a, winEscapeQuotesRe, '$1$1\\"');
          escaped = StringPrototypeReplace(escaped, winTrailingBackslashRe, "$1$1");
          return `"${escaped}"`;
        }
        return a;
        ...
        const unsafeInDoubleQuotes = RegExpPrototypeTest(unsafeInDoubleQuotesRe, a);
      ...
      const prefix = shellVarPrefix || StringPrototypeSlice(command, 0, denoPathLength);
      ...
      if (env) {
        ...
        if (pipeMatch) {
          const afterPipe = StringPrototypeSlice(shellSuffix, pipeMatch[0].length);
          const transformedAfter = transformDenoShellCommand(afterPipe, env, isCmdExe);
          if (transformedAfter !== afterPipe) {
            transformed = prefix + " " + ArrayPrototypeJoin(quotedArgs, " ") + " | " + transformedAfter;
          }
        }
      }
      return transformed;
 ...
 */ function findFirstNonFlagArg(args) {
    for(let i = 0; i < args.length; i++){
      ...
      if (arg === "-e" || arg === "--eval" || arg === "-p" || arg === "--print" || arg === "-r" || arg === "--require" || arg === "-C" || arg === "--conditions" || arg === "-c" || arg === "--check" || arg === "--import" || arg === "--loader" || arg === "--experimental-loader") {
 ...
 */ function buildCommand(file, args, env) {
    ...
    if (file === Deno.execPath() && !Deno.build.standalone) {
      ...
      const scriptInNpmPackage = firstNonFlagArg !== null ? op_node_in_npm_package(firstNonFlagArg) : false;
    ...
    if (file !== Deno.execPath() && args.length >= 2 && args[0] === "-c") {
    ...
    if (file !== Deno.execPath() && args.length >= 4 && args[0] === "/d" && args[1] === "/s" && args[2] === "/c") {
      let cmdStr = args[3];
      // Remove wrapping quotes added by normalizeSpawnArguments
      const hasWrappingQuotes = StringPrototypeStartsWith(cmdStr, '"') && StringPrototypeEndsWith(cmdStr, '"');
    ...
    if (file !== Deno.execPath()) {
      const denoPath = Deno.execPath();
      const denoArgIndex = ArrayPrototypeFindIndex(args, (arg)=>arg === denoPath);
      if (denoArgIndex !== -1) {
        ...
        if (argsForDeno.length > 0) {
          try {
            ...
            args = [
              ...new SafeArrayIterator(ArrayPrototypeSlice(args, 0, denoArgIndex + 1)),
  ...
  /** Convert a killSignal (string name or number) to its string name. */ function _resolveKillSignalName(killSignal) {
  ...
  function _createSpawnError(status, command, args = [], sync = false) {
    const syscall = sync ? "spawnSync " : "spawn ";
    const error = errnoException(codeMap.get(status), syscall + command);
    error.path = command;
  ...
  function normalizeInput(input) {
    ...
    if (ArrayBufferIsView(input)) {
      if (isTypedArray(input)) {
        return Buffer.from(TypedArrayPrototypeGetBuffer(input), TypedArrayPrototypeGetByteOffset(input), TypedArrayPrototypeGetByteLength(input));
      }
      ...
      return Buffer.from(DataViewPrototypeGetBuffer(input), DataViewPrototypeGetByteOffset(input), DataViewPrototypeGetByteLength(input));
    }
  ...
  function spawnSync(options) {
    const { env = Deno.env.toObject(), input, stdio = [
      "pipe",
      "pipe",
      "pipe"
    ], cwd, encoding, uid, gid, maxBuffer, timeout, killSignal, windowsVerbatimArguments = false, windowsHide = true } = options;
    let command = options.file || "";
    let args = options.args || [];
    const normalizedStdio = normalizeStdioOption(stdio);
    const stdin_ = normalizedStdio[0] === undefined ? "pipe" : normalizedStdio[0];
    const stdout_ = normalizedStdio[1] === undefined ? "pipe" : normalizedStdio[1];
    const stderr_ = normalizedStdio[2] === undefined ? "pipe" : normalizedStdio[2];
    ...
    const argsToProcess = args && args.length > 0 ? ArrayPrototypeSlice(args, 1) : [];
    const builtCommand = buildCommand(command, argsToProcess, env);
    ...
    try {
      ...
      if (output.killedByMaxBuffer || stdout && stdout.length > maxBuffer || stderr && stderr.length > maxBuffer) {
      ...
      result.signal = killedByDeno ? _resolveKillSignalName(killSignal) : output.signal;
  ...
  function getIpcHandleInfo(handle, options, target) {
    ...
    if (ObjectPrototypeIsPrototypeOf(Socket.prototype, handle)) {
      ...
      if (closeAfterSend && handle.server && typeof handle.server._setupWorker === "function") {
    ...
    if (ObjectPrototypeIsPrototypeOf(TCP.prototype, handle) || ObjectPrototypeIsPrototypeOf(Pipe.prototype, handle)) {
      return {
        ...
        message: {
          ...
          // Distinguishes the wrap type the receiver should reconstruct.
          nativeKind: ObjectPrototypeIsPrototypeOf(TCP.prototype, handle) ? "tcp" : "pipe",
          msg: undefined
        },
  ...
  function createIpcHandle(message, rawFd) {
    ...
    if (message.type === IPC_HANDLE_NET_SOCKET) {
      const inner = message.nativeKind === "pipe" ? new Pipe(socketType.SOCKET) : new TCP(tcpSocketType.SOCKET);
    ...
    if (message.type === IPC_HANDLE_NET_SERVER) {
      const inner = message.nativeKind === "pipe" ? new Pipe(socketType.SERVER) : new TCP(tcpSocketType.SERVER);
      const err = inner.open(rawFd);
      if (err !== 0) {
        throw errnoException(codeMap.get(err), "open");
      }
    ...
    if (message.type === IPC_HANDLE_NET_NATIVE) {
      if (message.nativeKind === "pipe") {
        const st = message.socketType === 1 ? socketType.SERVER : socketType.SOCKET;
      ...
      const st = message.socketType === 1 ? tcpSocketType.SERVER : tcpSocketType.SOCKET;
  ...
  function isChannelClosedError(err) {
    return ObjectPrototypeIsPrototypeOf(Deno.errors.BadResource.prototype, err) || ObjectPrototypeIsPrototypeOf(Deno.errors.ConnectionReset.prototype, err);
  }
  ...
  function setupChannel(target, ipc, serialization) {
    const { Socket } = lazyNet();
    const { Server: NetServer } = lazyNet();
    const { Socket: DgramSocket } = lazyDgram();
    ...
    const writeFn = serialization === "json" ? op_node_ipc_write_json : op_node_ipc_write_advanced;
    const readFn = serialization === "json" ? op_node_ipc_read_json : op_node_ipc_read_advanced;
    ...
    function sendHandleAck() {
      ...
      control.refCounted();
      PromisePrototypeThen(writeFn(ipc, {
        cmd: "NODE_HANDLE_ACK"
      }, NO_RAW_FD, queueOk), ()=>control.unrefCounted(), ()=>control.unrefCounted());
    }
    ...
    function sendInternalMessage(message) {
      ...
      control.refCounted();
      PromisePrototypeThen(writeFn(ipc, message, NO_RAW_FD, queueOk), ()=>control.unrefCounted(), ()=>control.unrefCounted());
    }
    ...
    function handleSocketListMessage(message) {
      ...
      if (message.cmd === NODE_SOCKET_NOTIFY_CLOSE) {
        const socketList = receivedSocketLists.get(message.key);
        const sendAck = ()=>sendInternalMessage({
    ...
    function dispatch(message, handleInfo, callback) {
      ...
      control.refCounted();
      const writePromise = handleInfo ? writeFn(ipc, handleInfo.message, handleInfo.rawFd, queueOk) : writeFn(ipc, message, NO_RAW_FD, queueOk);
      PromisePrototypeThen(writePromise, ()=>{
        ...
        if (!sigkillIssuedAtDispatch && target[kSigkillIssued] && (isBrokenPipeError(err) || isChannelClosedError(err))) {
        ...
        while(true){
          ...
          if (isInternal(msg)) {
            ...
            if (cmd === "CLOSE") {
              ...
              if (msg.socketListKey && ObjectPrototypeIsPrototypeOf(Socket.prototype, handle)) {
    ...
    function handleMessage(msg, handle) {
      ...
      ArrayPrototypePush(target.channel[kPendingMessages], [
        msg,
        handle
      ]);
    }
    target.on("newListener", ()=>{
      nextTick(()=>{
        ...
        for (const pending of new SafeArrayIterator(target.channel[kPendingMessages])){
    ...
    target.send = function(message, handle, options, callback) {
      ...
      if (typeof message !== "string" && typeof message !== "object" && typeof message !== "number" && typeof message !== "boolean" && typeof message !== "bigint") {
        throw new ERR_INVALID_ARG_TYPE("message", [
          "string",
          "object",
          "number",
          "boolean"
      ...
      if (handle) {
        if (!ObjectPrototypeIsPrototypeOf(Socket.prototype, handle) && !ObjectPrototypeIsPrototypeOf(NetServer.prototype, handle) && !ObjectPrototypeIsPrototypeOf(DgramSocket.prototype, handle) && !ObjectPrototypeIsPrototypeOf(TCP.prototype, handle) && !ObjectPrototypeIsPrototypeOf(Pipe.prototype, handle)) {
          throw new ERR_INVALID_HANDLE_TYPE();
        }
      } else {
        handle = undefined;
