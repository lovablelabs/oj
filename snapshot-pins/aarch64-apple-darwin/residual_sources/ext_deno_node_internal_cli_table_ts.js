"use strict"; return ((function() {
  ...
  const { getStringWidth } = core.loadExtScript("ext:deno_node/internal/util/inspect.mjs");
  const { ArrayPrototypeJoin, ArrayPrototypeMap, SafeArrayIterator, StringPrototypeRepeat, MathCeil, MathMax, ObjectHasOwn } = primordials;
  const tableChars = {
    middleMiddle: "\u2500",
    rowMiddle: "\u253c",
    topRight: "\u2510",
  ...
  const table = (head, columns)=>{
    ...
    const longestColumn = MathMax(...new SafeArrayIterator(ArrayPrototypeMap(columns, (a)=>a.length)));
