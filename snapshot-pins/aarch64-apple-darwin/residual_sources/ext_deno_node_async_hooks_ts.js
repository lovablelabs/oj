"use strict"; return ((function() {
  ...
  const { validateFunction, validateObject } = core.loadExtScript("ext:deno_node/internal/validators.mjs");
  const { AsyncHook, emitAfter, emitBefore, emitDestroy: emitDestroyHook, emitInit, enterAsyncResource, exitAsyncResource, executionAsyncId: internalExecutionAsyncId, executionAsyncResource: internalExecutionAsyncResource, newAsyncId } = core.loadExtScript("ext:deno_node/internal/async_hooks.ts");
  const { ObjectDefineProperties, ReflectApply, FunctionPrototypeBind, ArrayPrototypeUnshift, ObjectFreeze, SafeFinalizationRegistry, SafeArrayIterator, Symbol } = primordials;
  ...
  const asyncResourceRegistry = new SafeFinalizationRegistry((asyncId)=>emitDestroyHook(asyncId));
  ...
  class AsyncLocalStorage {
    ...
    run(store, callback, ...args) {
      this.enabled = true;
      const previous = this.#variable.enter(store === undefined ? kUndefinedStore : store);
      try {
    ...
    static snapshot() {
      ...
      return function(cb, ...args) {
        return resource.runInAsyncScope(cb, null, ...new SafeArrayIterator(args));
      };
    }
  }
