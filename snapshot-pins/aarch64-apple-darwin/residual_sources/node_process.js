...
import { op_current_thread_cpu_usage, op_fs_umask, op_getegid, op_geteuid, op_getgroups, op_inspector_close, op_inspector_enabled, op_inspector_port, op_node_load_env_file, op_node_process_constrained_memory, op_node_process_kill, op_node_process_resource_usage, op_node_process_set_title, op_node_process_setegid, op_node_process_seteuid, op_node_process_setgid, op_node_process_setuid, op_process_abort, op_stream_base_register_state } from "ext:core/ops";
...
const { onWarning } = core.loadExtScript("ext:deno_node/internal/process/warning.ts");
const { parseFileMode, validateBoolean, validateNumber, validateObject, validateString, validateUint32 } = core.loadExtScript("ext:deno_node/internal/validators.mjs");
const { denoErrorToNodeError, ERR_INVALID_ARG_TYPE, ERR_INVALID_ARG_VALUE_RANGE, ERR_OUT_OF_RANGE, ERR_UNCAUGHT_EXCEPTION_CAPTURE_ALREADY_SET, ERR_UNKNOWN_SIGNAL, ERR_WORKER_UNSUPPORTED_OPERATION, errnoException, NodeTypeError } = core.loadExtScript("ext:deno_node/internal/errors.ts");
...
const { arch: arch_, chdir, cwd, env, nextTick: _nextTick, version, versions } = core.loadExtScript("ext:deno_node/_process/process.ts");
...
const lazyStreamsMod = core.createLazyLoader("ext:deno_node/_process/streams.mjs");
...
const { isAndroid, isWindows } = core.loadExtScript("ext:deno_node/_util/os.ts");
...
const constants = core.loadExtScript("ext:deno_node/internal_binding/constants.ts");
const uv = core.loadExtScript("ext:deno_node/internal_binding/uv.ts");
const { buildAllowedFlags } = core.loadExtScript("ext:deno_node/internal/process/per_thread.mjs");
const { getActiveHandles, getActiveRequests, getActiveResourceNames } = core.loadExtScript("ext:deno_node/internal/process/active_resources.ts");
const { getActiveResourcesInfo: getTimerActiveResourcesInfo } = core.loadExtScript("ext:deno_node/internal/timers.mjs");
let fsUtilsModule;
const lazyLoadFsUtils = core.createLazyLoader("ext:deno_node/internal/fs/utils.mjs");
...
const lazyLoadUtil = core.createLazyLoader("node:util");
const { ArrayIsArray, ArrayPrototypeConcat, ArrayPrototypeFind, ArrayPrototypePush, BigInt, Error, ErrorCaptureStackTrace, ErrorPrototype, Float64Array, FunctionPrototypeBind, FunctionPrototypeCall, MathFloor, Number, NumberIsFinite, NumberIsInteger, NumberMAX_SAFE_INTEGER, NumberPrototypeToFixed, ObjectCreate, ObjectDefineProperty, ObjectEntries, ObjectFreeze, ObjectKeys, ObjectPrototypeIsPrototypeOf, Proxy, RangeError, ReflectApply, ReflectGet, ReflectGetOwnPropertyDescriptor, ReflectGetPrototypeOf, ReflectHas, ReflectOwnKeys, SafeArrayIterator, SafeMap, SafeWeakMap, SafeWeakSet, String, StringPrototypeStartsWith, SymbolToStringTag } = primordials;
export const argv = [
  "",
  ""
];
...
/** https://nodejs.org/api/process.html#processumaskmask */ export function umask(mask) {
  if (mask !== undefined) {
    if (internals.__isWorkerThread) {
...
export function cpuUsage(previousValue) {
  ...
  if (previousValue) {
    if (!previousCpuUsageValueIsValid(previousValue.user)) {
      ...
      throw new ERR_INVALID_ARG_VALUE_RANGE("prevValue.user", previousValue.user);
    }
    if (!previousCpuUsageValueIsValid(previousValue.system)) {
      validateNumber(previousValue.system, "prevValue.system");
      throw new ERR_INVALID_ARG_VALUE_RANGE("prevValue.system", previousValue.system);
    }
    return {
...
export function threadCpuUsage(previousValue) {
  if (previousValue) {
    if (!previousCpuUsageValueIsValid(previousValue.user)) {
      validateObject(previousValue, "prevValue");
      validateNumber(previousValue.user, "prevValue.user");
...
function getStdioActiveResources() {
  const result = [];
  const streams = [
    io.stdin,
    io.stdout,
...
export function constrainedMemory() {
...
function _kill(pid, sig) {
  const maybeMapErrno = (res)=>// the windows implementation is ported from libuv, so the error numbers already match libuv and don't need mapping
  ...
  const maybeSignal = ArrayPrototypeFind(ObjectEntries(constants.os.signals), (entry)=>entry[1] === sig);
  if (!maybeSignal) {
...
function wrapIdSetter(syscall, fn) {
  return function(id) {
    if (typeof id === "number") {
      validateUint32(id, "id");
      id >>>= 0;
    } else if (typeof id !== "string") {
      throw new ERR_INVALID_ARG_TYPE("id", [
        "number",
        "string"
      ], id);
    }
...
export { getegid, geteuid, getgid, getuid, report, setegid, seteuid, setgid, setuid };
...
// deno-lint-ignore no-explicit-any
const _dispatchedFatalErrors = new SafeWeakSet();
...
function _wrapSignalListener(event, listener) {
...
function _unwrapSignalListener(event, listener) {
...
function _findSignalListener(// deno-lint-ignore no-explicit-any
target, event, // deno-lint-ignore no-explicit-any
listener) {
...
/** https://nodejs.org/api/process.html#process_process_events */ Process.prototype.on = function(event, // deno-lint-ignore no-explicit-any
listener) {
  if (typeof event === "string" && StringPrototypeStartsWith(event, "SIG")) {
    ...
    } else if (event !== "SIGBREAK" && event !== "SIGINT" && event !== "SIGWINCH" && Deno.build.os === "windows") {
    // TODO(#26331): Ignores all signals except SIGBREAK, SIGINT, and SIGWINCH on windows.
      ...
      Deno.addSignalListener(event, _wrapSignalListener(event, listener));
    }
  } else {
    FunctionPrototypeCall(EventEmitter.prototype.on, this, event, listener);
  }
...
Process.prototype.off = function(event, // deno-lint-ignore no-explicit-any
listener) {
  if (typeof event === "string" && StringPrototypeStartsWith(event, "SIG")) {
    if (event === "SIGBREAK" && Deno.build.os !== "windows") {
    // Ignores SIGBREAK if the platform is not windows.
    ...
    // Ignores all signals except SIGBREAK, SIGINT, and SIGWINCH on windows.
      ...
      Deno.removeSignalListener(event, unwrapped);
    }
  } else {
    FunctionPrototypeCall(EventEmitter.prototype.off, this, event, listener);
  }
...
Process.prototype.emit = function(event, // deno-lint-ignore no-explicit-any
...args) {
  return ReflectApply(EventEmitter.prototype.emit, this, ArrayPrototypeConcat([
    event
  ], args));
};
Process.prototype.prependListener = function(event, // deno-lint-ignore no-explicit-any
listener) {
  if (typeof event === "string" && StringPrototypeStartsWith(event, "SIG")) {
    if (event === "SIGBREAK" && Deno.build.os !== "windows") {
    // Ignores SIGBREAK if the platform is not windows.
    } else {
      FunctionPrototypeCall(EventEmitter.prototype.prependListener, this, event, listener);
      Deno.addSignalListener(event, _wrapSignalListener(event, listener));
    }
  } else {
    FunctionPrototypeCall(EventEmitter.prototype.prependListener, this, event, listener);
...
Process.prototype.addListener = function(event, // deno-lint-ignore no-explicit-any
listener) {
  return this.on(event, listener);
};
Process.prototype.removeListener = function(event, listener) {
  return this.off(event, listener);
};
Process.prototype.removeAllListeners = function(// deno-lint-ignore no-explicit-any
event) {
  if (arguments.length === 0) {
    ...
    return FunctionPrototypeCall(EventEmitter.prototype.removeAllListeners, this);
  }
  if (typeof event === "string" && StringPrototypeStartsWith(event, "SIG")) {
    _removeAllSignalListeners(this, event);
  }
  return FunctionPrototypeCall(EventEmitter.prototype.removeAllListeners, this, event);
};
function _removeAllSignalListeners(// deno-lint-ignore no-explicit-any
target, event) {
  const events = target._events;
  ...
  if (typeof list === "function") {
    ...
    Deno.removeSignalListener(event, actual);
  } else {
...
const streamDelegates = new SafeWeakSet();
function makeStreamDelegate(name) {
  const delegate = new Proxy(ObjectCreate(null), {
    get (_target, prop) {
      ...
      return typeof value === "function" ? FunctionPrototypeBind(value, real) : value;
    },
    ...
    getOwnPropertyDescriptor (_target, prop) {
      const real = process[name];
      return real != null ? ReflectGetOwnPropertyDescriptor(real, prop) : undefined;
    },
...
/** https://nodejs.org/api/process.html#processrelease */ ObjectDefineProperty(process, "release", {
  __proto__: null,
  get () {
    return {
      name: "node",
...
/** https://nodejs.org/api/process.html#process_process_arch */ ObjectDefineProperty(process, "arch", {
  __proto__: null,
  get () {
    return arch;
  },
...
ObjectDefineProperty(process, "debugPort", {
  ...
  set (val) {
    ...
    if (port !== 0 && port < 1024 || port > 65535) {
      throw new RangeError("process.debugPort must be 0 or in range 1024 to 65535");
    }
    _debugPort = port;
...
/** https://nodejs.org/api/process.html#process_process_chdir_directory */ process.chdir = chdir;
/** https://nodejs.org/api/process.html#processconfig */ let _configCache;
ObjectDefineProperty(process, "config", {
  ...
  get () {
    if (_configCache === undefined) {
      ...
      try {
        forceSharedOpenssl = Deno.env.get("DENO_INTERNAL_NODE_TEST_FORCE_SHARED_OPENSSL") === "1";
      } catch  {
...
/** https://nodejs.org/api/process.html#process_process_cwd */ process.cwd = cwd;
/**
 * https://nodejs.org/api/process.html#process_process_env
 * Requires env permissions
 */ process.env = env;
/** https://nodejs.org/api/process.html#process_process_execargv */ process.execArgv = execArgv;
/** https://nodejs.org/api/process.html#process_process_exit_code */ process.exit = exit;
/** https://nodejs.org/api/process.html#processabort */ process.abort = abort;
/** https://nodejs.org/api/process.html#processopenStdin */ process.openStdin = ()=>{
...
// deno-lint-ignore no-explicit-any
let _uncaughtExceptionCaptureFn = null;
// deno-lint-ignore no-explicit-any
...
/** https://nodejs.org/api/process.html#processexitcode_1 */ ObjectDefineProperty(process, "exitCode", {
  __proto__: null,
  get () {
  ...
  set (code) {
    ...
    if (code == null) {
      ...
      if (code === "" || !NumberIsFinite(Number(code)) || !NumberIsInteger(Number(code))) {
...
/** https://nodejs.org/api/process.html#process_process_nexttick_callback_args */ process.nextTick = _nextTick;
process.dlopen = dlopen;
/** https://nodejs.org/api/process.html#process_process_pid */ ObjectDefineProperty(process, "pid", {
  __proto__: null,
  get () {
    return pid;
  }
});
/** https://nodejs.org/api/process.html#processppid */ ObjectDefineProperty(process, "ppid", {
  __proto__: null,
  get () {
    return Deno.ppid;
  }
});
/** https://nodejs.org/api/process.html#process_process_platform */ ObjectDefineProperty(process, "platform", {
  __proto__: null,
  get () {
    return platform;
  },
 ...
 */ process._kill = _kill;
/** https://nodejs.org/api/process.html#processkillpid-signal */ process.kill = kill;
process.memoryUsage = memoryUsage;
process.availableMemory = availableMemory;
process.constrainedMemory = constrainedMemory;
/** https://nodejs.org/api/process.html#processresourceusage */ process.resourceUsage = resourceUsage;
/** https://nodejs.org/api/process.html#process_process_stderr */ process.stderr = stderr;
/** https://nodejs.org/api/process.html#process_process_stdin */ process.stdin = stdin;
/** https://nodejs.org/api/process.html#process_process_stdout */ process.stdout = stdout;
/** https://nodejs.org/api/process.html#process_process_version */ process.version = version;
/** https://nodejs.org/api/process.html#process_process_versions */ process.versions = versions;
/** https://nodejs.org/api/process.html#process_process_emitwarning_warning_options */ process.emitWarning = emitWarning;
process.binding = (name)=>{
  return getBinding(name);
};
/** https://nodejs.org/api/process.html#processumaskmask */ process.umask = umask;
/** This method is removed on Windows */ process.getgid = getgid;
/** This method is removed on Windows */ process.getuid = getuid;
/** This method is removed on Windows */ process.getgroups = ()=>op_getgroups();
/** This method is removed on Windows */ process.getegid = getegid;
/** This method is removed on Windows */ process.geteuid = geteuid;
/** This method is removed on Windows */ process.setegid = setegid;
/** This method is removed on Windows */ process.seteuid = seteuid;
/** This method is removed on Windows */ process.setgid = setgid;
/** This method is removed on Windows */ process.setuid = setuid;
...
export function loadEnvFile(path = ".env") {
  ...
  try {
    ...
    if (ObjectPrototypeIsPrototypeOf(Deno.errors.InvalidData.prototype, err)) {
      throw new NodeTypeError("ERR_INVALID_ARG_TYPE", `Contents of '${path}' should be a valid string.`);
    }
    throw denoErrorToNodeError(err, {
      syscall: "open",
      path
...
/** https://nodejs.org/api/process.html#processexecpath */ ObjectDefineProperty(process, "execPath", {
  __proto__: null,
  get () {
    return String(execPath);
  },
...
/** https://nodejs.org/api/process.html#processuptime */ process.uptime = ()=>{
  return Number(NumberPrototypeToFixed(performance.now() / 1000, 9));
};
/** https://nodejs.org/api/process.html#processallowednodeenvironmentflags */ ObjectDefineProperty(process, "allowedNodeEnvironmentFlags", {
  __proto__: null,
  get () {
...
function synchronizeListeners() {
  ...
  // last.
  if (unhandledRejectionListenerCount > 0 || uncaughtExceptionListenerCount > 0 || uncaughtExceptionMonitorListenerCount > 0 || _uncaughtExceptionCaptureFn !== null) {
    internals.nodeProcessUnhandledRejectionCallback = (event)=>{
      if (process.listenerCount("unhandledRejection") === 0) {
        ...
        if (reason !== null && typeof reason === "object" && _dispatchedFatalErrors.has(reason)) {
          return;
        }
        ...
        if (!ObjectPrototypeIsPrototypeOf(ErrorPrototype, reason)) {
          const message = "This error originated either by throwing " + "inside of an async function without a catch block, or by rejecting a " + "promise which was not handled with .catch(). The promise rejected with the" + ` reason "${reason}".`;
  ...
  if (uncaughtExceptionListenerCount > 0 || uncaughtExceptionMonitorListenerCount > 0 || _uncaughtExceptionCaptureFn !== null) {
...
function mainModuleArgv(mainModule = Deno.mainModule) {
  if (Deno.build.standalone) {
...
// bootstrapped.
internals.__bootstrapNodeProcess = function(argv0Val, args, denoVersions, nodeDebug, warmup = false, runningOnMainThread = true) {
  if (!warmup) {
    ...
    const { streamBaseState } = core.loadExtScript("ext:deno_node/internal_binding/stream_wrap.ts");
    ...
    ObjectDefineProperty(process, "stdout", {
      ...
      enumerable: true,
      get () {
        return stdout != null && !streamDelegates.has(stdout) ? stdout : stdout = makeStdioWriteStream(1, io.stdout, "stdout");
      },
      set (v) {
        stdout = v;
      }
    ...
    ObjectDefineProperty(process, "stderr", {
      ...
      enumerable: true,
      get () {
        return stderr != null && !streamDelegates.has(stderr) ? stderr : stderr = makeStdioWriteStream(2, io.stderr, "stderr");
      },
      set (v) {
        stderr = v;
      }
    });
    core.loadExtScript("ext:deno_node/internal/console/constructor.mjs").bindStreamsLazy(globalThis.console, process);
    arch = arch_();
    ...
    if (getOptionValue("--pending-deprecation")) {
      ...
      uvBinding.errname = deprecate(uvBinding.errname, "Directly calling process.binding('uv').errname(<val>) is being " + "deprecated. Please make sure to use util.getSystemErrorName() " + "instead.", "DEP0119");
    ...
    if (!runningOnMainThread) {
      ...
      for(let i = 0; i < disabledFns.length; i++){
        ...
        const stub = function() {
          throw new ERR_WORKER_UNSUPPORTED_OPERATION(`process.${fn}()`);
        };
        stub.disabled = true;
        process[fn] = stub;
      }
  ...
  // that second call with "undefined is not a function".
    ...
    /** https://nodejs.org/api/process.html#process_process_stdout */ stdout = process.stdout = streams.createWritableStdioStream(io.stdout, "stdout", true);
    /** https://nodejs.org/api/process.html#process_process_stderr */ stderr = process.stderr = streams.createWritableStdioStream(io.stderr, "stderr", true);
  }
};
...
if (internals.__nodeBootstrapArgs !== undefined) {
  ...
  internals.__bootstrapNodeProcess(a.argv0, a.denoArgs, a.denoVersion, a.nodeDebug ?? "", false, a.runningOnMainThread);
