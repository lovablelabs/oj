"use strict"; return ((function() {
  ...
  const { DatabaseSync: DatabaseSyncOp, op_node_database_backup, Session, StatementSync } = core.ops;
  ...
  const { Error, FunctionPrototypeCall, NumberIsInteger, ObjectDefineProperty, ObjectDefineProperties, ObjectGetOwnPropertyDescriptor, ObjectPrototypeIsPrototypeOf, ObjectSetPrototypeOf, Proxy, ReflectConstruct, SafeSet, SafeWeakMap, StringPrototypeIncludes, SymbolDispose, SymbolFor, TypeError } = primordials;
  ...
  const nativeLimitsGetter = ObjectGetOwnPropertyDescriptor(DatabaseSyncOp.prototype, "limits").get;
  function createLimitsProxy(nativeLimits) {
    return new Proxy(nativeLimits, {
      ...
      has (_target, prop) {
        return typeof prop === "string" && LIMIT_NAMES_SET.has(prop);
      },
      ownKeys (_target) {
        return LIMIT_NAMES;
      },
  ...
  const parsePath = (path)=>{
    ...
    if (typeof parsedPath === "undefined" || StringPrototypeIncludes(parsedPath, "\0")) {
  ...
  function DatabaseSync(path, options) {
    ...
    return ReflectConstruct(DatabaseSyncOp, [
      parsePath(path),
      options
    ], new.target);
  }
  ...
  function validateBackupOptions(options) {
    ...
    if (options.rate !== undefined && !NumberIsInteger(options.rate)) {
      throw new InvalidArgTypeError('The "options.rate" argument must be an integer.');
    }
    if (options.source !== undefined && typeof options.source !== "string") {
      throw new InvalidArgTypeError('The "options.source" argument must be a string.');
    }
    if (options.target !== undefined && typeof options.target !== "string") {
      throw new InvalidArgTypeError('The "options.target" argument must be a string.');
    }
    if (options.progress !== undefined && typeof options.progress !== "function") {
      throw new InvalidArgTypeError('The "options.progress" argument must be a function.');
    }
  }
  ...
  function backup(sourceDb, path, options) {
    if (!ObjectPrototypeIsPrototypeOf(DatabaseSync.prototype, sourceDb)) {
      throw new InvalidArgTypeError('The "sourceDb" argument must be an object.');
    }
    if (!sourceDb.isOpen) {
  ...
  async function backupExec(sourceDb, parsedPath, options) {
    ...
    return op_node_database_backup(sourceDb, parsedPath, options);
  }
