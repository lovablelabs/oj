"use strict"; return ((function() {
  ...
  const { Error, MathMax, MathMin, MathTrunc, NumberIsNaN, TypedArrayPrototypeGetLength } = primordials;
  ...
  function searchBytes(source, end, needle, needleLength, offset, isForward, step) {
    if (isForward) {
  ...
  // needles.
  function indexOfBuffer(targetBuffer, buffer, byteOffset, encoding, forwardDirection, end) {
    ...
    const optOffset = indexOfOffset(haystackLength, byteOffset, needleLength, forwardDirection);
    if (needleLength === 0) {
    ...
    if (forwardDirection && needleLength + offset > searchEnd || needleLength > searchEnd) {
      return -1;
    }
    ...
    return searchBytes(targetBuffer, searchEnd, buffer, isUcs2 ? needleLength & ~1 : needleLength, isUcs2 ? offset & ~1 : offset, forwardDirection, isUcs2 ? 2 : 1);
  }
  // Mirror of `IndexOfNumberImpl` (node/src/node_buffer.cc).
  function indexOfNumber(targetBuffer, number, byteOffset, forwardDirection, end) {
    byteOffset = MathTrunc(byteOffset);
    end = toInt64(end);
    ...
    const optOffset = indexOfOffset(bufferLength, byteOffset, 1, forwardDirection);
