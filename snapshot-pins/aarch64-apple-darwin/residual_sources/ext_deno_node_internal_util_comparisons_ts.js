"use strict"; return ((function() {
  ...
  const { getOwnNonIndexProperties, ONLY_ENUMERABLE, SKIP_SYMBOLS } = core.loadExtScript("ext:deno_node/internal_binding/util.ts");
  const lazyAssert = core.createLazyLoader("node:assert");
  const { isAnyArrayBuffer, isArrayBufferView, isBigIntObject, isBooleanObject, isBoxedPrimitive, isCryptoKey, isDate, isFloat16Array, isFloat32Array, isFloat64Array, isKeyObject, isMap, isNumberObject, isPromise, isRegExp, isSet, isStringObject, isSymbolObject, isWeakMap, isWeakSet } = core.loadExtScript("ext:deno_node/internal/util/types.ts");
  ...
  const { Array, ArrayBuffer, ArrayIsArray, ArrayPrototypeFilter, ArrayPrototypePush, BigInt, BigInt64Array, BigIntPrototypeValueOf, BigUint64Array, Boolean, BooleanPrototypeValueOf, DataView, Date, DatePrototypeGetTime, Error, Float32Array, Float64Array, Function, Int16Array, Int32Array, Int8Array, Map, Number, NumberIsNaN, NumberPrototypeValueOf, Object, ObjectGetOwnPropertyDescriptor, ObjectGetOwnPropertySymbols: getOwnSymbols, ObjectGetPrototypeOf, ObjectIs, ObjectKeys, ObjectPrototypeHasOwnProperty: hasOwn, ObjectPrototypePropertyIsEnumerable: hasEnumerable, ObjectPrototypeToString, Promise, RegExp, SafeSet, Set, String, StringPrototypeValueOf, Symbol, SymbolPrototypeValueOf, TypedArrayPrototypeGetByteLength: getByteLength, TypedArrayPrototypeGetSymbolToStringTag, Uint16Array, Uint32Array, Uint8Array, Uint8ClampedArray, WeakMap, WeakSet } = primordials;
  ...
  const wellKnownConstructors = new SafeSet().add(Array).add(ArrayBuffer).add(BigInt).add(BigInt64Array).add(BigUint64Array).add(Boolean).add(Buffer).add(DataView).add(Date).add(Error)// TODO(Tango992): add Float16Array
  // .add(Float16Array)
  .add(Float32Array).add(Float64Array).add(Function).add(Int16Array).add(Int32Array).add(Int8Array).add(Map).add(Number).add(Object).add(Promise).add(RegExp).add(Set).add(String).add(Symbol).add(Uint16Array).add(Uint32Array).add(Uint8Array).add(Uint8ClampedArray).add(WeakMap).add(WeakSet);
  const kStrict = 1;
  const kLoose = 0;
  const kPartial = 2;
  ...
  function isPartialArrayBufferView(a, b) {
    ...
    return isPartialUint8Array(new Uint8Array(a.buffer, a.byteOffset, a.byteLength), new Uint8Array(b.buffer, b.byteOffset, b.byteLength));
  }
  ...
  function isEqualBoxedPrimitive(val1, val2) {
    ...
    /* c8 ignore next */ lazyAssert().fail(`Unknown boxed type ${val1}`);
  }
  function isEnumerableOrIdentical(val1, val2, prop, mode, memos) {
    return hasEnumerable(val2, prop) || // This is handled by Object.keys()
    mode === kPartial && (val2[prop] === undefined || prop === "message" && val2[prop] === "") || innerDeepEqual(val1[prop], val2[prop], mode, memos);
  }
  function innerDeepEqual(val1, val2, mode, memos) {
    ...
    if (mode !== kLoose) {
      ...
      if (val1 === null || typeof val1 !== "object") {
        return (val2 === null || typeof val2 !== "object") && // Check for NaN
  ...
  function objectComparisonStart(val1, val2, mode, memos) {
    ...
    if (ArrayIsArray(val1)) {
      if (!ArrayIsArray(val2) || val1.length !== val2.length && (mode !== kPartial || val1.length < val2.length)) {
        return false;
      }
      ...
      if (!isSet(val2) || val1.size !== val2.size && (mode !== kPartial || val1.size < val2.size)) {
        return false;
      }
      return keyCheck(val1, val2, mode, memos, valueType.isSet);
    } else if (isMap(val1)) {
      if (!isMap(val2) || val1.size !== val2.size && (mode !== kPartial || val1.size < val2.size)) {
        return false;
      }
      ...
      if (!isError(val2) || !isEnumerableOrIdentical(val1, val2, "message", mode, memos) || !isEnumerableOrIdentical(val1, val2, "name", mode, memos) || !isEnumerableOrIdentical(val1, val2, "cause", mode, memos) || !isEnumerableOrIdentical(val1, val2, "errors", mode, memos)) {
        return false;
      }
    ...
    } else if (ArrayIsArray(val2) || isArrayBufferView(val2) || isSet(val2) || isMap(val2) || isDate(val2) || isRegExp(val2) || isAnyArrayBuffer(val2) || isBoxedPrimitive(val2) || isError(val2)) {
      return false;
      ...
      if (!isCryptoKey(val2) || val1.extractable !== val2.extractable || !innerDeepEqual(val1.algorithm, val2.algorithm, mode, memos) || !innerDeepEqual(val1.usages, val2.usages, mode, memos) || !innerDeepEqual(val1[kKeyObject], val2[kKeyObject], mode, memos)) {
        return false;
      }
  ...
  function keyCheck(val1, val2, mode, memos, iterationType, keys2) {
    ...
    if (keys2.length === 0 && (iterationType === valueType.noIterator || iterationType === valueType.isArray && val2.length === 0 || val2.size === 0)) {
      return true;
    }
    if (memos === null) {
  ...
  function compareSmallSets(a, b, val, iteratorB, mode, memo) {
    ...
    return a.size !== 1 && innerDeepEqual(iteratorA.next().value, val, mode, memo) && (b.size === 1 || // Partial mode
  ...
  function objEquiv(a, b, mode, keys1, keys2, memos, iterationType) {
    ...
    if (keys2.length > 0) {
      ...
      for(; i < keys2.length; i++){
        ...
        if (!descriptor?.enumerable || !innerDeepEqual(descriptor.value !== undefined ? descriptor.value : a[key], b[key], mode, memos)) {
          return false;
        }
      }
    }
      ...
      for(let i = 0; i < a.length; i++){
        if (b[i] === undefined) {
          ...
          if (mode !== kLoose && (a[i] !== undefined || !hasOwn(a, i))) {
            return false;
          } else if (mode === kLoose && !innerDeepEqual(a[i], b[i], mode, memos)) {
            return false;
          }
        } else if (mode !== kLoose && a[i] === undefined || !innerDeepEqual(a[i], b[i], mode, memos)) {
          return false;
        }
      }
