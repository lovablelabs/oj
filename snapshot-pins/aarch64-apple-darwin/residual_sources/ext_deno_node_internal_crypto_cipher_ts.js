"use strict"; return ((function() {
  ...
  const { encode } = core;
  const { ArrayBufferIsView, Boolean, DataViewPrototypeGetBuffer, DataViewPrototypeGetByteLength, DataViewPrototypeGetByteOffset, Error, FunctionPrototypeCall, MathFloor, ObjectPrototypeIsPrototypeOf, ObjectSetPrototypeOf, SafeRegExp, SafeSet, SetPrototypeHas, StringPrototypeReplace, StringPrototypeStartsWith, StringPrototypeToLowerCase, SymbolSpecies, TypeError, TypeErrorPrototype, TypedArrayPrototypeAt, TypedArrayPrototypeGetBuffer, TypedArrayPrototypeGetByteLength, TypedArrayPrototypeGetByteOffset, Uint8Array } = primordials;
  const { op_node_aes_unwrap_key, op_node_aes_wrap_key, op_node_cipheriv_encrypt, op_node_cipheriv_final, op_node_cipheriv_set_aad, op_node_cipheriv_take, op_node_create_cipheriv, op_node_create_decipheriv, op_node_create_private_key, op_node_decipheriv_auth_tag, op_node_decipheriv_decrypt, op_node_decipheriv_final, op_node_decipheriv_set_aad, op_node_export_private_key_pem, op_node_export_secret_key, op_node_private_decrypt, op_node_private_encrypt, op_node_public_decrypt, op_node_public_encrypt, op_node_validate_oaep_hash } = core.ops;
  const { Buffer } = core.loadExtScript("ext:deno_node/internal/buffer.mjs");
  const lazyStream = core.createLazyLoader("node:stream");
  const { createPrivateKey, createPublicKey, getArrayBufferOrView } = core.loadExtScript("ext:deno_node/internal/crypto/keys.ts");
  const { isKeyObject } = core.loadExtScript("ext:deno_node/internal/crypto/_keys.ts");
  const { kHandle } = core.loadExtScript("ext:deno_node/internal/crypto/constants.ts");
  const { getDefaultEncoding } = core.loadExtScript("ext:deno_node/internal/crypto/util.ts");
  const { ERR_INVALID_ARG_TYPE, ERR_INVALID_ARG_VALUE, ERR_UNKNOWN_ENCODING, NodeError } = core.loadExtScript("ext:deno_node/internal/errors.ts");
  const { isAnyArrayBuffer, isArrayBufferView, isTypedArray } = core.loadExtScript("ext:deno_node/internal/util/types.ts");
  const { ERR_CRYPTO_INVALID_STATE, ERR_CRYPTO_UNKNOWN_CIPHER } = core.loadExtScript("ext:deno_node/internal/errors.ts");
  ...
  function getArrayBufferViewParts(view) {
    ...
    return {
      ...
      byteOffset: DataViewPrototypeGetByteOffset(view),
      byteLength: DataViewPrototypeGetByteLength(view)
    };
  }
  function getArrayBufferViewByteLength(view) {
    const { byteLength } = getArrayBufferViewParts(view);
    return byteLength;
  }
  function toFastBufferView(view) {
    const { buffer, byteOffset, byteLength } = getArrayBufferViewParts(view);
    return new FastBuffer(buffer, byteOffset, byteLength);
  }
  function opensslError(code, reason) {
    const err = new NodeError(code, reason);
    err.reason = reason;
    return err;
  }
  function isAesWrap(cipher) {
    return cipher === "aes128-wrap" || cipher === "aes192-wrap" || cipher === "aes256-wrap" || cipher === "id-aes128-wrap-pad" || cipher === "id-aes192-wrap-pad" || cipher === "id-aes256-wrap-pad";
  }
  function isStringOrBuffer(val) {
    return typeof val === "string" || isArrayBufferView(val) || isAnyArrayBuffer(val) || Buffer.isBuffer(val);
  }
  ...
  function validateCipherUpdateData(data) {
  ...
  function Cipheriv(cipher, key, iv, options) {
    ...
    if (this._isAesWrap) {
      ...
      try {
        this._context = op_node_create_cipheriv(cipher, toU8(key), toU8(iv), authTagLength);
      } catch (e) {
        ...
        if (ObjectPrototypeIsPrototypeOf(TypeErrorPrototype, e) && StringPrototypeStartsWith(e.message, "Unknown cipher")) {
    ...
    this._needsBlockCache = !this._isAesWrap && !(cipher == "aes-128-gcm" || cipher == "aes-256-gcm" || cipher == "aes-128-ctr" || cipher == "aes-192-ctr" || cipher == "aes-256-ctr" || cipher == "chacha20" || cipher == "chacha20-poly1305");
  ...
  Cipheriv.prototype.final = function(encoding = getDefaultEncoding()) {
    if (this._finalized) {
    ...
    const hasNoBufferedData = TypedArrayPrototypeGetByteLength(this._cache.cache) === 0;
    ...
    if (!this._autoPadding && TypedArrayPrototypeGetByteLength(this._cache.cache) != bs) {
      throw opensslError("ERR_OSSL_EVP_WRONG_FINAL_BLOCK_LENGTH", "wrong final block length");
    }
    const maybeTag = op_node_cipheriv_final(this._context, this._autoPadding, this._cache.cache, buf);
    if (maybeTag) {
  ...
  Cipheriv.prototype.setAAD = function(buffer, _options) {
    if (this._finalized) {
  ...
  Cipheriv.prototype.update = function(data, inputEncoding, outputEncoding = getDefaultEncoding()) {
    if (this._finalized) {
    ...
    if (this._isAesWrap) {
      const output = Buffer.from(op_node_aes_wrap_key(this._aesWrapAlgorithm, this._aesWrapKey, this._aesWrapIv, buf));
  ...
  function _lazyInitCipherDecoder(self, encoding) {
    if (encoding === "buffer") {
      return;
    }
 ...
 * Used by CBC, ECB modes of block ciphers */ class BlockModeCache {
    cache;
    blockSize;
    ...
    add(data) {
      ...
      this.cache.set(new Uint8Array(buffer, byteOffset, byteLength), cache.length);
    }
    /** Gets the chunk of the length of largest multiple of blockSize.
   * Used for preparing data for encryption/decryption */ get() {
    ...
    set lastChunkIsNonZero(value) {
      this.#lastChunkIsNonZero = value;
    }
  }
  function getBlockSize(cipher) {
  ...
  function Decipheriv(cipher, key, iv, options) {
    ...
    if (this._isAesWrap) {
      ...
      try {
        this._context = op_node_create_decipheriv(cipher, toU8(key), toU8(iv), authTagLength);
      } catch (e) {
        // The op reports an unrecognized algorithm as a TypeError that includes
        // the cipher name; surface Node's ERR_CRYPTO_UNKNOWN_CIPHER instead.
        if (ObjectPrototypeIsPrototypeOf(TypeErrorPrototype, e) && StringPrototypeStartsWith(e.message, "Unknown cipher")) {
    ...
    this._needsBlockCache = !this._isAesWrap && !(cipher == "aes-128-gcm" || cipher == "aes-256-gcm" || cipher == "aes-128-ctr" || cipher == "aes-192-ctr" || cipher == "aes-256-ctr" || cipher == "chacha20" || cipher == "chacha20-poly1305");
    this._isGcmMode = cipher == "aes-128-gcm" || cipher == "aes-192-gcm" || cipher == "aes-256-gcm";
  ...
  Decipheriv.prototype.final = function(encoding = getDefaultEncoding()) {
    if (this._finalized) {
      throw new ERR_CRYPTO_INVALID_STATE("final");
    }
    ...
    op_node_decipheriv_final(this._context, this._autoPadding, this._cache.cache, buf, this._authTag || NO_TAG);
    if (!this._needsBlockCache || TypedArrayPrototypeGetByteLength(this._cache.cache) === 0) {
      this._finalized = true;
      return encoding === "buffer" ? Buffer.from([]) : this._decoder.end();
    }
    ...
    if (this._autoPadding) {
      ...
      if (padLen === 0 || padLen > bs) {
        throw opensslError("ERR_OSSL_EVP_BAD_DECRYPT", "bad decrypt");
      }
  ...
  Decipheriv.prototype.setAAD = function(buffer, _options) {
    if (this._finalized) {
      throw new ERR_CRYPTO_INVALID_STATE("setAAD");
    }
  ...
  Decipheriv.prototype.setAuthTag = function(buffer, _encoding) {
    if (this._authTag) {
    ...
    if (this._isGcmMode && this._authTagLength === -1 && tagByteLength !== 16) {
      throw new TypeError(`Invalid authentication tag length: ${tagByteLength}`);
    }
    // deno-lint-ignore deno-internal/prefer-primordials -- `buffer` may be Buffer/TypedArray/DataView
  ...
  Decipheriv.prototype.update = function(data, inputEncoding, outputEncoding = getDefaultEncoding()) {
    if (this._finalized) {
      throw new ERR_CRYPTO_INVALID_STATE("update");
    }
    validateCipherUpdateData(data);
    ...
    if (this._isAesWrap) {
      const output = Buffer.from(op_node_aes_unwrap_key(this._aesWrapAlgorithm, this._aesWrapKey, this._aesWrapIv, buf));
      if (outputEncoding !== "buffer") {
        return this._decoder.write(output);
      }
      return output;
  ...
  function _lazyInitDecipherDecoder(self, encoding) {
    if (encoding === "buffer") {
      return;
    }
    const normalizedEncoding = normalizeEncoding(encoding);
  ...
  function checkUnsupportedKeyType(key) {
    const keyType = isKeyObject(key) ? key.asymmetricKeyType : key?.key?.asymmetricKeyType;
  ...
  function normalizeOaepHash(hash) {
    ...
    const normalized = StringPrototypeReplace(StringPrototypeToLowerCase(hash), WEBCRYPTO_SHA_HYPHEN_RE, "$1");
  ...
  function bufferEncodingFrom(keyOptions) {
    return keyOptions?.encoding;
  }
  function validateOaepLabel(label) {
    if (label === undefined) return undefined;
    if (!isArrayBufferView(label) && !isAnyArrayBuffer(label)) {
      throw new ERR_INVALID_ARG_TYPE("oaepLabel", [
        "Buffer",
        "TypedArray",
        "DataView"
      ], label);
  ...
  function privateEncrypt(privateKey, buffer) {
    ...
    buffer = getArrayBufferOrView(buffer, "buffer", bufferEncodingFrom(privateKey));
    return Buffer.from(op_node_private_encrypt(data, buffer, padding, oaepHash, oaepLabel));
  }
  function privateDecrypt(privateKey, buffer) {
    ...
    const oaepLabel = validateOaepLabel(privateKey.oaepLabel);
    buffer = getArrayBufferOrView(buffer, "buffer", bufferEncodingFrom(privateKey));
    return Buffer.from(op_node_private_decrypt(data, buffer, padding, oaepHash, oaepLabel));
  }
  function publicEncrypt(publicKey, buffer) {
    ...
    buffer = getArrayBufferOrView(buffer, "buffer", bufferEncodingFrom(publicKey));
    return Buffer.from(op_node_public_encrypt(data, buffer, padding, oaepHash, oaepLabel));
  }
  function prepareKey(key) {
    ...
    if (isStringOrBuffer(key)) {
      ...
      const data = key.export({
        type: "spki",
        format: "pem"
      });
      return {
        data: getArrayBufferOrView(data, "key")
      };
      ...
      if (format === "jwk") {
        ...
        const isPrivate = typeof data === "object" && data !== null && typeof data.d === "string";
        const keyObject = isPrivate ? createPrivateKey({
          key: data,
          format: "jwk"
        }) : createPublicKey({
          key: data,
      ...
      if (passphrase != null) {
        ...
        const handle = op_node_create_private_key(keyData, keyFormat, type ?? "", passphraseData);
        const pem = op_node_export_private_key_pem(handle, "pkcs8", null, null);
        return {
          data: getArrayBufferOrView(pem, "key")
        };
      }
      return {
  ...
  function publicDecrypt(publicKey, buffer) {
    checkUnsupportedKeyType(publicKey);
    const { data } = prepareKey(publicKey);
