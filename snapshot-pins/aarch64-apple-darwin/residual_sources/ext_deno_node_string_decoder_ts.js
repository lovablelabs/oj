"use strict"; return ((function() {
  ...
  const { normalizeEncoding: castEncoding } = core.loadExtScript("ext:deno_node/internal/util.mjs");
  const { ERR_INVALID_ARG_TYPE, ERR_INVALID_THIS, ERR_UNKNOWN_ENCODING, NodeError } = core.loadExtScript("ext:deno_node/internal/errors.ts");
  const { ArrayBufferIsView, ObjectDefineProperties, Symbol, MathMin, DataViewPrototypeGetBuffer, DataViewPrototypeGetByteLength, DataViewPrototypeGetByteOffset, NumberPrototypeToString, ObjectPrototypeIsPrototypeOf, String, TypedArrayPrototypeGetBuffer, TypedArrayPrototypeGetByteLength, TypedArrayPrototypeGetByteOffset, StringPrototypeToLowerCase, Uint8Array } = primordials;
  ...
  function isBufferType(buf) {
    return ObjectPrototypeIsPrototypeOf(Buffer.prototype, buf) && buf.BYTES_PER_ELEMENT;
  }
  function normalizeBuffer(buf) {
    ...
    if (isBufferType(buf)) {
      ...
      return Buffer.from(new Uint8Array(isTA ? TypedArrayPrototypeGetBuffer(buf) : DataViewPrototypeGetBuffer(buf), isTA ? TypedArrayPrototypeGetByteOffset(buf) : DataViewPrototypeGetByteOffset(buf), isTA ? TypedArrayPrototypeGetByteLength(buf) : DataViewPrototypeGetByteLength(buf)));
    }
  }
  ...
  function bufferToString(buf, encoding, start, end) {
    ...
    if (len > MAX_STRING_LENGTH) {
      throw new NodeError("ERR_STRING_TOO_LONG", `Cannot create a string longer than 0x${maxStringLengthHex} characters`);
    }
  ...
  function decode(buf) {
    ...
    let prepend = "";
    let rest = "";
    if (enc === ENCODING_UTF8 || enc === ENCODING_UTF16 || enc === ENCODING_BASE64 || enc === ENCODING_BASE64URL) {
      if (this[kMissingBytes] > 0) {
        if (enc === ENCODING_UTF8) {
          for(let i = 0; i < buf.length - bufIdx && i < this[kMissingBytes]; i++){
            if ((buf[i] & 0xC0) !== 0x80) {
        ...
        buf.copy(this.lastChar, this[kBufferedBytes], bufIdx, bufIdx + bytesToCopy);
        bufIdx += bytesToCopy;
        this[kBufferedBytes] += bytesToCopy;
        this[kMissingBytes] -= bytesToCopy;
        if (this[kMissingBytes] === 0) {
          prepend = bufferToString(this.lastChar, this.encoding, 0, this[kBufferedBytes]);
      ...
      if (buf.length - bufIdx === 0) {
        ...
        if (this[kBufferedBytes] > 0) {
          buf.copy(this.lastChar, 0, buf.length - this[kBufferedBytes]);
  ...
  function flush() {
    ...
    const ret = bufferToString(this.lastChar, this.encoding, 0, this[kBufferedBytes]);
    this[kBufferedBytes] = 0;
    this[kMissingBytes] = 0;
    return ret;
  }
