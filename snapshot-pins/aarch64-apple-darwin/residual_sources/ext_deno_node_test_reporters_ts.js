"use strict"; return ((function() {
  ...
  const { ArrayPrototypeJoin, ArrayPrototypePush, ObjectAssign, ReflectConstruct, SafeArrayIterator, SafeRegExp, String, StringPrototypeReplace, StringPrototypeSplit, SymbolAsyncIterator } = primordials;
  ...
  function getTransform() {
    if (_Transform === null) {
      _Transform = core.loadExtScript("ext:deno_node/internal/streams/transform.js").Transform;
    }
    return _Transform;
  }
  ...
  function reportTapDetails(nesting, data) {
    ...
    if (details) {
      ...
      for (const line of new SafeArrayIterator(StringPrototypeSplit(String(details), "\n"))){
  ...
  function reportTest(status, data) {
    ...
    const directive = data.skip !== undefined ? ` # SKIP${data.skip ? ` ${tapEscape(data.skip)}` : ""}` : data.todo !== undefined ? ` # TODO${data.todo ? ` ${tapEscape(data.todo)}` : ""}` : "";
    const testNumber = data.testNumber ? ` ${data.testNumber}` : "";
    return `${tapIndent(nesting)}${status}${testNumber} ${tapEscape(data.name ?? "<anonymous>")}${directive}\n`;
  }
  ...
  async function* junit(source) {
    ...
    while(true){
      ...
      const duration = data.duration_ms === undefined ? "" : ` time="${xmlEscape(data.duration_ms / 1000)}"`;
      if (type === "test:pass") {
        ArrayPrototypePush(testcases, `  <testcase name="${name}"${duration}/>\n`);
      } else {
        const message = xmlEscape(formatError(data.details));
        ArrayPrototypePush(testcases, `  <testcase name="${name}"${duration}><failure>${message}</failure></testcase>\n`);
      }
    }
