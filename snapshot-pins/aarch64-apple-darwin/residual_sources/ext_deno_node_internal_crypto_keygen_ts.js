"use strict"; return ((function() {
  ...
  const { PrivateKeyObject, PublicKeyObject, SecretKeyObject } = core.loadExtScript("ext:deno_node/internal/crypto/keys.ts");
  const { kAesKeyLengths } = core.loadExtScript("ext:deno_node/internal/crypto/util.ts");
  const { ERR_CRYPTO_INCOMPATIBLE_KEY_OPTIONS, ERR_CRYPTO_INVALID_DIGEST, ERR_CRYPTO_UNKNOWN_CIPHER, ERR_CRYPTO_UNKNOWN_DH_GROUP, ERR_INCOMPATIBLE_OPTION_PAIR, ERR_INVALID_ARG_VALUE, ERR_MISSING_OPTION } = core.loadExtScript("ext:deno_node/internal/errors.ts");
  const { getCiphers } = core.loadExtScript("ext:deno_node/internal/crypto/util.ts");
  const { getHashes } = core.loadExtScript("ext:deno_node/internal/crypto/hash.ts");
  const { validateBuffer, validateFunction, validateInt32, validateInteger, validateObject, validateOneOf, validateString, validateUint32 } = core.loadExtScript("ext:deno_node/internal/validators.mjs");
  const lazyProcess = core.createLazyLoader("node:process");
  const { promisify } = core.loadExtScript("ext:deno_node/util.ts");
  const { op_node_generate_dh_group_key, op_node_generate_dh_group_key_async, op_node_generate_dh_key, op_node_generate_dh_key_async, op_node_generate_dsa_key, op_node_generate_dsa_key_async, op_node_generate_ec_key, op_node_generate_ec_key_async, op_node_generate_ed25519_key, op_node_generate_ed25519_key_async, op_node_generate_ed448_key, op_node_generate_ed448_key_async, op_node_generate_rsa_key, op_node_generate_rsa_key_async, op_node_generate_rsa_pss_key, op_node_generate_rsa_pss_key_async, op_node_generate_secret_key, op_node_generate_secret_key_async, op_node_generate_x25519_key, op_node_generate_x25519_key_async, op_node_generate_x448_key, op_node_generate_x448_key_async, op_node_get_private_key_from_pair, op_node_get_public_key_from_pair } = core.ops;
  ...
  function generateKeyPair(type, options, callback) {
    ...
    validateFunction(callback, "callback");
    _generateKeyPair(type, options).then((res)=>callback(null, res.publicKey, res.privateKey), (err)=>callback(err, null, null));
  }
  function _generateKeyPair(type, options) {
  ...
  function isStringOrBuffer(val) {
    return typeof val === "string" || ArrayBuffer.isView(val) || val instanceof ArrayBuffer || val instanceof SharedArrayBuffer;
  }
  ...
  function parsePublicKeyEncoding(enc, keyType, objName) {
    ...
    const { format, type } = parseKeyFormatAndType(enc, keyType, keyType ? true : undefined, objName);
    return {
      format,
      type
    };
  ...
  function validateKeyEncoding(keyType, options) {
      ...
      case "rsa-pss":
        {
          ...
          if (type === "rsa") {
            if (mode === kSync) {
              return op_node_generate_rsa_key(modulusLength, publicExponent);
            } else {
              return op_node_generate_rsa_key_async(modulusLength, publicExponent);
            }
          }
          ...
          if (hash !== undefined) {
            process.emitWarning('"options.hash" is deprecated, ' + 'use "options.hashAlgorithm" instead.', "DeprecationWarning", "DEP0154");
          ...
          if (mgf1Hash !== undefined) {
            process.emitWarning('"options.mgf1Hash" is deprecated, ' + 'use "options.mgf1HashAlgorithm" instead.', "DeprecationWarning", "DEP0154");
          ...
          if (mode === kSync) {
            return op_node_generate_rsa_pss_key(modulusLength, publicExponent, hashAlgorithm ?? hash, mgf1HashAlgorithm ?? mgf1Hash, saltLength);
          } else {
            return op_node_generate_rsa_pss_key_async(modulusLength, publicExponent, hashAlgorithm ?? hash, mgf1HashAlgorithm ?? mgf1Hash, saltLength);
          }
        }
      case "dsa":
        {
          ...
          if (mode === kSync) {
            ...
            return op_node_generate_dsa_key_async(modulusLength, divisorLength);
          }
        }
      case "ec":
        {
          ...
          // pass.
            ...
            process.emitWarning('paramEncoding: "explicit" is not supported; ' + "the generated key will use named-curve encoding instead.", "Warning");
          } else {
      ...
      case "dh":
        {
          ...
          if (group != null) {
            ...
            if (group !== "modp5" && group !== "modp14" && group !== "modp15" && group !== "modp16" && group !== "modp17" && group !== "modp18") {
          ...
          if (mode === kSync) {
            ...
            return op_node_generate_dh_key_async(prime, primeLength ?? 0, g);
          }
        }
      default:
    }
