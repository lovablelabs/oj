"use strict"; return ((function() {
  ...
  const { CHAR_DOT, CHAR_FORWARD_SLASH } = core.loadExtScript("ext:deno_node/path/_constants.ts");
  const { ERR_INVALID_ARG_TYPE } = core.loadExtScript("ext:deno_node/internal/errors.ts");
  const { _format, assertPath, isPosixPathSeparator, normalizeString } = core.loadExtScript("ext:deno_node/path/_util.ts");
  ...
  const { StringPrototypeReplace, StringPrototypeCharCodeAt, StringPrototypeIndexOf, StringPrototypeSlice, SafeRegExp } = primordials;
  const sep = "/";
  const delimiter = ":";
  const posixCwd = (()=>{
    if (isWindows) {
 ...
 */ function resolve(...pathSegments) {
    if (pathSegments.length === 0 || pathSegments.length === 1 && (pathSegments[0] === "" || pathSegments[0] === ".")) {
      const cwd = posixCwd();
