"use strict"; return ((function() {
  ...
  const { ArrayPrototypePush, Date, ObjectAssign, ObjectFreeze, ObjectPrototypeIsPrototypeOf, StringPrototypeIncludes } = primordials;
  const { op_node_x509_ca, op_node_x509_check_email, op_node_x509_check_host, op_node_x509_check_ip, op_node_x509_check_issued, op_node_x509_check_private_key, op_node_x509_fingerprint, op_node_x509_fingerprint256, op_node_x509_fingerprint512, op_node_x509_get_info_access, op_node_x509_get_issuer, op_node_x509_get_raw, op_node_x509_get_serial_number, op_node_x509_get_signature_algorithm_name, op_node_x509_get_signature_algorithm_oid, op_node_x509_get_subject, op_node_x509_get_subject_alt_name, op_node_x509_get_valid_from, op_node_x509_get_valid_to, op_node_x509_key_usage, op_node_x509_parse, op_node_x509_public_key, op_node_x509_to_legacy_object, op_node_x509_to_string, op_node_x509_verify } = core.ops;
  const { KeyObject, PublicKeyObject } = core.loadExtScript("ext:deno_node/internal/crypto/keys.ts");
  ...
  const { validateBoolean, validateObject, validateString } = core.loadExtScript("ext:deno_node/internal/validators.mjs");
  ...
  const kEmptyObject = ObjectFreeze({
    __proto__: null
  });
  function getFlags(options = kEmptyObject) {
    validateObject(options, "options");
    const { subject = "default", wildcards = true, partialWildcards = true, multiLabelWildcards = false, singleLabelSubdomains = false } = {
      ...options
    };
    let flags = 0;
  ...
  class X509Certificate {
    ...
    checkPrivateKey(privateKey) {
      if (!ObjectPrototypeIsPrototypeOf(KeyObject.prototype, privateKey)) {
        throw new ERR_INVALID_ARG_TYPE("privateKey", "KeyObject", privateKey);
      }
      if (privateKey.type !== "private") {
        throw new ERR_INVALID_ARG_VALUE("privateKey", privateKey);
      }
      return op_node_x509_check_private_key(this.#handle, privateKey[kHandle]);
    }
    get fingerprint() {
    ...
    toLegacyObject() {
      ...
      if (obj.subject) {
        obj.subject = ObjectAssign({
          __proto__: null
        }, obj.subject);
      }
      if (obj.issuer) {
        obj.issuer = ObjectAssign({
          __proto__: null
        }, obj.issuer);
      }
      if (obj.infoAccess) {
        obj.infoAccess = ObjectAssign({
          __proto__: null
        }, obj.infoAccess);
      }
      return obj;
    ...
    verify(publicKey) {
      if (!ObjectPrototypeIsPrototypeOf(KeyObject.prototype, publicKey)) {
        throw new ERR_INVALID_ARG_TYPE("publicKey", "KeyObject", publicKey);
      }
      if (publicKey.type !== "public") {
        throw new ERR_INVALID_ARG_VALUE("publicKey", publicKey);
      }
      return op_node_x509_verify(this.#handle, publicKey[kHandle]);
    }
  }
  ...
  function deserializeX509Certificate(data) {
