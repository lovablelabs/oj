"use strict"; return ((function() {
  ...
  const { CHAR_BACKWARD_SLASH, CHAR_COLON, CHAR_DOT, CHAR_QUESTION_MARK } = core.loadExtScript("ext:deno_node/path/_constants.ts");
  const { ERR_INVALID_ARG_TYPE } = core.loadExtScript("ext:deno_node/internal/errors.ts");
  const { _format, assertPath, isPathSeparator, isPosixPathSeparator, isWindowsDeviceRoot, normalizeString } = core.loadExtScript("ext:deno_node/path/_util.ts");
  const { default: assert } = core.loadExtScript("ext:deno_node/assert.ts");
  const lazyLoadGlob = core.createLazyLoader("ext:deno_node/_fs/_fs_glob.ts");
  const { ArrayPrototypeIncludes, ArrayPrototypeJoin, ArrayPrototypePop, ArrayPrototypeSlice, StringPrototypeCharCodeAt, StringPrototypeIncludes, StringPrototypeIndexOf, StringPrototypeRepeat, StringPrototypeSlice, StringPrototypeSplit, StringPrototypeToLowerCase, StringPrototypeToUpperCase, TypeError } = primordials;
  const sep = "\\";
  const delimiter = ";";
 ...
 */ function resolve(...pathSegments) {
    ...
    for(let i = pathSegments.length - 1; i >= -1; i--){
      ...
      if (i >= 0) {
        ...
        path = Deno.cwd();
        if (pathSegments.length === 0 || pathSegments.length === 1 && (pathSegments[0] === "" || pathSegments[0] === ".") && isPathSeparator(StringPrototypeCharCodeAt(path, 0))) {
          return path;
        }
      } else {
 ...
 */ function join(...paths) {
    ...
    let hasReservedComponent = false;
    {
      ...
      for(let i = 0; i <= joined.length; i++){
        if (i === joined.length || isPathSeparator(StringPrototypeCharCodeAt(joined, i))) {
          if (i > segStart) {
