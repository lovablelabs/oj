"use strict"; return ((function() {
  ...
  const { AssertionError } = core.loadExtScript("ext:deno_node/internal/assert/assertion_error.js");
  const { innerOk } = core.loadExtScript("ext:deno_node/internal/assert/utils.ts");
  const { inspect } = core.loadExtScript("ext:deno_node/util.ts");
  const { ERR_AMBIGUOUS_ARGUMENT, ERR_CONSTRUCT_CALL_REQUIRED, ERR_INVALID_ARG_TYPE, ERR_INVALID_ARG_VALUE, ERR_INVALID_RETURN_VALUE, ERR_MISSING_ARGS } = core.loadExtScript("ext:deno_node/internal/errors.ts");
  const { isDeepEqual, isDeepStrictEqual, isPartialStrictEqual } = core.loadExtScript("ext:deno_node/internal/util/comparisons.ts");
  const { CallTracker } = core.loadExtScript("ext:deno_node/internal/assert/calltracker.js");
  const { deprecate } = core.loadExtScript("ext:deno_node/util.ts");
  const { isPromise, isRegExp } = core.loadExtScript("ext:deno_node/internal_binding/types.ts");
  const { validateFunction, validateOneOf } = core.loadExtScript("ext:deno_node/internal/validators.mjs");
  const { isError } = core.loadExtScript("ext:deno_node/internal/util.mjs");
  const { ArrayPrototypeForEach, ArrayPrototypeIndexOf, ArrayPrototypeJoin, ArrayPrototypePush, ArrayPrototypeSlice, Error, ErrorPrototype, NumberIsNaN, ObjectAssign, ObjectDefineProperty, ObjectIs, ObjectKeys, ObjectPrototypeIsPrototypeOf, ReflectApply, ReflectHas, RegExpPrototypeExec, SafeArrayIterator, StringPrototypeIndexOf, StringPrototypeSlice, StringPrototypeSplit, String, Symbol } = primordials;
  ...
  function compareExceptionKey(actual, expected, key, message, keys, fn) {
    if (!ReflectHas(actual, key) || !isDeepStrictEqual(actual[key], expected[key])) {
      if (!message) {
  ...
  function expectedException(actual, expected, message, fn) {
    ...
    // Check for matching Error classes.
    } else if (expected.prototype !== undefined && ObjectPrototypeIsPrototypeOf(expected.prototype, actual)) {
      return;
      ...
      const res = ReflectApply(expected, {}, [
        actual
      ]);
      if (res !== true) {
        if (!message) {
  ...
  function hasMatchingError(actual, expected) {
    ...
    // Guard instanceof against arrow functions as they don't have a prototype.
    if (expected.prototype !== undefined && ObjectPrototypeIsPrototypeOf(expected.prototype, actual)) {
      return true;
    }
    if (ObjectPrototypeIsPrototypeOf(Error, expected)) {
      return false;
    }
    return ReflectApply(expected, {}, [
      actual
    ]) === true;
  }
  ...
  function internalMatch(string, regexp, message, fn) {
    ...
    if (typeof string !== "string" || RegExpPrototypeExec(regexp, string) !== null !== matchFn) {
      if (ObjectPrototypeIsPrototypeOf(ErrorPrototype, message)) {
        throw message;
      }
      const generatedMessage = !message;
      // 'The input was expected to not match the regular expression ' +
      message ||= typeof string !== "string" ? 'The "string" argument must be of type string. Received type ' + `${typeof string} (${inspect(string)})` : (matchFn ? "The input did not match the regular expression " : "The input was expected to not match the regular expression ") + `${inspect(regexp)}. Input:\n\n${inspect(string)}\n`;
      const err = new AssertionError({
        actual: string,
        expected: regexp,
        message,
  ...
  async function rejects(asyncFn, ...args) {
    expectsError(rejects, await waitForActual(asyncFn), ...new SafeArrayIterator(args));
  }
  async function doesNotReject(asyncFn, ...args) {
    expectsNoError(doesNotReject, await waitForActual(asyncFn), ...new SafeArrayIterator(args));
  }
  /**
 ...
 */ function ifError(err) {
  ...
  const CallTracker_ = deprecate(CallTracker, "assert.CallTracker is deprecated.", "DEP0173");
