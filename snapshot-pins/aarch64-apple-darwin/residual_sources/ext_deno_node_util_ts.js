"use strict"; return ((function() {
  ...
  const { validateAbortSignal, validateBoolean, validateNumber, validateObject, validateString, validateStringArray } = core.loadExtScript("ext:deno_node/internal/validators.mjs");
  const { myersDiff } = core.loadExtScript("ext:deno_node/internal/assert/myers_diff.js");
  const { parseArgs } = core.loadExtScript("ext:deno_node/internal/util/parse_args/parse_args.js");
  const { MIMEParams, MIMEType } = core.loadExtScript("ext:deno_node/internal/mime.ts");
  const abortSignal = core.loadExtScript("ext:deno_web/03_abort_signal.js");
  const { ERR_INVALID_ARG_TYPE, ERR_WORKER_UNSUPPORTED_OPERATION } = core.loadExtScript("ext:deno_node/internal/errors.ts");
  const { default: binding } = core.loadExtScript("ext:deno_node/internal_binding/util.ts");
  const { validateOneOf } = core.loadExtScript("ext:deno_node/internal/validators.mjs");
  ...
  /** @deprecated - use `Array.isArray()` instead. */ const isArray = ArrayIsArray;
  /** @deprecated Use Object.assign() instead. */ function _extend(target, source) {
 ...
 */ function inherits(ctor, superCtor) {
  ...
  const { _TextDecoder, _TextEncoder, getSystemErrorMap, getSystemErrorMessage, getSystemErrorName } = core.loadExtScript("ext:deno_node/_utils.ts");
  /** The global TextDecoder */ const TextDecoder = _TextDecoder;
  /** The global TextEncoder */ const TextEncoder = _TextEncoder;
