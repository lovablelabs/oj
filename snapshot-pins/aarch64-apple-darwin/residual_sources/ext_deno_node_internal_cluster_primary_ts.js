"use strict"; return ((function() {
  ...
  const { RoundRobinHandle } = core.loadExtScript("ext:deno_node/internal/cluster/round_robin_handle.ts");
  const { SharedHandle } = core.loadExtScript("ext:deno_node/internal/cluster/shared_handle.ts");
  const { ArrayPrototypeSlice, ObjectKeys, ObjectValues, SafeMap } = primordials;
  const SCHED_NONE = 1;
  const SCHED_RR = 2;
  ...
  function init(cluster) {
    ...
    function createWorkerProcess(id, env) {
      ...
      const inspectSource = execArgv.find((arg)=>debugArgRegex.test(arg)) ?? (debugArgRegex.test(nodeOptions) ? nodeOptions : undefined);
      if (inspectSource !== undefined) {
        let inspectPort;
        if ("inspectPort" in cluster.settings) {
          inspectPort = typeof cluster.settings.inspectPort === "function" ? cluster.settings.inspectPort() : cluster.settings.inspectPort;
        } else {
        ...
        if (inspectPort) {
          const flag = /--inspect-brk\b/.test(inspectSource) ? "--inspect-brk" : /--inspect-wait\b/.test(inspectSource) ? "--inspect-wait" : "--inspect";
          execArgv.push(`${flag}=127.0.0.1:${inspectPort}`);
        }
      }
      return lazyChildProcess().fork(cluster.settings.exec, cluster.settings.args, {
    ...
    function queryServer(worker, message) {
      ...
      const key = `${message.address}:${message.port}:${message.addressType}:` + `${message.fd}` + (message.port === 0 ? `:${message.index}` : "");
      ...
      if (handle === undefined) {
        ...
        if (message.port < 0 && typeof address === "string" && !isWindows) {
