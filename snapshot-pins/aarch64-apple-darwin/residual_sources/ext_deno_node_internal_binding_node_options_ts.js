"use strict"; return ((function() {
  ...
  const { SafeMap, ArrayPrototypeForEach, ArrayPrototypePush, ArrayPrototypeConcat, ArrayPrototypeSlice, StringPrototypeSlice, StringPrototypeStartsWith } = primordials;
  // This module ports:
  ...
  function splitNodeOptions(input) {
    const args = [];
    let current = "";
    let inDouble = false;
    let inSingle = false;
  ...
  let optionsMap;
  let execArgvOptionsMap;
  let execArgvSnapshot;
  function setOptionSourceExecArgv(execArgv) {
  ...
  function parseOption(options, arg) {
    if (StringPrototypeStartsWith(arg, "--title=")) {
      options.set("--title", {
        value: StringPrototypeSlice(arg, 8)
      });
      return;
    }
    ...
    switch(arg){
      ...
      case "--no-tls-min-v1.0":
        options.set("--tls-min-v1.0", {
          value: false
        });
        break;
      case "--no-tls-min-v1.1":
        options.set("--tls-min-v1.1", {
          value: false
        });
        break;
      case "--no-tls-min-v1.2":
        options.set("--tls-min-v1.2", {
          value: false
        });
        break;
      case "--no-tls-min-v1.3":
        options.set("--tls-min-v1.3", {
          value: false
        });
        break;
      case "--no-tls-max-v1.2":
        options.set("--tls-max-v1.2", {
          value: false
        });
        break;
      case "--no-tls-max-v1.3":
        options.set("--tls-max-v1.3", {
          value: false
        });
        break;
      case "--no-use-bundled-ca":
        options.set("--use-bundled-ca", {
          value: false
        });
        break;
      case "--no-use-openssl-ca":
        options.set("--use-openssl-ca", {
          value: false
        });
        break;
      case "--no-use-system-ca":
        options.set("--use-system-ca", {
          value: false
        });
        break;
      default:
        if (StringPrototypeStartsWith(arg, "--dns-result-order=")) {
          const value = StringPrototypeSlice(arg, "--dns-result-order=".length);
          options.set("--dns-result-order", {
            value
          });
        }
        break;
