"use strict"; return ((function() {
  ...
  const { ArrayIsArray, ArrayPrototypeForEach, ArrayPrototypeIncludes, ArrayPrototypeIndexOf, ArrayPrototypeJoin, ArrayPrototypeLastIndexOf, ArrayPrototypePush, ArrayPrototypeSlice, ArrayPrototypeSplice, DatePrototypeGetTime, DatePrototypeToString, Error, ErrorPrototype, JSONStringify, MapPrototypeClear, MapPrototypeDelete, MapPrototypeGet, MapPrototypeHas, MapPrototypeSet, NumberIsFinite, NumberIsInteger, ObjectDefineProperty, ObjectKeys, ObjectPrototypeHasOwnProperty, ObjectGetOwnPropertyDescriptor, ObjectGetPrototypeOf, ObjectPrototypeIsPrototypeOf, Promise, PromisePrototypeThen, PromiseResolve, PromiseWithResolvers, Proxy, ReflectApply, ReflectConstruct, ReflectGet, RegExpPrototypeExec, RegExpPrototypeTest, SafeArrayIterator, SafeMap, SafeMapIterator, SafeRegExp, String, StringPrototypeEndsWith, StringPrototypeIncludes, StringPrototypeIndexOf, StringPrototypeMatch, StringPrototypeSlice, StringPrototypeSplit, StringPrototypeToLowerCase, StringPrototypeStartsWith, Symbol, SymbolDispose, SymbolFor, SymbolToPrimitive, TypeError, queueMicrotask } = primordials;
  ...
  function sanitizeThrowValue(err) {
    ...
    try {
      ...
      return new Error("test threw a non-Error object with a throwing custom inspect");
    }
  }
  ...
  const { validateBoolean, validateFunction, validateInteger, validateNumber, validateObject, validateString, validateStringArray } = core.loadExtScript("ext:deno_node/internal/validators.mjs");
  const nodeErrors = core.loadExtScript("ext:deno_node/internal/errors.ts");
  const { ERR_INVALID_ARG_TYPE, ERR_INVALID_ARG_VALUE } = nodeErrors.codes;
  ...
  const { emitExperimentalWarning } = core.loadExtScript("ext:deno_node/internal/util.mjs");
  const { default: assert } = core.loadExtScript("ext:deno_node/assert.ts");
  const { tapEscape, tapIndent } = core.loadExtScript("ext:deno_node/internal/test/reporters.ts");
  const methodsToCopy = [
    "deepEqual",
    "deepStrictEqual",
    "doesNotMatch",
  ...
  function isFileSnapshotUpdateMode() {
    ...
    if (nodeOptions && RegExpPrototypeTest(new SafeRegExp(/(?:^|\s)--test-update-snapshots(?:\s|=|$)/), nodeOptions)) {
      _fileSnapshotUpdateMode = true;
      return true;
    }
  ...
  function fileSnapshot(actual, path, options) {
    ...
    if (options.serializers === undefined) {
      serializers = [
        defaultFileSnapshotSerializer
      ];
    } else {
      if (!ArrayIsArray(options.serializers)) {
        throw new ERR_INVALID_ARG_TYPE("options.serializers", "Array", options.serializers);
      }
      serializers = options.serializers;
      for(let i = 0; i < serializers.length; i++){
        if (typeof serializers[i] !== "function") {
          throw new ERR_INVALID_ARG_TYPE(`options.serializers[${i}]`, "function", serializers[i]);
        }
      }
    }
    let value = actual;
    try {
      ...
      const e = new ERR_INVALID_STATE("The provided serializers did not generate a string.");
      e.cause = err;
      e.input = actual;
      throw e;
    }
    ...
    if (isFileSnapshotUpdateMode()) {
      try {
        ...
        const e = new ERR_INVALID_STATE(`Cannot write snapshot file '${path}'.`);
        e.cause = err;
        e.filename = path;
        throw e;
      }
    ...
    try {
      ...
      const message = isMissing ? `Cannot read snapshot file '${path}'. Missing snapshots can be ` + "generated by rerunning the command with the --test-update-snapshots " + "flag." : `Cannot read snapshot file '${path}'.`;
  ...
  function getReadable() {
    if (_Readable === null) {
      _Readable = core.loadExtScript("ext:deno_node/internal/streams/readable.js").Readable;
    }
    return _Readable;
  }
  let _fsWatch = null;
  ...
  function run(options) {
    ...
    if (testTagFilters !== undefined) {
      ...
      if (!ArrayIsArray(testTagFilters)) {
        throw new ERR_INVALID_ARG_TYPE("options.testTagFilters", [
          "string",
          "string[]"
      ...
      for(let i = 0; i < testTagFilters.length; i++){
        if (typeof testTagFilters[i] !== "string") {
          throw new ERR_INVALID_ARG_TYPE(`options.testTagFilters[${i}]`, "string", testTagFilters[i]);
        }
      }
  ...
  function detectNodeTestReporter() {
    ...
    const match = StringPrototypeMatch(nodeOptions, new SafeRegExp(/--test-reporter(?:=|\s+)(\S+)/));
  ...
  function parsePatternFlag(flag) {
    ...
    while((m = RegExpPrototypeExec(re, nodeOptions)) !== null){
      const value = m[1];
      let pattern;
      const litMatch = StringPrototypeMatch(value, new SafeRegExp(/^\/(.*)\/([a-z]*)$/));
      if (litMatch) {
        try {
  ...
  function isTestOnlyFlagSet() {
    ...
    testOnlyFlagCache = RegExpPrototypeTest(new SafeRegExp(/(^|\s)--test-only(\s|=|$)/), nodeOptions);
    return testOnlyFlagCache;
  }
  const TEST_ONLY_WARNING = "# 'only' and 'runOnly' require the --test-only command-line option.";
  ...
  class TapContext {
    ...
    test(name, options, fn) {
      ...
      const p = PromisePrototypeThen(this.#subtestTail, ()=>runTapEntry(entry, childDepth, n, parentState));
    ...
    try {
      ...
      if (rootBeforeHooks.length > 0) {
        ...
        for (const hook of new SafeArrayIterator(rootBeforeHooks)){
          try {
            const r = ReflectApply(hook, null, [
              rootCtx
            ]);
  ...
  async function runTapEntry(entry, depth, n, parentState) {
    ...
    if (entry.options.skip) {
      ...
      try {
        await runWithTestGuards(async ()=>{
          const ret = ReflectApply(entry.fn, ctx, [
            ctx
          ]);
    ...
    if (entry.options.only && !isTestOnlyFlagSet() && parentState && !parentState.onlyWarningEmitted) {
  ...
  function getExpectFailureMatch(expectFailure) {
    if (expectFailure !== null && typeof expectFailure === "object" && ObjectPrototypeHasOwnProperty(expectFailure, "match")) {
      return expectFailure.match;
    }
    if (expectFailure === true || typeof expectFailure === "string" || expectFailure === undefined) {
      return undefined;
    }
  ...
  async function runWithTestGuards(invoke, opts) {
    ...
    try {
      ...
      PromisePrototypeThen(body, (value)=>result.resolve(value), (err)=>result.reject(err));
  ...
  async function runNodeTestFunction(fn, nodeTestContext) {
    ...
    return await ReflectApply(fn, nodeTestContext, [
      nodeTestContext
    ]);
  }
  async function runPossiblyExpectingFailure(fn, nodeTestContext, options) {
    ...
    if (!options.expectFailure || options.skip || options.todo) {
      try {
        const result = await runWithTestGuards(()=>runNodeTestFunction(fn, nodeTestContext), guards);
    ...
    let failed = false;
    try {
      await runWithTestGuards(()=>runNodeTestFunction(fn, nodeTestContext), guards);
      nodeTestContext._checkPlan();
    } catch (err) {
      failed = true;
  ...
  class TestPlan {
    ...
    check() {
      if (this.#actual !== this.#expected) {
        throw new Error(`plan expected ${this.#expected} assertion(s) but received ${this.#actual}`);
      }
    }
  }
  class NodeTestContext {
    ...
    test(name, options, fn, overrides) {
      ...
      const stepPromise = PromisePrototypeThen(this.#denoContext.step({
        name: prepared.name,
        fn: async (denoTestContext)=>{
          const newNodeTextContext = new NodeTestContext(denoTestContext, parentContext, prepared.name, prepared.options.tags);
          let bodyOk = false;
          try {
            // The parent's `before()` hooks run once, before its first subtest.
            await parentContext._runBeforeHooksOnce();
            for (const hook of new SafeArrayIterator(parentContext.#beforeEachHooks)){
              // `t.beforeEach()` hooks run in the parent test's context, so
              // `getTestContext()` inside them observes the parent (Node).
              await runInTestContext(parentContext, ()=>hook(newNodeTextContext));
            }
            await runPossiblyExpectingFailure(prepared.fn, newNodeTextContext, prepared.options);
            bodyOk = true;
            ...
            for (const hook of new SafeArrayIterator(parentContext.#afterEachHooks)){
              // `t.afterEach()` hooks likewise run in the parent's context.
              await runInTestContext(parentContext, ()=>hook(newNodeTextContext));
            }
          }
        },
        ignore: !!prepared.options.todo || !!prepared.options.skip || this.#runOnly && !prepared.options.only,
        sanitizeExit: false,
        sanitizeOps: false,
        sanitizeResources: false
      }), ()=>undefined, // A failed step settles `false` rather than rejecting, but guard against
    ...
    async _drainSubtests() {
      ...
      const promises = ArrayPrototypeSplice(this.#subtestPromises, 0, this.#subtestPromises.length);
  ...
  class TestSuite {
    ...
    addTest(name, options, fn, overrides) {
      ...
      ArrayPrototypePush(this.entries, {
        name: prepared.name,
        fn: async (denoTestContext)=>{
          const newNodeTextContext = new NodeTestContext(denoTestContext, suiteNodeContext, prepared.name, prepared.options.tags);
          ...
          try {
            for (const hook of new SafeArrayIterator(rootBeforeEachHooks)){
              await runInTestContext(newNodeTextContext, ()=>hook(newNodeTextContext));
            }
            for(let i = suiteChain.length - 1; i >= 0; i--){
              ...
              for (const hook of new SafeArrayIterator(suiteChain[i].beforeEachHooks)){
                await runInTestContext(suiteCtx, ()=>hook(newNodeTextContext));
              }
            }
            return await runPossiblyExpectingFailure(prepared.fn, newNodeTextContext, prepared.options);
          } catch (err) {
            if (newNodeTextContext[skippedSymbol]) {
              return undefined;
            } else {
            ...
            for(let i = 0; i < suiteChain.length; i++){
              const suiteCtx = suiteChain[i].nodeTestContext;
              for (const hook of new SafeArrayIterator(suiteChain[i].afterEachHooks)){
                try {
                  await runInTestContext(suiteCtx, ()=>hook(newNodeTextContext));
                } catch  {}
              }
    ...
    addSuite(name, options, fn, overrides) {
      ...
      ArrayPrototypePush(this.entries, {
        name: prepared.name,
        fn: wrapSuiteFn(prepared.fn, resolve, prepared.name, parentSuiteContext, parentSuite, prepared.options.tags),
        ignore: !!prepared.options.todo || !!prepared.options.skip
      });
      return promise;
    }
  ...
  function prepareTags(tags, argName) {
    ...
    for(let i = 0; i < tags.length; i++){
      ...
      if (tag === "") {
        throw new ERR_INVALID_ARG_VALUE(`${argName}[${i}]`, tag, "must not be an empty string");
      }
  ...
  function wrapTestFn(fn, resolve, name, options) {
    return async function(t) {
      const nodeTestContext = new NodeTestContext(t, undefined, name, options.tags);
  ...
  function wrapSuiteFn(fn, resolve, name, parentNodeContext, parentSuite, tags) {
    return async function(t) {
      ...
      const suiteNodeContext = new NodeTestContext(t, parentNodeContext, name, tags);
      const prevSuite = currentSuite;
      const suite = currentSuite = new TestSuite(t, suiteNodeContext, parentSuite);
      try {
  ...
  function prepareDenoTestForSuite(name, options, fn, overrides) {
    ...
    const denoTestOptions = {
      name: prepared.name,
      fn: wrapSuiteFn(prepared.fn, noop, prepared.name, undefined, undefined, prepared.options.tags),
      only: prepared.options.only,
      ignore: !!prepared.options.todo || !!prepared.options.skip,
      sanitizeOnly: false,
      sanitizeExit: false,
  ...
  class MockPropertyContext {
    ...
    mockImplementation(value) {
      if (!this.#descriptor.writable) {
        throw new ERR_INVALID_ARG_VALUE("propertyName", this.#propertyName, "cannot be set");
      }
  ...
  function createMockFunction(original, implementation, ctx) {
    const mockFn = function(...args) {
      ...
      const impl = ctx._shouldMock() ? ctx._nextImpl() ?? implementation ?? original : original;
      let result;
      let error;
      ...
      try {
        ...
        error = e;
        ctx._recordCall(isCtor ? thisArg : thisArg, args, undefined, error, ctorTarget);
        throw e;
      }
      ctx._recordCall(isCtor ? result : thisArg, args, result, undefined, ctorTarget);
      return result;
    };
  ...
  function mockMethodImpl(object, methodName, implementation, options) {
    ...
    if (!descriptor) {
      throw new TypeError(`Cannot mock property '${String(methodName)}' because it does not exist`);
    }
    ...
    if (typeof original !== "function") {
      throw new TypeError(`Cannot mock property '${String(methodName)}' because it is not a function`);
    }
    const restore = ()=>{
  ...
  class MockTimers {
    ...
    enable(options = {
      ...
      if (this._enabled) {
        throw new ERR_INVALID_STATE("MockTimers is already enabled. Reset it first to enable it again");
      }
      validateObject(options, "options");
      ...
      if (apis === undefined) {
        ...
        for(let i = 0; i < apis.length; i++){
          if (!ArrayPrototypeIncludes(SUPPORTED_APIS, apis[i])) {
            throw new ERR_INVALID_ARG_VALUE(`options.apis[${i}]`, apis[i], `must be one of ${ArrayPrototypeJoin(SUPPORTED_APIS, ", ")}`);
          }
        }
      }
      if (now === undefined) {
      ...
      if (!NumberIsFinite(now) || !NumberIsInteger(now)) {
        throw new ERR_INVALID_ARG_VALUE("options.now", now, "must be a finite, non-negative integer or a Date object");
      }
      this._enabled = true;
      this._now = now;
      for(let i = 0; i < apis.length; i++){
      ...
      for(let i = 0; i < apis.length; i++){
        ...
        if (api === "Date") {
          ...
          this.#mockGlobal("setTimeout", (callback, delay, ...args)=>this._setTimeout(callback, delay, args, false));
          this.#mockGlobal("clearTimeout", (handle)=>this._clearTimer(handle));
        } else if (api === "setInterval") {
          this.#mockGlobal("setInterval", (callback, delay, ...args)=>this._setInterval(callback, delay, args));
          this.#mockGlobal("clearInterval", (handle)=>this._clearTimer(handle));
        } else if (api === "setImmediate") {
          this.#mockGlobal("setImmediate", (callback, ...args)=>this._setTimeout(callback, 0, args, true));
          this.#mockGlobal("clearImmediate", (handle)=>this._clearTimer(handle));
          ...
          globalThis.AbortSignal.timeout = (delay)=>this.#mockedAbortSignalTimeout(delay);
        }
      }
    ...
    reset() {
      ...
      for (const { 0: name, 1: original } of new SafeMapIterator(this.#originals)){
    ...
    #assertTimeArg(time) {
      if (time < 0) {
        throw new ERR_INVALID_ARG_VALUE("time", time, "must be a non-negative integer");
      }
    }
    ...
    #findNextTimer() {
      ...
      for (const { 1: t } of new SafeMapIterator(this._timers)){
        if (next === null || t.fireAt < next.fireAt || t.fireAt === next.fireAt && (t.immediate !== next.immediate ? t.immediate : t.id < next.id)) {
          next = t;
        }
      }
      return next;
  ...
  function createMockDate(mockTimers) {
    function MockDate(...args) {
      if (!new.target) {
        return DatePrototypeToString(ReflectConstruct(originalDate, [
          mockTimers._now
        ], MockDate));
      }
      if (args.length === 0) {
        return ReflectConstruct(originalDate, [
          mockTimers._now
        ], MockDate);
      }
  ...
  const kBadExportsMessage = "Cannot create mock because named exports cannot " + "be applied to the provided default export.";
  ...
  function isUrlLike(value) {
    return value !== null && typeof value === "object" && typeof value.href === "string" && typeof value.protocol === "string";
  }
  ...
  const kStackFrameUrl = new SafeRegExp("((?:file|https?):\\/\\/[^\\s)]+?):\\d+:\\d+");
  function getCallerUrl() {
    ...
    for(let i = 0; i < lines.length; i++){
      const line = lines[i];
      if (StringPrototypeIncludes(line, "ext:deno_node/") || StringPrototypeIncludes(line, "ext:core/")) {
        continue;
      }
      const match = RegExpPrototypeExec(kStackFrameUrl, line);
      if (match !== null) {
        const candidate = match[1];
        if (StringPrototypeStartsWith(candidate, "file:") || StringPrototypeStartsWith(candidate, "http")) {
          return candidate;
        }
      }
    }
  ...
  function detectFormat(key) {
    ...
    if (StringPrototypeEndsWith(key, ".mjs") || StringPrototypeEndsWith(key, ".mts")) {
      return "module";
    }
    if (StringPrototypeEndsWith(key, ".cjs") || StringPrototypeEndsWith(key, ".cts")) {
      return "commonjs";
    }
    ...
    if (StringPrototypeStartsWith(key, "http:") || StringPrototypeStartsWith(key, "https:")) {
      return "module";
    }
    return "commonjs";
  }
  ...
  function generateCjsSource(entry, key) {
    let src = '"use strict";\n';
    src += "const $e = " + registryAccessSource() + ".get(" + JSONStringify(key) + ");\n";
    src += "if ($e === undefined) { throw new Error(" + JSONStringify('mock exports not found for "' + key + '"') + "); }\n";
    if (entry.hasDefaultExport) {
      src += "module.exports = $e.moduleExports.default;\n";
    }
    if (entry.exportNames.length > 0) {
      src += "if (module.exports === null || typeof module.exports !== " + '"object") { throw new Error(' + JSONStringify(kBadExportsMessage) + "); }\n";
      for(let i = 0; i < entry.exportNames.length; i++){
        const name = entry.exportNames[i];
        src += "module.exports[" + JSONStringify(name) + "] = " + "$e.moduleExports[" + JSONStringify(name) + "];\n";
      }
    }
    return src;
  }
  function generateEsmSource(entry, key) {
    let src = "const $e = " + registryAccessSource() + ".get(" + JSONStringify(key) + ");\n";
    src += "if ($e === undefined) { throw new Error(" + JSONStringify('mock exports not found for "' + key + '"') + "); }\n";
    if (entry.isCjs) {
      ...
      if (entry.exportNames.length > 0) {
        src += 'if ($d === null || typeof $d !== "object") { throw new Error(' + JSONStringify(kBadExportsMessage) + "); }\n";
        for(let i = 0; i < entry.exportNames.length; i++){
          const name = entry.exportNames[i];
          src += "$d[" + JSONStringify(name) + "] = $e.moduleExports[" + JSONStringify(name) + "];\n";
        }
      }
  ...
  function mockModuleResolveHook(specifier, context, nextResolve) {
    ...
    if (conditions === undefined || !ArrayPrototypeIncludes(conditions, "import")) {
      return result;
    }
  ...
  function mockModuleLoadHook(url, context, nextLoad) {
    ...
    const conditions = context?.conditions;
    const viaRequire = conditions !== undefined && ArrayPrototypeIncludes(conditions, "require");
    if (viaRequire) {
      if (!entry.cache) {
        evictCjsCache(key);
      }
  ...
  function mockModule(specifier, options = {
    __proto__: null
  }) {
    let specStr;
    if (typeof specifier === "string") {
      ...
      throw new ERR_INVALID_ARG_TYPE("specifier", [
        "string",
        "URL"
      ], specifier);
    }
    ...
    let hasDefaultExport = ObjectPrototypeHasOwnProperty(options, "defaultExport");
    ...
    if (exportsOption !== undefined) {
      if (namedExports !== undefined || options.defaultExport !== undefined) {
        throw new ERR_INVALID_ARG_VALUE("options.exports", exportsOption, 'cannot be used with "namedExports" or "defaultExport"');
      }
    ...
    if (existing !== undefined && existing.active) {
      throw new ERR_INVALID_STATE(`Cannot mock '${specStr}'. The module is already mocked.`);
    }
  ...
  const mock = {
    fn: (original, implementation, options)=>{
      ...
      const ctx = new MockFunctionContext(implementation ?? original, undefined, options?.times);
      ArrayPrototypePush(activeMocks, ctx);
      const mockFn = createMockFunction(original, implementation ?? original, ctx);
      return mockFn;
    },
    ...
    property: function(object, propertyName, value) {
      ...
      if (typeof propertyName !== "string" && typeof propertyName !== "symbol") {
        throw new ERR_INVALID_ARG_TYPE("propertyName", [
          "string",
          "symbol"
        ], propertyName);
      }
      const hasValue = arguments.length > 2;
      const ctx = new MockPropertyContext(object, propertyName, hasValue, value);
      ArrayPrototypePush(activeMocks, ctx);
