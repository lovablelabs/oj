"use strict"; return ((function() {
  ...
  const { ErrorPrototype, ObjectPrototypeIsPrototypeOf, SafeRegExp, StringPrototypeMatch, TypedArrayPrototypeGetByteLength, TypedArrayPrototypeSubarray } = primordials;
  /**
 ...
 */ function writeBuffer(fd, buffer, offset, length, position, ctx) {
    assert ??= lazyLoadAssert();
    assert(offset >= 0, "offset should be greater or equal to 0");
    assert(offset + length <= TypedArrayPrototypeGetByteLength(buffer), `buffer doesn't have enough data: byteLength = ${TypedArrayPrototypeGetByteLength(buffer)}, offset + length = ${offset + length}`);
    if (position) {
