"use strict"; return ((function() {
  ...
  const { AggregateError, ArrayIsArray, ArrayPrototypeIncludes, ArrayPrototypeIndexOf, ArrayPrototypeMap, ArrayPrototypeJoin, ArrayPrototypePush, ArrayPrototypePop, ArrayPrototypeSlice, ArrayPrototypeForEach, ArrayPrototypeSplice, Error, ErrorPrototype, ErrorCaptureStackTrace, JSONStringify, MapPrototypeGet, MapPrototypeSet, SafeMap, MathAbs, NumberIsInteger, ObjectAssign, ObjectDefineProperty, ObjectDefineProperties, ObjectGetOwnPropertyDescriptor, ObjectIsExtensible, ObjectKeys, ObjectPrototypeHasOwnProperty, ObjectPrototypeIsPrototypeOf, ObjectSetPrototypeOf, RangeErrorPrototype, RegExpPrototypeTest, SafeArrayIterator, SafeRegExp, String, StringPrototypeEndsWith, StringPrototypeIncludes, StringPrototypeMatch, StringPrototypeSlice, StringPrototypeStartsWith, StringPrototypeToLocaleLowerCase, StringPrototypeToLowerCase, StringPrototypeToString, Symbol, SymbolFor, SymbolPrototypeToString, SyntaxError, SyntaxErrorPrototype, TypeError, TypeErrorPrototype, URIError, URIErrorPrototype } = primordials;
  ...
  const { codeMap, errorMap, mapSysErrnoToUvErrno, UV_EBADF } = core.loadExtScript("ext:deno_node/internal_binding/uv.ts");
  ...
  const lazyLoadAssert = ()=>{
    return core.createLazyLoader("node:assert")().default;
  };
 ...
 */ const exceptionWithHostPort = hideStackFrames(function exceptionWithHostPort(err, syscall, address, port, additional) {
  ...
  const handleDnsError = hideStackFrames((err, syscall, address)=>{
    ...
    return denoErrorToNodeError(err, {
      syscall
    });
  });
  /**
 ...
 */ const dnsException = hideStackFrames(function(code, syscall, hostname) {
    ...
    if (typeof code === "number") {
      ...
      if (code === MapPrototypeGet(codeMap, "EAI_NODATA") || code === MapPrototypeGet(codeMap, "EAI_NONAME")) {
 ...
 */ class NodeErrorAbstraction extends Error {
    constructor(name, code, message){
      ...
      this.code = code;
      this.name = name;
      this.stack = this.stack && `${name} [${this.code}]${StringPrototypeSlice(this.stack, this.name.length)}`;
    }
    toString() {
  ...
  class NodeSystemError extends Error {
    ...
    // deno-lint-ignore no-explicit-any
    [SymbolFor("nodejs.util.inspect.custom")](_recurseTimes, ctx) {
      return inspect(this, {
        ...ctx,
        getters: true,
        customInspect: false
  ...
  function makeNodeErrorWithCode(Base, key) {
    return class NodeErrorWithCode extends Base {
      ...
      constructor(...args){
        ...
        super(typeof template === "function" ? template(...new SafeArrayIterator(args)) : template);
        this.code = key;
  ...
  // message template.
  function E(sym, // deno-lint-ignore no-explicit-any
  val, // deno-lint-ignore no-explicit-any
  def, // deno-lint-ignore no-explicit-any
  ...otherClasses) {
    MapPrototypeSet(messages, sym, val);
    let cls;
    if (def === SystemError) {
      cls = makeSystemErrorWithCode(sym, val);
    } else {
  ...
  const ERR_FS_CP_DIR_TO_NON_DIR = makeSystemErrorWithCode("ERR_FS_CP_DIR_TO_NON_DIR", "Cannot overwrite non-directory with directory");
  const ERR_FS_CP_EEXIST = makeSystemErrorWithCode("ERR_FS_CP_EEXIST", "Target already exists");
  const ERR_FS_CP_EINVAL = makeSystemErrorWithCode("ERR_FS_CP_EINVAL", "Invalid src or dest");
  const ERR_FS_CP_FIFO_PIPE = makeSystemErrorWithCode("ERR_FS_CP_FIFO_PIPE", "Cannot copy a FIFO pipe");
  const ERR_FS_CP_NON_DIR_TO_DIR = makeSystemErrorWithCode("ERR_FS_CP_NON_DIR_TO_DIR", "Cannot overwrite directory with non-directory");
  const ERR_FS_CP_SOCKET = makeSystemErrorWithCode("ERR_FS_CP_SOCKET", "Cannot copy a socket file");
  const ERR_FS_CP_SYMLINK_TO_SUBDIRECTORY = makeSystemErrorWithCode("ERR_FS_CP_SYMLINK_TO_SUBDIRECTORY", "Cannot overwrite symlink in subdirectory of self");
  const ERR_FS_CP_UNKNOWN = makeSystemErrorWithCode("ERR_FS_CP_UNKNOWN", "Cannot copy an unknown file type");
  const ERR_FS_EISDIR = makeSystemErrorWithCode("ERR_FS_EISDIR", "Path is a directory");
  const ERR_TTY_INIT_FAILED = makeSystemErrorWithCode("ERR_TTY_INIT_FAILED", "TTY initialization failed");
  function createInvalidArgType(name, expected) {
    // https://github.com/nodejs/node/blob/f3eb224/lib/internal/errors.js#L1037-L1087
    expected = ArrayIsArray(expected) ? expected : [
      expected
    ];
    let msg = "The ";
  ...
  class ERR_CRYPTO_TIMING_SAFE_EQUAL_LENGTH extends NodeRangeError {
    constructor(){
      super("ERR_CRYPTO_TIMING_SAFE_EQUAL_LENGTH", "Input buffers must have the same byte length");
    }
  }
  ...
  class ERR_CONSTRUCT_CALL_REQUIRED extends NodeTypeError {
    constructor(x){
      super("ERR_CONSTRUCT_CALL_REQUIRED", `Class constructor ${x} cannot be invoked without \`new\``);
    }
  }
  class ERR_CONSTRUCT_CALL_INVALID extends NodeTypeError {
    constructor(x){
      super("ERR_CONSTRUCT_CALL_INVALID", `Constructor for class ${x} cannot be invoked`);
    }
  }
  class ERR_CLOSED_MESSAGE_PORT extends NodeError {
    constructor(){
      super("ERR_CLOSED_MESSAGE_PORT", "Cannot send data on closed MessagePort");
    }
  }
  ...
  class ERR_CRYPTO_UNKNOWN_DH_GROUP extends NodeError {
    constructor(){
      super("ERR_CRYPTO_UNKNOWN_DH_GROUP", "Unknown DH group");
    }
  }
  class ERR_CRYPTO_UNKNOWN_CIPHER extends NodeError {
    constructor(){
      super("ERR_CRYPTO_UNKNOWN_CIPHER", "Unknown cipher");
    }
  }
  ...
  class ERR_CRYPTO_INVALID_DIGEST extends NodeTypeError {
    constructor(x, prefix){
      super("ERR_CRYPTO_INVALID_DIGEST", prefix ? `Invalid ${prefix} digest: ${x}` : `Invalid digest: ${x}`);
    }
  }
  ...
  class ERR_CRYPTO_INVALID_SCRYPT_PARAMS extends NodeRangeError {
    constructor(details){
      super("ERR_CRYPTO_INVALID_SCRYPT_PARAMS", details ? `Invalid scrypt params: ${details}` : "Invalid scrypt params");
    }
  }
  ...
  class ERR_HTTP_BODY_NOT_ALLOWED extends NodeError {
    constructor(){
      super("ERR_HTTP_BODY_NOT_ALLOWED", "Adding content for this request method or response status is not allowed.");
    }
  }
  class ERR_HTTP_CONTENT_LENGTH_MISMATCH extends NodeError {
    constructor(bodyLength, contentLength){
      super("ERR_HTTP_CONTENT_LENGTH_MISMATCH", `Response body's content-length of ${bodyLength} byte(s) does not match the content-length of ${contentLength} byte(s) set in header`);
    }
  }
  ...
  class ERR_INVALID_MIME_SYNTAX extends NodeTypeError {
    constructor(production, str, invalidIndex){
      const msg = invalidIndex !== -1 ? ` at ${invalidIndex}` : "";
      super("ERR_INVALID_MIME_SYNTAX", `The MIME syntax for a ${production} in "${str}" is invalid` + msg);
    }
  }
  class ERR_INVALID_OBJECT_DEFINE_PROPERTY extends NodeTypeError {
    constructor(message){
      super("ERR_INVALID_OBJECT_DEFINE_PROPERTY", message);
    }
  }
  ...
  class ERR_REQUIRE_ASYNC_MODULE extends NodeError {
    constructor(filename, parentFilename, requireStack = []){
      let message = `require() cannot be used on an ESM graph with top-level await. Use import() instead. To see where the top-level await comes from, use --stack-trace-limit=100 and inspect the dependency graph. Requiring ${filename}. From ${parentFilename}`;
  ...
  class ERR_REQUIRE_CYCLE_MODULE extends NodeError {
    constructor(filename, parentFilename){
      super("ERR_REQUIRE_CYCLE_MODULE", `Cannot require() ES Module ${filename} in a cycle. (from ${parentFilename})`);
      this.name = `Error [${this.code}]`;
      this.toString = nodeErrorToStringWithEmbeddedCode;
    }
  }
  function nodeErrorToStringWithEmbeddedCode() {
  ...
  class ERR_SOCKET_BAD_PORT extends NodeRangeError {
    constructor(name, port, allowZero = true){
      ...
      const portStr = typeof port === "symbol" ? SymbolPrototypeToString(port) : typeof port === "bigint" ? `${port}n` : String(port);
      super("ERR_SOCKET_BAD_PORT", `${name} should be ${operator} 0 and < 65536. Received ${portStr}.`);
    }
  }
  ...
  class ERR_SOCKET_CLOSED_BEFORE_CONNECTION extends NodeError {
    constructor(){
      super("ERR_SOCKET_CLOSED_BEFORE_CONNECTION", `Socket closed before the connection was established`);
    }
  }
  ...
  class ERR_TLS_ALPN_CALLBACK_WITH_PROTOCOLS extends NodeTypeError {
    constructor(){
      super("ERR_TLS_ALPN_CALLBACK_WITH_PROTOCOLS", "The ALPNCallback and ALPNProtocols TLS options are mutually exclusive");
    }
  }
  ...
  class ERR_TLS_ALPN_CALLBACK_INVALID_RESULT extends NodeError {
    constructor(){
      super("ERR_TLS_ALPN_CALLBACK_INVALID_RESULT", `ALPN callback returned a value not present in the client's ALPN protocols`);
    }
  }
  ...
  class ERR_WORKER_INVALID_EXEC_ARGV extends NodeError {
    constructor(errors, msg = "invalid execArgv flags"){
      super("ERR_WORKER_INVALID_EXEC_ARGV", `Initiated Worker with ${msg}: ${ArrayPrototypeJoin(errors, ", ")}`);
    }
  }
  ...
  class ERR_WORKER_PATH extends NodeTypeError {
    constructor(filename){
      const base = "The worker script or module filename must be an absolute path or a relative path starting with './' or '../'.";
      let detail = "";
      if (typeof filename === "string" && (StringPrototypeStartsWith(filename, "file://") || StringPrototypeStartsWith(filename, "File://"))) {
        detail = " Wrap file:// URLs with `new URL`.";
      } else if (typeof filename === "string" && StringPrototypeStartsWith(filename, "data:")) {
  ...
  class ERR_WORKER_MESSAGING_ERRORED extends NodeError {
    constructor(){
      super("ERR_WORKER_MESSAGING_ERRORED", "The destination thread threw an error while receiving the message.");
    }
  }
  class ERR_WORKER_MESSAGING_FAILED extends NodeError {
    constructor(){
      super("ERR_WORKER_MESSAGING_FAILED", "The destination thread refused or failed to receive the message.");
    }
  }
  class ERR_WORKER_MESSAGING_SAME_THREAD extends NodeError {
    constructor(){
      super("ERR_WORKER_MESSAGING_SAME_THREAD", "Cannot send a message to the same thread.");
    }
  }
  class ERR_WORKER_MESSAGING_TIMEOUT extends NodeError {
    constructor(){
      super("ERR_WORKER_MESSAGING_TIMEOUT", "Sending a message to another thread timed out.");
    }
  }
  ...
  class ERR_TRAILING_JUNK_AFTER_STREAM_END extends NodeTypeError {
    constructor(){
      super("ERR_TRAILING_JUNK_AFTER_STREAM_END", "Trailing garbage found after the compressed stream");
    }
  }
  ...
  class ERR_HTTP2_TOO_MANY_CUSTOM_SETTINGS extends NodeError {
    constructor(){
      super("ERR_HTTP2_TOO_MANY_CUSTOM_SETTINGS", "Number of custom settings exceeds MAX_ADDITIONAL_SETTINGS");
    }
  }
  function _http2InvalidSettingMsg(name, actual) {
  ...
  class _ERR_HTTP2_INVALID_SETTING_VALUE_TypeError extends NodeTypeError {
    ...
    constructor(name, actual){
      super("ERR_HTTP2_INVALID_SETTING_VALUE", _http2InvalidSettingMsg(name, actual));
      this.actual = actual;
    }
  }
  // deno-lint-ignore camelcase
  ...
  // deno-lint-ignore no-explicit-any
  const _RangeErrorWithHSFE = _ERR_HTTP2_INVALID_SETTING_VALUE_RangeError;
  _RangeErrorWithHSFE.HideStackFramesError = _ERR_HTTP2_INVALID_SETTING_VALUE_RangeError;
  ...
  class ERR_HTTP2_STREAM_CANCEL extends NodeError {
    cause;
    constructor(error){
      super("ERR_HTTP2_STREAM_CANCEL", error && typeof error.message === "string" ? `The pending stream has been canceled (caused by: ${error.message})` : "The pending stream has been canceled");
      if (error) {
        this.cause = error;
      }
    }
  ...
  class ERR_INVALID_CHAR extends NodeTypeError {
    constructor(name, field){
      super("ERR_INVALID_CHAR", field === undefined ? `Invalid character in ${name}` : `Invalid character in ${name} ["${field}"]`);
    }
  }
  ...
  class ERR_NOT_IMPLEMENTED extends NodeError {
    constructor(message){
      super("ERR_NOT_IMPLEMENTED", message ? `Not implemented: ${message}` : "Not implemented");
    }
  }
  ...
  class ERR_INVALID_PACKAGE_TARGET extends NodeError {
    ...
    target, isImport, base){
      ...
      if (key === ".") {
        ...
        msg = `Invalid "exports" main target ${JSONStringify(target)} defined ` + `in the package config ${displayJoin(pkgPath, "package.json")}${base ? ` imported from ${base}` : ""}${relError ? '; targets must start with "./"' : ""}`;
      } else {
        msg = `Invalid "${isImport ? "imports" : "exports"}" target ${JSONStringify(target)} defined for '${key}' in the package config ${displayJoin(pkgPath, "package.json")}${base ? ` imported from ${base}` : ""}${relError ? '; targets must start with "./"' : ""}`;
      }
  ...
  class ERR_PACKAGE_IMPORT_NOT_DEFINED extends NodeTypeError {
    constructor(specifier, packagePath, base){
      const msg = `Package import specifier "${specifier}" is not defined${packagePath ? ` in package ${displayJoin(packagePath, "package.json")}` : ""} imported from ${base}`;
  ...
  class ERR_PACKAGE_PATH_NOT_EXPORTED extends NodeError {
    constructor(subpath, pkgPath, basePath){
      let msg;
      if (subpath === ".") {
        msg = `No "exports" main defined in ${displayJoin(pkgPath, "package.json")}${basePath ? ` imported from ${basePath}` : ""}`;
      } else {
        msg = `Package subpath '${subpath}' is not defined by "exports" in ${displayJoin(pkgPath, "package.json")}${basePath ? ` imported from ${basePath}` : ""}`;
      }
  ...
  class ERR_PARSE_ARGS_UNEXPECTED_POSITIONAL extends NodeTypeError {
    constructor(x){
      super("ERR_PARSE_ARGS_UNEXPECTED_POSITIONAL", `Unexpected argument '${x}'. This ` + `command does not take positional arguments`);
    }
  }
  class ERR_PARSE_ARGS_UNKNOWN_OPTION extends NodeTypeError {
    constructor(option, allowPositionals){
      ...
      super("ERR_PARSE_ARGS_UNKNOWN_OPTION", `Unknown option '${option}'${suggestDashDash}`);
    }
  }
  ...
  function denoWriteFileErrorToNodeError(e, ctx) {
    if (ObjectPrototypeIsPrototypeOf(Deno.errors.BadResource.prototype, e)) {
      return uvException({
        errno: UV_EBADF,
        ...ctx
  ...
  const denoErrorToNodeSystemError = hideStackFrames((e, syscall)=>{
    ...
    return new NodeSystemError("ERR_SYSTEM_ERROR", ctx, "A system error occurred");
  });
  function extractOsErrorNumberFromErrorMessage(e) {
    if (typeof e === "object" && typeof e.os_errno === "number") {
      return e.os_errno;
    }
    const match = ObjectPrototypeIsPrototypeOf(ErrorPrototype, e) ? StringPrototypeMatch(e.message, new SafeRegExp(/\(os error (\d+)\)/)) : false;
    if (match) {
      return +match[1];
    }
    return undefined;
  ...
  codes.ERR_PARSE_ARGS_UNEXPECTED_POSITIONAL = ERR_PARSE_ARGS_UNEXPECTED_POSITIONAL;
 ...
 */ // deno-lint-ignore no-explicit-any
  ...
  function displayJoin(dir, fileName) {
    const sep = StringPrototypeIncludes(dir, "\\") ? "\\" : "/";
    return StringPrototypeEndsWith(dir, sep) ? dir + fileName : dir + sep + fileName;
  }
  return {
    AbortError,
