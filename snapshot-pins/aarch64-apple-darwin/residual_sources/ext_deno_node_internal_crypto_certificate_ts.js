"use strict"; return ((function() {
  ...
  const { op_node_cert_export_challenge, op_node_cert_export_public_key, op_node_verify_spkac } = core.ops;
  ...
  function verifySpkac(spkac, encoding) {
    return op_node_verify_spkac(getArrayBufferOrView(spkac, "spkac", encoding));
  }
  function exportPublicKey(spkac, encoding) {
    const publicKey = op_node_cert_export_public_key(getArrayBufferOrView(spkac, "spkac", encoding));
    return publicKey ? Buffer.from(publicKey) : "";
  }
  function exportChallenge(spkac, encoding) {
    const challenge = op_node_cert_export_challenge(getArrayBufferOrView(spkac, "spkac", encoding));
