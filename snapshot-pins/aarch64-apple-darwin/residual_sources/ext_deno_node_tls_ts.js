"use strict"; return ((function() {
  ...
  const { getExecArgvOptionValue, getOptionValue } = core.loadExtScript("ext:deno_node/internal/options.ts");
  const { convertALPNProtocols } = core.loadExtScript("ext:deno_node/internal/tls_common.js");
  const { X509Certificate } = core.loadExtScript("ext:deno_node/internal/crypto/x509.ts");
  const { op_get_env_no_permission_check, op_get_root_certificates, op_node_get_ca_certificates, op_set_default_ca_certificates } = core.ops;
  const { validateOneOf, validateString } = core.loadExtScript("ext:deno_node/internal/validators.mjs");
  const { isArrayBufferView } = core.loadExtScript("ext:deno_node/internal/util/types.ts");
  const { ERR_INVALID_ARG_TYPE } = core.loadExtScript("ext:deno_node/internal/errors.ts");
  const { isTypedArray } = core;
  const { ArrayIsArray, ArrayPrototypeIncludes, ArrayPrototypeForEach, ArrayPrototypeMap, ArrayPrototypePush, DataViewPrototypeGetBuffer, DataViewPrototypeGetByteLength, DataViewPrototypeGetByteOffset, Error, ObjectKeys, ObjectFreeze, Proxy, ReflectDefineProperty, ReflectDeleteProperty, ReflectGet, ReflectGetOwnPropertyDescriptor, ReflectHas, ReflectIsExtensible, ReflectOwnKeys, ReflectPreventExtensions, ReflectSet, StringPrototypeIncludes, StringPrototypeSplit, StringPrototypeTrim, StringPrototypeToLowerCase, SafeSet, SetPrototypeAdd, SetPrototypeHas, TypedArrayPrototypeGetBuffer, TypedArrayPrototypeGetByteLength, TypedArrayPrototypeGetByteOffset, Uint8Array } = primordials;
  ...
  function arrayBufferViewToString(view) {
    ...
    const buffer = isTA ? TypedArrayPrototypeGetBuffer(view) : DataViewPrototypeGetBuffer(view);
    const byteOffset = isTA ? TypedArrayPrototypeGetByteOffset(view) : DataViewPrototypeGetByteOffset(view);
    const byteLength = isTA ? TypedArrayPrototypeGetByteLength(view) : DataViewPrototypeGetByteLength(view);
    if (utf8Decoder === null) {
  ...
  function getCiphers() {
    ...
    return ArrayPrototypeMap(ObjectKeys(cipherMap), (name)=>StringPrototypeToLowerCase(name));
  }
  let lazyRootCertificates = null;
  const cachedCACertificates = {
    __proto__: null
  };
  function writeNativeCryptoDebug(message) {
    const nodeDebugNative = op_get_env_no_permission_check("NODE_DEBUG_NATIVE") ?? "";
    if (!StringPrototypeIncludes(nodeDebugNative, "crypto")) {
      return;
    }
    core.print(`${message}\n`, true);
  }
  function emitDefaultCertificatesDebug() {
    const useOpenSslCa = op_get_env_no_permission_check("DENO_NODE_USE_OPENSSL_CA") === "1";
    if (useOpenSslCa) {
      return;
    }
    const stores = [];
    ArrayPrototypeForEach(StringPrototypeSplit(op_get_env_no_permission_check("DENO_TLS_CA_STORE") ?? "mozilla", ","), (store)=>{
    ...
    if (ArrayPrototypeIncludes(stores, "mozilla")) {
      writeNativeCryptoDebug("Started loading bundled root certificates off-thread");
    }
    if (ArrayPrototypeIncludes(stores, "system")) {
      writeNativeCryptoDebug("Started loading system root certificates off-thread");
    }
    if (op_get_env_no_permission_check("NODE_EXTRA_CA_CERTS")) {
      writeNativeCryptoDebug("Started loading extra root certificates off-thread");
    }
  }
  function ensureLazyRootCertificates(target) {
    if (lazyRootCertificates === null) {
      lazyRootCertificates = op_get_root_certificates();
      // Clear target and repopulate
      target.length = 0;
      ArrayPrototypeForEach(lazyRootCertificates, (v)=>ArrayPrototypePush(target, v));
      ObjectFreeze(target);
    }
  }
  const rootCertificates = new Proxy([], {
  ...
  function getDefaultUsesSystemCertificates() {
    ...
    if (globalThis.process?.env?.NODE_USE_SYSTEM_CA === "1" || op_get_env_no_permission_check("NODE_USE_SYSTEM_CA") === "1") {
      return true;
    }
  ...
  function getDefaultUsesBundledCertificates() {
    const storesValue = op_get_env_no_permission_check("DENO_TLS_CA_STORE");
    if (storesValue != null) {
  ...
  function makeNodeCryptoError(code, message) {
    const err = new Error(message);
    err.code = code;
    return err;
  }
  function validateDefaultCACertificates(certs) {
    let validCount = 0;
    for(let i = 0; i < certs.length; i++){
      try {
        ...
        if (validCount === 0) {
          throw makeNodeCryptoError("ERR_CRYPTO_OPERATION_FAILED", "No valid certificates found in the provided array");
        }
        throw makeNodeCryptoError("ERR_OSSL_PEM_ASN.1_ENCODING_ROUTINES", "error:0680009B:asn1 encoding routines::too long");
      }
    }
  }
  function setDefaultCACertificates(certs) {
    ...
    for(let i = 0; i < certs.length; ++i){
      ...
      if (typeof cert === "string") {
        ...
        throw new ERR_INVALID_ARG_TYPE(`certs[${i}]`, [
          "string",
          "ArrayBufferView"
        ], cert);
      }
    ...
    ArrayPrototypeForEach(ObjectKeys(cachedCACertificates), (key)=>{
      if (key !== "bundled") {
  ...
  function getCACertificates(type = "default") {
    ...
    if (type === "default" && !hasDefaultCACertificatesOverride) {
      ...
      if (getDefaultUsesBundledCertificates()) {
        ArrayPrototypeForEach(getCACertificates("bundled"), (cert)=>ArrayPrototypePush(certs, cert));
      }
      if (getDefaultUsesSystemCertificates()) {
        ArrayPrototypeForEach(getCACertificates("system"), (cert)=>ArrayPrototypePush(certs, cert));
      }
      ArrayPrototypeForEach(getCACertificates("extra"), (cert)=>ArrayPrototypePush(certs, cert));
      ObjectFreeze(certs);
      emitDefaultCertificatesDebug();
    } else if (type === "bundled") {
      certs = rootCertificates;
    } else {
      certs = ObjectFreeze(op_node_get_ca_certificates(type));
      if (type === "default") {
        emitDefaultCertificatesDebug();
      }
    }
