"use strict"; return ((function() {
  ...
  const { ArrayPrototypeSlice, MapPrototypeGet, MapPrototypeSet, SafeArrayIterator, SafeMap } = primordials;
  const { notImplemented } = core.loadExtScript("ext:deno_node/_utils.ts");
  const { Buffer } = core.loadExtScript("ext:deno_node/internal/buffer.mjs");
  const { ERR_CRYPTO_INVALID_DIGEST, ERR_INVALID_ARG_TYPE, hideStackFrames } = core.loadExtScript("ext:deno_node/internal/errors.ts");
  ...
  const { validateInt32, validateObject } = core.loadExtScript("ext:deno_node/internal/validators.mjs");
  const { kHandle, kKeyObject } = core.loadExtScript("ext:deno_node/internal/crypto/constants.ts");
  ...
  const cipherInfoByName = new SafeMap();
  const cipherInfoByNid = new SafeMap();
  ...
  // Aliases
  MapPrototypeSet(cipherInfoByName, "aes128", MapPrototypeGet(cipherInfoByName, "aes-128-cbc"));
  MapPrototypeSet(cipherInfoByName, "aes192", MapPrototypeGet(cipherInfoByName, "aes-192-cbc"));
  MapPrototypeSet(cipherInfoByName, "aes256", MapPrototypeGet(cipherInfoByName, "aes-256-cbc"));
  ...
  function getHashBlockSize(algorithm) {
  ...
  function getCipherInfo(nameOrNid, options) {
    ...
    const info = typeof nameOrNid === "number" ? MapPrototypeGet(cipherInfoByNid, nameOrNid) : MapPrototypeGet(cipherInfoByName, nameOrNid);
    if (info === undefined) {
      return undefined;
    }
