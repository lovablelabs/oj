"use strict"; return ((function() {
  ...
  const { ArrayIsArray, ArrayPrototypeConcat, ArrayPrototypeIncludes, ArrayPrototypeMap, ArrayPrototypePush, Error, MathMax, Number, NumberIsNaN, ObjectAssign, ObjectKeys, SafeSet, String, StringFromCharCode, StringPrototypeCharCodeAt, StringPrototypeSlice, StringPrototypeToLowerCase, Symbol } = primordials;
  ...
  const { codes, hideStackFrames } = core.loadExtScript("ext:deno_node/internal/errors.ts");
  const { HTTP2_HEADER_ACCESS_CONTROL_ALLOW_CREDENTIALS, HTTP2_HEADER_ACCESS_CONTROL_MAX_AGE, HTTP2_HEADER_ACCESS_CONTROL_REQUEST_METHOD, HTTP2_HEADER_AGE, HTTP2_HEADER_AUTHORITY, HTTP2_HEADER_AUTHORIZATION, HTTP2_HEADER_CONNECTION, HTTP2_HEADER_CONTENT_ENCODING, HTTP2_HEADER_CONTENT_LANGUAGE, HTTP2_HEADER_CONTENT_LENGTH, HTTP2_HEADER_CONTENT_LOCATION, HTTP2_HEADER_CONTENT_MD5, HTTP2_HEADER_CONTENT_RANGE, HTTP2_HEADER_CONTENT_TYPE, HTTP2_HEADER_COOKIE, HTTP2_HEADER_DATE, HTTP2_HEADER_DNT, HTTP2_HEADER_ETAG, HTTP2_HEADER_EXPIRES, HTTP2_HEADER_FROM, HTTP2_HEADER_HOST, HTTP2_HEADER_HTTP2_SETTINGS, HTTP2_HEADER_IF_MATCH, HTTP2_HEADER_IF_MODIFIED_SINCE, HTTP2_HEADER_IF_NONE_MATCH, HTTP2_HEADER_IF_RANGE, HTTP2_HEADER_IF_UNMODIFIED_SINCE, HTTP2_HEADER_KEEP_ALIVE, HTTP2_HEADER_LAST_MODIFIED, HTTP2_HEADER_LOCATION, HTTP2_HEADER_MAX_FORWARDS, HTTP2_HEADER_METHOD, HTTP2_HEADER_PATH, HTTP2_HEADER_PROTOCOL, HTTP2_HEADER_PROXY_AUTHORIZATION, HTTP2_HEADER_PROXY_CONNECTION, HTTP2_HEADER_RANGE, HTTP2_HEADER_REFERER, HTTP2_HEADER_RETRY_AFTER, HTTP2_HEADER_SCHEME, HTTP2_HEADER_SET_COOKIE, HTTP2_HEADER_STATUS, HTTP2_HEADER_TE, HTTP2_HEADER_TK, HTTP2_HEADER_TRANSFER_ENCODING, HTTP2_HEADER_UPGRADE, HTTP2_HEADER_UPGRADE_INSECURE_REQUESTS, HTTP2_HEADER_USER_AGENT, HTTP2_HEADER_X_CONTENT_TYPE_OPTIONS, HTTP2_METHOD_CONNECT, HTTP2_METHOD_DELETE, HTTP2_METHOD_GET, HTTP2_METHOD_HEAD, NGHTTP2_NV_FLAG_NO_INDEX, NGHTTP2_NV_FLAG_NONE, NGHTTP2_SESSION_CLIENT, NGHTTP2_SESSION_SERVER } = core.loadExtScript("ext:deno_node/internal/http2/constants.ts");
  const { ERR_HTTP2_CONNECT_AUTHORITY, ERR_HTTP2_CONNECT_PATH, ERR_HTTP2_CONNECT_SCHEME, ERR_HTTP2_HEADER_SINGLE_VALUE, ERR_HTTP2_INVALID_CONNECTION_HEADERS, ERR_HTTP2_INVALID_PSEUDOHEADER, ERR_HTTP2_INVALID_SETTING_VALUE, ERR_HTTP2_TOO_MANY_CUSTOM_SETTINGS, ERR_INVALID_ARG_TYPE, ERR_INVALID_HTTP_TOKEN } = codes;
  ...
  let { settingsBuffer, optionsBuffer, sessionState, streamState } = op_http2_http_state();
  let httpStateRuntimeReady = false;
  function ensureHttpStateBuffers() {
    if (httpStateRuntimeReady) return;
    httpStateRuntimeReady = true;
    ({ settingsBuffer, optionsBuffer, sessionState, streamState } = op_http2_http_state());
  }
  ...
  function addCustomSettingsToObj() {
    ...
    for(let i = 0; i < num; i++){
      ...
      toRet[settingsBuffer[IDX_SETTINGS_FLAGS + 1 + 2 * i + 1].toString()] = Number(settingsBuffer[IDX_SETTINGS_FLAGS + 1 + 2 * i + 2]);
    }
    return toRet;
  }
  ...
  function updateSettingsBuffer(settings) {
    ...
    if (typeof settings.customSettings === "object") {
      ...
      for(const setting in customSettings){
        ...
        if (typeof val === "number") {
          ...
          if (NumberIsNaN(nsetting) || typeof nsetting !== "number" || 0 >= nsetting || nsetting > 0xffff) {
            throw new ERR_HTTP2_INVALID_SETTING_VALUE.RangeError("Range Error", nsetting, 0, 0xffff);
          }
          if (NumberIsNaN(val) || typeof val !== "number" || 0 >= val || val > 0xffffffff) {
    ...
    if (typeof settings.maxHeaderListSize === "number" || typeof settings.maxHeaderSize === "number") {
      ...
      if (settings.maxHeaderSize !== undefined && settings.maxHeaderSize !== settings.maxHeaderListSize) {
        lazyProcess().default.emitWarning("settings.maxHeaderSize overwrite settings.maxHeaderListSize");
 ...
 */ function prepareRequestHeadersArray(headers, session) {
    ...
    if (!connect || protocol !== undefined) {
      if (authority === undefined && headers[HTTP2_HEADER_HOST] === undefined) {
        ...
        ArrayPrototypePush(additionalPsuedoHeaders, HTTP2_HEADER_AUTHORITY, authority);
      }
      if (scheme === undefined) {
    ...
    const rawHeaders = additionalPsuedoHeaders.length ? ArrayPrototypeConcat(additionalPsuedoHeaders, headers) : headers;
    if (headers[kSensitiveHeaders] !== undefined) {
      rawHeaders[kSensitiveHeaders] = headers[kSensitiveHeaders];
    }
    const headersList = buildNgHeaderString(rawHeaders, assertValidPseudoHeader, session[kStrictSingleValueFields]);
    return {
      rawHeaders,
      headersList,
      scheme,
 ...
 */ function prepareRequestHeadersObject(headers, session) {
    const headersObject = ObjectAssign({
      __proto__: null
    }, headers);
    ...
    if (!connect || headersObject[HTTP2_HEADER_PROTOCOL] !== undefined) {
      ...
      if (headersObject[HTTP2_HEADER_SCHEME] === undefined) {
        headersObject[HTTP2_HEADER_SCHEME] = StringPrototypeSlice(session[kProtocol], 0, -1);
      }
    ...
    const headersList = buildNgHeaderString(headersObject, assertValidPseudoHeader, session[kStrictSingleValueFields]);
    return {
      headersObject,
      headersList,
  ...
  function escapeNgHeaderValueZeroBytes(value) {
    ...
    for(let i = 0; i < value.length; ++i){
      ...
      changed = true;
      escaped += StringPrototypeSlice(value, start, i) + kInvalidHeaderValueByte;
      start = i + 1;
    }
 ...
 */ function buildNgHeaderString(arrayOrMap, assertValuePseudoHeader = assertValidPseudoHeader, strictSingleValueFields) {
    ...
    const neverIndex = ArrayPrototypeMap(sensitiveHeaders, (v)=>StringPrototypeToLowerCase(v));
    function processHeader(key, value) {
      key = StringPrototypeToLowerCase(key);
      const isSingleValueHeader = strictSingleValueFields && kSingleValueHeaders.has(key);
  ...
  class NghttpError extends Error {
    constructor(integerCode, customErrorCode){
      super(customErrorCode ? kCustomErrorMessages[customErrorCode] ?? customErrorCode : op_http2_error_string(integerCode));
  ...
  const assertIsArray = hideStackFrames((value, name, types)=>{
    if (value !== undefined && (value === null || !ArrayIsArray(value))) {
      throw new ERR_INVALID_ARG_TYPE(name, types || "Array", value);
    }
  });
  ...
  function toHeaderObject(headers, sensitiveHeaders) {
    ...
    for(let n = 0; n < headers.length; n += 2){
      ...
      if (existing === undefined) {
        obj[name] = name === HTTP2_HEADER_SET_COOKIE ? [
          value
        ] : value;
