...
const { Array, ArrayIsArray, ArrayPrototypeJoin, ArrayPrototypePush, ArrayPrototypePushApply, ArrayPrototypeShift, ArrayPrototypeUnshift, Error, FunctionPrototypeBind, FunctionPrototypeCall, MathFloor, NumberPrototypeToString, ObjectCreate, ObjectDefineProperties, ObjectDefineProperty, ObjectKeys, ObjectSetPrototypeOf, ObjectValues, SafeRegExp, SafeSet, String, StringPrototypeToLowerCase, StringPrototypeToString, Symbol, SymbolIterator, TypedArrayPrototypeGetByteLength } = primordials;
const { getDefaultHighWaterMark } = core.loadExtScript("ext:deno_node/internal/streams/state.js");
...
const { kNeedDrain, kOutHeaders, utcDate } = core.loadExtScript("ext:deno_node/internal/http.ts");
const { Buffer } = core.loadExtScript("ext:deno_node/internal/buffer.mjs");
import { _checkInvalidHeaderChar as checkInvalidHeaderChar, _checkIsHttpToken as checkIsHttpToken, chunkExpression as RE_TE_CHUNKED } from "node:_http_common";
const { defaultTriggerAsyncIdScope, symbols } = core.loadExtScript("ext:deno_node/internal/async_hooks.ts");
const { async_id_symbol } = symbols;
const { ERR_HTTP_BODY_NOT_ALLOWED, ERR_HTTP_CONTENT_LENGTH_MISMATCH, ERR_HTTP_HEADERS_SENT, ERR_HTTP_INVALID_HEADER_VALUE, ERR_HTTP_TRAILER_INVALID, ERR_INVALID_ARG_TYPE, ERR_INVALID_ARG_VALUE, ERR_INVALID_CHAR, ERR_INVALID_HTTP_TOKEN, ERR_METHOD_NOT_IMPLEMENTED, ERR_STREAM_CANNOT_PIPE, ERR_STREAM_DESTROYED, ERR_STREAM_NULL_VALUES, ERR_STREAM_WRITE_AFTER_END, errnoException, hideStackFrames } = core.loadExtScript("ext:deno_node/internal/errors.ts");
const { kLastWriteWasAsync, streamBaseState } = core.loadExtScript("ext:deno_node/internal_binding/stream_wrap.ts");
...
export const kRejectNonStandardBodyWrites = Symbol("kRejectNonStandardBodyWrites");
const nop = ()=>{};
const RE_CONN_CLOSE = new SafeRegExp(/(?:^|\W)close(?:$|\W)/i);
function isCookieField(s) {
  return s.length === 6 && StringPrototypeToLowerCase(s) === "cookie";
}
export function OutgoingMessage(options) {
...
ObjectDefineProperties(OutgoingMessage.prototype, {
  ...
  writableHighWaterMark: {
    ...
    get: function writableHighWaterMark() {
      return this[kSocket] ? this[kSocket].writableHighWaterMark : this[kHighWaterMark] || HIGH_WATER_MARK;
    },
    enumerable: true,
    configurable: true
  },
  ...
  setHeader: {
    ...
    value: function setHeader(name, value) {
      ...
      headers[StringPrototypeToLowerCase(name)] = [
        name,
        value
      ];
      return this;
  ...
  addTrailers: {
    ...
    value: function end(chunk, encoding, callback) {
      ...
      if (chunk) {
        if (this.finished) {
          _onError(this, new ERR_STREAM_WRITE_AFTER_END(), typeof callback !== "function" ? nop : callback);
          return this;
        }
      ...
      if (_checkStrictContentLength(this) && this[kBytesWritten] !== this._contentLength) {
   ...
   *  client. This is invoked by onSocket(). */ _flushHeaders: {
    __proto__: null,
  ...
  _flushBuffer: {
    ...
    value: function _flushBuffer() {
      ...
      const { data, encoding, callback } = ArrayPrototypeShift(this.outputData);
  ...
  _writeRaw: {
    ...
    value: function _writeRaw(data, encoding, callback) {
      ...
      ArrayPrototypePush(this.outputData, {
        data,
        encoding,
        callback
      });
        ...
        if (headers === this[kOutHeaders]) {
          ...
          if (headers.length && ArrayIsArray(headers[0])) {
            ...
            for(let n = 0; n < headers.length; n += 2){
              this._storeHeaderEntry(state, headers[n], headers[n + 1], true);
            }
          }
        } else {
  ...
  _storeHeaderEntry: {
    __proto__: null,
    value: function _storeHeaderEntry(state, field, value, validate) {
      if (validate) {
  ...
  _matchHeader: {
    __proto__: null,
    value: function _matchHeader(state, field, value) {
...
function _checkStrictContentLength(msg) {
...
function write_(msg, chunk, encoding, callback, fromEnd) {
  ...
  if (msg.strictContentLength) {
    len ??= typeof chunk === "string" ? Buffer.byteLength(chunk, encoding) : TypedArrayPrototypeGetByteLength(chunk);
    if (_checkStrictContentLength(msg) && (fromEnd ? msg[kBytesWritten] + len !== msg._contentLength : msg[kBytesWritten] + len > msg._contentLength)) {
...
function tryDirectEnd(msg, chunk, encoding, callback) {
  if (typeof chunk !== "string" || msg.req === undefined || msg.strictContentLength || msg._header !== null || msg.outputData.length !== 0 || msg[kChunkedLength] !== 0 || msg._bodyWriter !== null || !msg._hasBody || msg._trailer !== "" || msg._removedContLen || msg._removedTE || msg.chunkedEncoding || msg.statusCode === 204 || msg.statusCode === 304) {
    return false;
  }
  ...
  if (headers !== null && (headers["transfer-encoding"] !== undefined || headers.trailer !== undefined)) {
    return false;
  }
  const socket = msg.socket;
  const writableState = socket?._writableState;
  if (socket == null || socket.destroyed || !socket.writable || socket.connecting || socket._httpMessage !== msg || socket._handle?.isStreamBase !== true || typeof socket._handle.writeUtf8String !== "function" || normalizedEncoding === "latin1" && typeof socket._handle.writeLatin1String !== "function" || writableState === undefined || writableState.corked !== 0 || writableState.length !== 0 || writableState.needDrain) {
    return false;
  }
...
function tryDirectEmptyEnd(msg, callback) {
  if (msg.req === undefined || msg.strictContentLength || msg._headerSent || msg.outputData.length !== 0 || msg[kChunkedLength] !== 0 || msg._bodyWriter !== null || !msg._hasBody || msg._trailer !== "" || msg._removedContLen || msg._removedTE || msg.chunkedEncoding || msg.statusCode === 204 || msg.statusCode === 304) {
    return false;
  }
  const headers = msg[kOutHeaders];
  if (headers !== null && (headers["transfer-encoding"] !== undefined || headers.trailer !== undefined)) {
  ...
  const socket = msg.socket;
  const writableState = socket?._writableState;
  if (socket == null || socket.destroyed || !socket.writable || socket.connecting || socket._httpMessage !== msg || socket._handle?.isStreamBase !== true || typeof socket._handle.writeLatin1String !== "function" || writableState === undefined || writableState.corked !== 0 || writableState.length !== 0 || writableState.needDrain) {
    return false;
  }
  if (msg._header === null) {
...
function normalizeDirectStringEncoding(encoding) {
...
function writeDirectString(socket, data, encoding, finish) {
...
function _onError(msg, err, callback) {
