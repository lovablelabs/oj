"use strict"; return ((function() {
  ...
  const { op_http_cancel, op_http_close, op_http_close_after_finish, op_http_copy_span_to_otel_info, op_http_get_request_header, op_http_get_request_headers, op_http_get_request_method, op_http_get_request_url, op_http_get_request_remote_addr, op_http_is_raw_request, op_http_metric_handle_otel_error, op_http_notify_serving, op_http_read_request_body, op_http_request_on_cancel, op_http_serve, op_http_serve_address_override, op_http_serve_default_compression, op_http_serve_on, op_http_set_promise_complete, op_http_set_response_native, op_http_set_response_body_bytes, op_http_set_response_body_bytes_with_headers, op_http_set_response_body_resource, op_http_set_response_body_static_with_content_type, op_http_set_response_body_static_with_default_header, op_http_set_response_body_static_with_header, op_http_set_response_body_text, op_http_set_response_body_text_with_headers, op_http_set_response_header, op_http_set_response_headers, op_http_set_response_trailers, op_http_try_take_full_request_body, op_http_try_take_full_request_body_text, op_http_upgrade_websocket_next, op_http_wait } = core.ops;
  const { ArrayPrototypeFind, ArrayPrototypeMap, ArrayPrototypePush, ObjectHasOwn, ObjectPrototypeIsPrototypeOf, PromisePrototype, PromisePrototypeCatch, PromiseResolve, SafeArrayIterator, SafePromisePrototypeFinally, SafePromiseAll, PromisePrototypeThen, StringPrototypeIncludes, StringPrototypeSlice, StringPrototypeStartsWith, StringPrototypeToLowerCase, Symbol, SymbolAsyncDispose, TypeError, TypedArrayPrototypeGetByteLength, TypedArrayPrototypeGetSymbolToStringTag, Uint8Array, Promise, Number } = primordials;
  const { InnerBody } = core.loadExtScript("ext:deno_fetch/22_body.js");
  const { dropServeNativeResponse, getInnerResponse, responseBodyUsed, ResponsePrototype, serveNativeResponseKey, serveFastBodyKey, serveFastConsumedKey, serveFastContentTypeKey, serveFastHeaderKindKey, serveFastStatusKey, SERVE_FAST_HEADER_CONTENT_TYPE, SERVE_FAST_HEADER_DEFAULT_TEXT, SERVE_FAST_HEADER_NONE, toInnerResponse, wireHeaderList } = core.loadExtScript("ext:deno_fetch/23_response.js");
  const { abortRequest, cacheRequestHeaders, fromInnerRequest, requestHeadersExposed } = core.loadExtScript("ext:deno_fetch/23_request.js");
  const { AbortController } = core.loadExtScript("ext:deno_web/03_abort_signal.js");
  const { getReadableStreamResourceBacking, isReadableStreamDisturbed, readableStreamForRid, ReadableStreamPrototype, resourceForReadableStream } = core.loadExtScript("ext:deno_web/06_streams.js");
  const { listen, listenOptionApiName } = core.loadExtScript("ext:deno_net/01_net.js");
  const { hasTlsKeyPairOptions, listenTls } = core.loadExtScript("ext:deno_net/02_tls.js");
  const { otelState, builtinTracer, ContextManager, currentSnapshot, enterSpan, restoreSnapshot } = core.loadExtScript("ext:deno_telemetry/telemetry.ts");
  const { updateSpanFromRequest, updateSpanFromServerResponse } = core.loadExtScript("ext:deno_telemetry/util.ts");
  ...
  class InnerRequest {
    ...
    close(success = true) {
      if (this.#streamRid !== undefined) {
        ...
        const beingRead = ObjectPrototypeIsPrototypeOf(ReadableStreamPrototype, stream) && (stream.locked || isReadableStreamDisturbed(stream));
        if (!beingRead) {
      ...
      if (this.#completed) {
        if (success) {
          ...
          this.#completed.reject(new Interrupted("HTTP response was not sent successfully"));
        }
      }
      if (this.#context.legacyAbort) {
    ...
    get body() {
      ...
      if (this.method == "GET" || this.method == "HEAD" || this.method == "CONNECT") {
        this.#body = null;
        return null;
      }
      ...
      this.#body = new InnerBody(// `autoClose: true` so the stream closes its own resource when it reaches
      ...
      readableStreamForRid(this.#streamRid, true, undefined, (controller, error)=>{
    ...
    onCancel(callback) {
      ...
      PromisePrototypeThen(op_http_request_on_cancel(this.#external), (r)=>{
  ...
  class CallbackContext {
    ...
    serverRid;
    closed;
    /** @type {Promise<void> | undefined} */ closing;
    listener;
  ...
  function trySetServeFastStaticResponse(req, response, innerRequest, closeInnerRequest = closeInnerRequestImmediately) {
    ...
    switch(response[serveFastHeaderKindKey]){
      ...
      case SERVE_FAST_HEADER_CONTENT_TYPE:
        op_http_set_response_body_static_with_content_type(req, body, status, response[serveFastContentTypeKey]);
        return true;
  ...
  function fastSyncResponseOrStream(req, respBody, status, innerRequest, headers, onError) {
    ...
    if (body !== undefined) {
      ...
      if (singleHeader !== null && (singleHeader[0] === "Content-Type" || singleHeader[0] === "content-type") && singleHeader[1] === "text/plain;charset=UTF-8") {
        innerRequest?.close();
        op_http_set_response_body_static_with_default_header(req, body, status);
        return;
      }
      if (singleHeader !== null) {
        innerRequest?.close();
        if (singleHeader[0] === "Content-Type" || singleHeader[0] === "content-type") {
          op_http_set_response_body_static_with_content_type(req, body, status, singleHeader[1]);
          return;
        }
        op_http_set_response_body_static_with_header(req, body, status, singleHeader[0], singleHeader[1]);
        return;
      }
    }
    if (TypedArrayPrototypeGetSymbolToStringTag(body) === "Uint8Array") {
      innerRequest?.close();
      if (headers?.length > 0) {
        op_http_set_response_body_bytes_with_headers(req, body, status, headers);
      } else {
        op_http_set_response_body_bytes(req, body, status);
      }
      return;
    ...
    if (typeof body === "string") {
      innerRequest?.close();
      if (headers?.length > 0) {
        op_http_set_response_body_text_with_headers(req, body, status, headers);
      } else {
        op_http_set_response_body_text(req, body, status);
      }
      return;
 ...
 */ function mapToCallback(context, callback, onError) {
    ...
    let mapped = async function(req, span) {
      ...
      try {
        ...
        if (inner === undefined) {
          throw new TypeError("Return value from serve handler must be a Response constructed via the Response constructor in this realm");
        }
        if (inner.type === "error") {
        ...
        try {
          ...
          if (inner === undefined) {
            throw new TypeError("Return value from onError handler must be a Response constructed via the Response constructor in this realm");
          }
        } catch (error) {
          if (otelState.METRICS_ENABLED) {
            op_http_metric_handle_otel_error(req);
          }
          internals.log("error", "Exception in onError while handling exception", error);
      ...
      if (innerRequest?.[_upgraded]) {
        if (response.status !== 101) {
          internals.log("error", "Upgrade response was not returned from callback");
          context.close();
          return;
        }
      }
      ...
      if (nativeResponse !== null && nativeResponse !== undefined && op_http_set_response_native(req, nativeResponse)) {
      ...
      fastSyncResponseOrStream(req, respBody, status, innerRequest, headers, (error)=>reportResponseStreamError(onError, error));
    };
      ...
      mapped = function(req, _span) {
        ...
        for (const propagator of new SafeArrayIterator(otelState.PROPAGATORS)){
          activeContext = propagator.extract(activeContext, headers, {
            get (carrier, key) {
              return ArrayPrototypeFind(carrier, (carrierEntry)=>carrierEntry[0] === key)?.[1];
            },
            keys (carrier) {
        ...
        const span = builtinTracer().startSpan("deno.serve", {
          kind: 1
        }, activeContext);
  ...
  function mapToNativeResponseCallback(context, callback, onError) {
    const zeroArgCallback = callback.length === 0 && !otelState.TRACING_ENABLED && !otelState.METRICS_ENABLED;
    function finishOrReturnNative(req, span, innerRequest, response, fromPromise = false) {
      if (!ObjectPrototypeIsPrototypeOf(ResponsePrototype, response)) {
        throw new TypeError("Return value from serve handler must be a response or a promise resolving to a response");
      }
      ...
      if (innerRequest?.request !== undefined && requestHeadersExposed(innerRequest.request)) {
      ...
      if (trySetServeFastStaticResponse(req, response, innerRequest, fromPromise ? closeInnerRequestImmediately : closeInnerRequestForNative)) {
        return undefined;
      }
      const nativeResponse = response[serveNativeResponseKey];
      if (nativeResponse !== null && nativeResponse !== undefined && op_http_set_response_native(req, nativeResponse)) {
      ...
      inner = toInnerResponse(response);
      fastSyncResponseOrStream(req, inner.body, inner.status, innerRequest, wireHeaderList(inner), (error)=>reportResponseStreamError(onError, error));
      return undefined;
    }
    function handleOnErrorError(req, span, innerRequest, error) {
      ...
      internals.log("error", "Exception in onError while handling exception", error);
      return finishOrReturnNative(req, span, innerRequest, internalServerError());
    }
    function handleError(req, span, innerRequest, error) {
      ...
      try {
        return finishOrReturnMaybePromise(req, span, innerRequest, response, true);
      } catch (error) {
        return handleOnErrorError(req, span, innerRequest, error);
      }
    }
    function finishOrReturnMaybePromise(req, span, innerRequest, response, isOnErrorResponse = false) {
      if (response !== null && (typeof response === "object" || typeof response === "function") && typeof response.then === "function" || innerRequest?.request !== undefined && requestHeadersExposed(innerRequest.request)) {
        const finished = PromisePrototypeThen(PromiseResolve(response), (response)=>finishOrReturnNative(req, span, innerRequest, response, true));
        return PromisePrototypeThen(finished, undefined, (error)=>isOnErrorResponse ? handleOnErrorError(req, span, innerRequest, error) : handleError(req, span, innerRequest, error));
      }
  ...
  function serveInner(options, handler) {
    ...
    const automaticCompression = options.automaticCompression ?? op_http_serve_default_compression();
    ...
    if (wantsTunnel) {
      ...
      return serveHttpOnListener(listener, signal, handler, onError, ()=>{
        if (options.onListen) {
          ...
          const additional = listener.addr.port === 443 ? "" : `:${listener.addr.port}`;
          internals.log("info", `Listening on https://${formatHostName(listener.addr.hostname)}${additional}`);
        }
      }, automaticCompression);
    }
    const listenOpts = {
    ...
    const onListen = (scheme)=>{
      if (options.onListen) {
        ...
        const helper = host !== "localhost" && (addr.hostname === "0.0.0.0" || addr.hostname === "::") ? ` (${scheme}localhost:${addr.port}/)` : "";
        internals.log("info", `Listening on ${url}${helper}`);
      }
    };
    return serveHttpOnListener(listener, signal, handler, onError, onListen, automaticCompression);
  }
  /**
 * Serve HTTP/1.1 and/or HTTP/2 on an arbitrary listener.
 */ function serveHttpOnListener(listener, signal, handler, onError, onListen, automaticCompression = op_http_serve_default_compression()) {
    let serverContext = undefined;
    let callback = undefined;
    let nativeCallback = undefined;
    const promiseErrorHandler = (error)=>{
      internals.log("error", "Terminating Deno.serve loop due to unexpected error", error);
      serverContext?.close();
    };
    const dispatch = (req)=>{
      PromisePrototypeCatch(callback(req, undefined), promiseErrorHandler);
    };
    const nativeFastPath = !otelState.TRACING_ENABLED && !otelState.METRICS_ENABLED;
    ...
    serverContext = new CallbackContext(signal, op_http_serve(listener[internalRidSymbol], automaticCompression, dispatch, rawNoRequest, nativeDispatch, serveNativeResponseKey, serveFastStatusKey, serveFastBodyKey, serveFastHeaderKindKey, serveFastContentTypeKey, serveFastConsumedKey), listener);
 ...
 */ function serveHttpOnConnection(connection, signal, handler, onError, onListen) {
    ...
    serverContext = new CallbackContext(signal, op_http_serve_on(connection[internalRidSymbol], automaticCompression, dispatch, rawNoRequest, nativeDispatch, serveNativeResponseKey, serveFastStatusKey, serveFastBodyKey, serveFastHeaderKindKey, serveFastContentTypeKey, serveFastConsumedKey), null);
    callback = mapToCallback(serverContext, handler, onError);
    nativeCallback = mapToNativeResponseCallback(serverContext, handler, onError);
    onListen(serverContext.scheme);
  ...
  function serveHttpOn(context, addr) {
    ...
    const finished = (async ()=>{
      ...
      try {
        ...
        if (!ObjectPrototypeIsPrototypeOf(BadResourcePrototype, error) && !ObjectPrototypeIsPrototypeOf(InterruptedPrototype, error)) {
  ...
  function registerDeclarativeServer(exports) {
    ...
    if (exports.onListen !== undefined && typeof exports.onListen !== "function") {
    ...
    return ({ servePort, serveHost, workerCountWhenMain })=>{
      const server = Deno.serve({
        port: servePort,
        hostname: serveHost,
        [kLoadBalanced]: workerCountWhenMain == null ? true : workerCountWhenMain > 0,
        onListen: (localAddr)=>{
          if (workerCountWhenMain != null) {
            ...
            switch(localAddr.transport){
              case "tcp":
                target = `http://${formatHostName(localAddr.hostname)}:${localAddr.port}/`;
                break;
              case "unix":
            ...
            const nThreads = workerCountWhenMain > 0 ? ` with ${workerCountWhenMain + 1} threads` : "";
            internals.log("info", `%cdeno serve%c: Listening on %c${target}%c${nThreads}`, "color: green", "color: inherit", "color: yellow", "color: inherit");
          }
        },
