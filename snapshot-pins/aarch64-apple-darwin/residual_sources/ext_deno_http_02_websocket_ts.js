"use strict"; return ((function() {
  ...
  const { op_http_websocket_accept_header, op_http_ws_create_from_stream_resource } = core.ops;
  const { ArrayPrototypeIncludes, ArrayPrototypeMap, ArrayPrototypePush, StringPrototypeCharCodeAt, StringPrototypeSplit, StringPrototypeToLowerCase, StringPrototypeToUpperCase, TypeError, Symbol, Promise, Uint8Array } = primordials;
  const { toInnerRequest } = core.loadExtScript("ext:deno_fetch/23_request.js");
  const { fromInnerResponse, newInnerResponse } = core.loadExtScript("ext:deno_fetch/23_response.js");
  ...
  function upgradeWebSocket(request, options = {
    ...
    const { _eventLoop, _idleTimeoutDuration, _idleTimeoutTimeout, _readyState, _rid, _role, _serverHandleIdleTimeout, createWebSocketBranded, installServerInspector, SERVER, WebSocket } = loadWebSocket();
    ...
    if (inner._wantsUpgrade) {
      ...
      (async ()=>{
        try {
          ...
          if (socket[_idleTimeoutDuration]) {
            socket.addEventListener("close", ()=>clearTimeout(socket[_idleTimeoutTimeout]));
          }
      ...
      if (nodeSocket.destroyed) {
        throw new TypeError("Socket is already destroyed - cannot upgrade to WebSocket");
      }
      ...
      if (typeof handle.takeStream !== "function") {
        throw new TypeError("Socket is not a TCP socket - only TCP connections can be upgraded to WebSocket");
      }
      ...
      (async ()=>{
        try {
          ...
          const wsRid = op_http_ws_create_from_stream_resource(streamRid, extraBytes);
 ...
 */ function buildCaseInsensitiveCommaValueFinder(checkText) {
    const charCodes = ArrayPrototypeMap(StringPrototypeSplit(StringPrototypeToLowerCase(checkText), ""), (c)=>[
        StringPrototypeCharCodeAt(c, 0),
        StringPrototypeCharCodeAt(StringPrototypeToUpperCase(c), 0)
      ]);
    /** @type {number} */ let i;
    /** @type {number} */ let char;
    /** @param {string} value */ return function(value) {
    ...
    /** @param value {string} */ function hasWord(value) {
    ...
    /** @param value {string} */ function skipWhitespace(value) {
      while(char === spaceCharCode || char === tabCharCode){
        char = StringPrototypeCharCodeAt(value, ++i);
      }
    }
    /** @param value {string} */ function skipUntilComma(value) {
  ...
  internals.buildCaseInsensitiveCommaValueFinder = buildCaseInsensitiveCommaValueFinder;
  return {
    _ws,
    upgradeWebSocket
  };
