"use strict"; return ((function() {
  ...
  const { ArrayIsArray, ArrayPrototypeIndexOf, ArrayPrototypePush, ArrayPrototypeShift, ArrayPrototypeSlice, ArrayPrototypeSplice, ArrayPrototypeUnshift, Error, FunctionPrototypeApply, FunctionPrototypeCall, JSONStringify, Number, ObjectAssign, ObjectDefineProperty, ObjectPrototypeIsPrototypeOf, ObjectSetPrototypeOf, Promise, RegExpPrototypeExec, SafeRegExp, StringPrototypeCharCodeAt, StringPrototypeIndexOf, StringPrototypeSplit, SymbolAsyncDispose } = primordials;
  ...
  const lazyAddressOverride = core.createLazyLoader("ext:deno_node/internal/http/address_override.js");
  const { nextTick } = core.loadExtScript("ext:deno_node/_next_tick.ts");
  const { ERR_INVALID_URL, ERR_PROXY_TUNNEL } = core.loadExtScript("ext:deno_node/internal/errors.ts");
  const { httpServerPreClose, setupConnectionsTracking, storeHTTPOptions } = core.createLazyLoader("node:_http_server")();
  ...
  const net = lazyNet();
  const http = lazyHttp();
  const { applyAddressOverride, notifyAddressOverrideServing, startOverrideListener } = lazyAddressOverride();
  ...
  /** Makes a GET request to an https server. */ function get(...args) {
  ...
  function getPfxAgentKey(pfx, passphrase) {
    ...
    for(let i = 0; i < pfx.length; i++){
      const value1 = pfx[i];
      const raw = value1?.buf || value1;
      const pass = value1?.passphrase || passphrase;
      key += `:${raw}:${pass}`;
    }
    return key;
  }
  ...
  function openCONNECTTunnel(agent, options, cb) {
    const proxy = options._proxy;
    const targetHost = options._proxyTargetHost;
    const targetPort = options._proxyTargetPort;
    const hostHeader = StringPrototypeIndexOf(targetHost, ":") !== -1 && StringPrototypeCharCodeAt(targetHost, 0) !== 91 ? `[${targetHost}]:${targetPort}` : `${targetHost}:${targetPort}`;
    let tunnelSocket;
    const timeout = options.timeout || agent.options && agent.options.timeout || 0;
    // Initial TCP/TLS connection to the proxy itself.
    const proxyConnectOpts = {
      __proto__: null,
      host: proxy.hostname,
      port: proxy.port
    };
    ...
    if (timeout > 0) {
      timeoutId = setTimeout(()=>{
        ...
        const err = new Error(`Request timed out: Tunneling socket connection timed out after ${timeout}ms`);
        err.code = "ETIMEDOUT";
        fail(err);
      }, timeout);
    }
    ...
    function failUnexpectedEnd() {
      ...
      fail(new ERR_PROXY_TUNNEL("Connection to establish proxy tunnel ended unexpectedly"));
    }
    function onProxyReady() {
      ...
      function onData(chunk) {
        ...
        if (!m) {
          fail(new ERR_PROXY_TUNNEL(`Failed to establish tunnel to ${hostHeader} over ${proxy.protocol}//${proxy.hostname}:${proxy.port}: ${statusLine}`));
          return;
        }
        ...
        if (statusCode < 200 || statusCode >= 300) {
          ...
          const err = new ERR_PROXY_TUNNEL(`Failed to establish tunnel to ${hostHeader} over ${proxy.protocol}//${proxy.hostname}:${proxy.port}: ${statusLine}`);
        ...
        const tlsOpts = {
          ...
          host: targetHost,
          port: targetPort,
          servername: options.servername || (lazyNet().default.isIP(targetHost) ? undefined : targetHost)
        };
  ...
  /** Makes a request to an https server. */ function request(...args) {
    let options = {};
