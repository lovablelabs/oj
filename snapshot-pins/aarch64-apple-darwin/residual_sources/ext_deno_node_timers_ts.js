"use strict"; return ((function() {
  ...
  const { FunctionPrototypeBind, ObjectCreate, ObjectDefineProperty, Promise, PromiseReject, PromiseWithResolvers, SafeArrayIterator, SafePromisePrototypeFinally, SymbolFor } = primordials;
  ...
  const { getActiveTimer, Immediate, kDestroy, Timeout } = core.loadExtScript("ext:deno_node/internal/timers.mjs");
  const { validateAbortSignal, validateBoolean, validateFunction, validateNumber, validateObject } = core.loadExtScript("ext:deno_node/internal/validators.mjs");
  const { kEmptyObject, promisify } = core.loadExtScript("ext:deno_node/internal/util.mjs");
  const { AbortError, ERR_ILLEGAL_CONSTRUCTOR, ERR_INVALID_THIS } = core.loadExtScript("ext:deno_node/internal/errors.ts");
  const lazyEventTarget = core.createLazyLoader("ext:deno_node/internal/event_target.mjs");
  ...
  function setTimeoutPromise(after, value, options = kEmptyObject) {
    try {
    ...
    const timeout = mockTimers !== null && mockTimers._apiEnabled("setTimeout") ? mockTimers._setTimeout(resolve, after, [
      value
    ], false) : new Timeout(resolve, after, [
      value
    ], false, ref);
    if (signal) {
  ...
  function setImmediatePromise(value, options = kEmptyObject) {
    try {
      validateObject(options, "options");
      if (typeof options?.signal !== "undefined") {
        validateAbortSignal(options.signal, "options.signal");
    ...
    let oncancel;
    const { promise, resolve, reject } = PromiseWithResolvers();
    const immediate = mockTimers !== null && mockTimers._apiEnabled("setImmediate") ? mockTimers._setTimeout(()=>resolve(value), 0, [], true) : new Immediate(()=>resolve(value));
    if (!ref) {
      immediate.unref();
    }
    if (signal) {
  ...
  async function* setIntervalAsync(after, value, options = {
    __proto__: null
  }) {
    validateObject(options, "options");
    if (typeof options?.signal !== "undefined") {
    ...
    try {
      ...
      interval = mockTimers !== null && mockTimers._apiEnabled("setInterval") ? mockTimers._setInterval(onInterval, after, []) : new Timeout(onInterval, after, [], true, ref);
      if (signal) {
        onCancel = ()=>{
  ...
  function validateScheduler(self) {
    if (self !== scheduler) {
