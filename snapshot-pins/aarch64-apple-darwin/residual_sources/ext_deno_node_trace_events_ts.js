"use strict"; return ((function() {
  ...
  const { ArrayFrom, ArrayIsArray, ArrayPrototypeJoin, ArrayPrototypePush, ArrayPrototypeSlice, ArrayPrototypeUnshift, JSONParse, JSONStringify, MathTrunc, Number, NumberPrototypeToString, ReflectApply, SafeArrayIterator, SafeMap, SafeSet, SafeSetIterator, StringFromCharCode, StringPrototypeEndsWith, StringPrototypeStartsWith, Symbol, Uint8Array } = primordials;
  const { ERR_TRACE_EVENTS_CATEGORY_REQUIRED } = core.loadExtScript("ext:deno_node/internal/errors.ts");
  const { validateObject, validateStringArray } = core.loadExtScript("ext:deno_node/internal/validators.mjs");
  const lazyBindingMod = core.createLazyLoader("ext:deno_node/internal_binding/mod.ts");
  function getProc() {
    ...
    enable() {
      if (!this[kEnabled]) {
        ...
        if (enabledTracingObjects.size > kMaxTracingCount) {
          const p = getProc();
          if (p && p.emitWarning) {
            p.emitWarning("Possible trace_events memory leak detected. There are more than " + `${kMaxTracingCount} enabled Tracing objects.`);
          }
        }
  ...
  function writeMainTraceFile(pid) {
    ...
    for (const entryName of new SafeArrayIterator(entries)){
      ...
      if (!StringPrototypeStartsWith(entryName, prefix) || !StringPrototypeEndsWith(entryName, ".json")) {
        continue;
      }
      try {
  ...
  function writeWorkerSliceFile(pid) {
    ...
    try {
      getFs().writeFileSync(filename, JSONStringify({
        traceEvents: recordedEvents
      }));
    } catch  {
    // Best-effort exit-time write.
    }
  ...
  function installAsyncHooksTimerTracing() {
    ...
    globalThis.setTimeout = function(cb, ms, ...args) {
      ...
      const wrapped = function() {
        try {
          ...
          trace(PHASE_NESTABLE_ASYNC_END, "node,node.async_hooks", "Timeout", id);
        }
      };
      const callArgs = ArrayPrototypeSlice(args);
    ...
    if (typeof originalSetImmediate === "function") {
      globalThis.setImmediate = function(cb, ...args) {
        ...
        const id = ++traceIdCounter;
        trace(PHASE_NESTABLE_ASYNC_BEGIN, "node,node.async_hooks", "Immediate", id);
        const wrapped = function() {
          try {
            return ReflectApply(cb, this, arguments);
          } finally{
            trace(PHASE_NESTABLE_ASYNC_END, "node,node.async_hooks", "Immediate", id);
          }
        };
        const callArgs = ArrayPrototypeSlice(args);
