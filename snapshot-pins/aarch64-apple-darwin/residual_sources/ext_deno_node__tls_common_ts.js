"use strict"; return ((function() {
  ...
  const { ArrayBufferPrototype, ArrayIsArray, ArrayPrototypeFilter, ArrayPrototypeJoin, ArrayPrototypeMap, ArrayPrototypePush, Boolean, DataViewPrototypeGetBuffer, DataViewPrototypeGetByteLength, DataViewPrototypeGetByteOffset, Error, JSONParse, NumberIsNaN, NumberParseInt, ObjectDefineProperty, ObjectHasOwn, ObjectPrototypeIsPrototypeOf, RegExpPrototypeExec, RegExpPrototypeTest, SafeArrayIterator, SafeRegExp, SafeSet, SafeWeakSet, SetPrototypeHas, String, StringPrototypeCharCodeAt, StringPrototypeIncludes, StringPrototypeReplace, StringPrototypeSplit, StringPrototypeStartsWith, TypedArrayPrototypeGetBuffer, TypedArrayPrototypeGetByteLength, TypedArrayPrototypeGetByteOffset, TypeError, Uint8Array, WeakSetPrototypeHas } = primordials;
  const { ERR_CRYPTO_CUSTOM_ENGINE_NOT_SUPPORTED, ERR_INVALID_ARG_TYPE, ERR_TLS_INVALID_PROTOCOL_METHOD, ERR_TLS_INVALID_PROTOCOL_VERSION, ERR_TLS_PROTOCOL_VERSION_CONFLICT } = core.loadExtScript("ext:deno_node/internal/errors.ts");
  const { getOptionValue } = core.loadExtScript("ext:deno_node/internal/options.ts");
  const { isArrayBufferView, isTypedArray } = core.loadExtScript("ext:deno_node/internal/util/types.ts");
  const { validateString } = core.loadExtScript("ext:deno_node/internal/validators.mjs");
  const { op_node_validate_crl, op_node_load_pfx } = core.ops;
  const { createPrivateKey } = core.loadExtScript("ext:deno_node/internal/crypto/keys.ts");
  ...
  const CIPHER_NAME_RE = new SafeRegExp("^[!+\\-@]?[A-Z0-9][A-Z0-9_=\\-]*(?:@[A-Z0-9_=\\-]+)?$");
  ...
  function validateCipherList(ciphers) {
    ...
    for (const entry of new SafeArrayIterator(entries)){
      ...
      const normalized = StringPrototypeReplace(entry, new SafeRegExp("^[!+\\-]"), "");
      const name = StringPrototypeSplit(normalized, "@", 1)[0];
      if (StringPrototypeStartsWith(normalized, "@SECLEVEL=") || SetPrototypeHas(CIPHER_META_NAMES, name) || StringPrototypeStartsWith(name, "TLS_") || StringPrototypeIncludes(name, "-")) {
        hasValidEntry = true;
        break;
      }
    }
  ...
  function parseSecLevel(ciphers) {
  ...
  function rsaMinBitsForSecLevel(level) {
    if (level <= 0) return 0;
  ...
  function ecMinBitsForSecLevel(level) {
    if (level <= 0) return 0;
  ...
  function throwKeyTooSmall() {
    const err = new Error("error:0A00018A:SSL routines::key too small");
  ...
  // reject them).
  function validateKeyStrength(options) {
    if (!options.key) return;
    const level = parseSecLevel(options.ciphers);
    if (level <= 0) return;
    const keys = ArrayIsArray(options.key) ? options.key : [
      options.key
    ];
    for (const k of new SafeArrayIterator(keys)){
      ...
      if (typeof k === "object" && !isArrayBufferView(k) && !ObjectPrototypeIsPrototypeOf(ArrayBufferPrototype, k)) {
        keyData = k.pem ?? k.key;
        passphrase = k.passphrase ?? passphrase;
      }
      if (!keyData) continue;
      let keyObject;
      try {
        keyObject = passphrase != null ? createPrivateKey({
          key: keyData,
          passphrase
  ...
  const kValidVersions = new SafeSet([
    "TLSv1",
    "TLSv1.1",
    "TLSv1.2",
    "TLSv1.3"
  ...
  function toStringOrUndefined(val) {
    ...
    if (isArrayBufferView(val) || ObjectPrototypeIsPrototypeOf(ArrayBufferPrototype, val)) {
      return new TextDecoder().decode(val);
    }
    return `${val}`;
  }
  function normalizeCertValue(val) {
    if (val == null) return undefined;
    if (ArrayIsArray(val)) {
      return ArrayPrototypeFilter(ArrayPrototypeMap(val, (v)=>toStringOrUndefined(v)), Boolean);
    }
    return toStringOrUndefined(val);
  }
  function getProtocolRange(options) {
    ...
    if (options.secureProtocol) {
      ...
      if (options.minVersion || options.maxVersion) {
        throw new ERR_TLS_PROTOCOL_VERSION_CONFLICT(options.minVersion || options.maxVersion, "secureProtocol");
      }
      const range = kProtocolMap[options.secureProtocol];
      if (!range) {
        if (options.secureProtocol === "SSLv2_method" || options.secureProtocol === "SSLv2_client_method" || options.secureProtocol === "SSLv2_server_method") {
          throw new ERR_TLS_INVALID_PROTOCOL_METHOD("SSLv2 methods disabled");
        }
        if (options.secureProtocol === "SSLv3_method" || options.secureProtocol === "SSLv3_client_method" || options.secureProtocol === "SSLv3_server_method") {
          throw new ERR_TLS_INVALID_PROTOCOL_METHOD("SSLv3 methods disabled");
        }
        throw new ERR_TLS_INVALID_PROTOCOL_METHOD(`Unknown method: ${options.secureProtocol}`);
      }
      minVersion = range[0];
      maxVersion = range[1];
    } else {
      if (options.minVersion) {
        if (!SetPrototypeHas(kValidVersions, options.minVersion)) {
          throw new ERR_TLS_INVALID_PROTOCOL_VERSION(options.minVersion, "minVersion");
        }
        minVersion = options.minVersion;
      }
      if (options.maxVersion) {
        if (!SetPrototypeHas(kValidVersions, options.maxVersion)) {
          throw new ERR_TLS_INVALID_PROTOCOL_VERSION(options.maxVersion, "maxVersion");
        }
  ...
  function isValidKeyCertValue(val) {
    return typeof val === "string" || isArrayBufferView(val) || ObjectPrototypeIsPrototypeOf(ArrayBufferPrototype, val);
  }
  function validateKeyCertOption(val, name, allowKeyObjects) {
    ...
    if (ArrayIsArray(val)) {
      for(let i = 0; i < val.length; i++){
        ...
        if (allowKeyObjects && typeof item === "object" && item !== null) continue;
  ...
  function toUint8Array(val) {
    ...
    if (isArrayBufferView(val)) {
      const buffer = isTypedArray(val) ? TypedArrayPrototypeGetBuffer(val) : DataViewPrototypeGetBuffer(val);
      const byteOffset = isTypedArray(val) ? TypedArrayPrototypeGetByteOffset(val) : DataViewPrototypeGetByteOffset(val);
      const byteLength = isTypedArray(val) ? TypedArrayPrototypeGetByteLength(val) : DataViewPrototypeGetByteLength(val);
  ...
  // concatenating.
  function normalizeKeyPem(val, defaultPassphrase) {
    if (val == null) return undefined;
    const items = ArrayIsArray(val) ? val : [
      val
    ];
    const parts = [];
    for (const item of new SafeArrayIterator(items)){
      ...
      if (typeof item === "object" && !isArrayBufferView(item) && !ObjectPrototypeIsPrototypeOf(ArrayBufferPrototype, item)) {
        pem = toStringOrUndefined(item.pem ?? item.key);
  ...
  const secureContextBrand = new SafeWeakSet();
  class SecureContext {
    context;
    constructor(options = {
      __proto__: null
      ...
      if (options.clientCertEngine != null) {
        ...
        const err = new Error(`error:25066067:DSO support routines:dlfcn_load:could not load the shared library`);
      ...
      if (options.privateKeyIdentifier != null) {
        validateString(options.privateKeyIdentifier, "options.privateKeyIdentifier");
      }
      ...
      let pfxCa;
      if (options.pfx != null) {
        const pfxItems = ArrayIsArray(options.pfx) ? options.pfx : [
          options.pfx
        ];
        for (const item of new SafeArrayIterator(pfxItems)){
          ...
          let buf = item;
          let passphrase = options.passphrase;
          if (typeof item === "object" && !isArrayBufferView(item) && !ObjectPrototypeIsPrototypeOf(ArrayBufferPrototype, item) && (item.buf !== undefined || item.passphrase !== undefined)) {
            buf = item.buf;
            if (item.passphrase != null) {
              passphrase = item.passphrase;
            }
      ...
      // Validate CRL data.
      if (options.crl != null) {
        const crls = ArrayIsArray(options.crl) ? options.crl : [
          options.crl
        ];
      ...
      this.context.setOptions = function setOptions(_options) {
        if (!WeakSetPrototypeHas(secureContextBrand, this)) {
          throw new TypeError("Illegal invocation");
        }
      };
      this.context.addCACert = function addCACert(cert) {
        if (!WeakSetPrototypeHas(secureContextBrand, this)) {
          throw new TypeError("Illegal invocation");
        }
  ...
  function translatePeerCertificate(c) {
    ...
    if (typeof c.infoAccess === "string") {
      ...
      StringPrototypeReplace(info, new SafeRegExp("([^\\n:]*):([^\\n]*)(?:\\n|$)", "g"), (_all, key, value)=>{
        const normalized = StringPrototypeCharCodeAt(value, 0) === 0x22 ? JSONParse(value) : value;
