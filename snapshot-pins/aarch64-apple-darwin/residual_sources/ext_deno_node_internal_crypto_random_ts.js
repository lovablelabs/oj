"use strict"; return ((function() {
  ...
  const { Array, ArrayBufferPrototypeGetByteLength, BigInt, BigIntPrototypeToString, DataView, DataViewPrototypeGetBuffer, DataViewPrototypeGetByteLength, DataViewPrototypeGetByteOffset, DataViewPrototypeGetUint8, MathClz32, NumberParseInt, ObjectPrototypeIsPrototypeOf, PromisePrototypeCatch, PromisePrototypeThen, SafeArrayIterator, StringFromCharCode, StringPrototypePadStart, StringPrototypeSubstring, TypedArrayPrototypeGetBuffer, TypedArrayPrototypeGetByteLength, TypedArrayPrototypeGetByteOffset, Uint8Array, Uint8ArrayPrototype } = primordials;
  const { op_node_check_prime_bytes, op_node_check_prime_bytes_async, op_node_gen_prime, op_node_gen_prime_async } = core.ops;
  const { default: randomBytes } = core.loadExtScript("ext:deno_node/internal/crypto/_randomBytes.ts");
  const { default: randomFill, randomFillSync } = core.loadExtScript("ext:deno_node/internal/crypto/_randomFill.mjs");
  const { default: randomInt } = core.loadExtScript("ext:deno_node/internal/crypto/_randomInt.ts");
  const { validateBoolean, validateFunction, validateInt32, validateObject } = core.loadExtScript("ext:deno_node/internal/validators.mjs");
  const { isAnyArrayBuffer, isArrayBufferView, isTypedArray } = core.loadExtScript("ext:deno_node/internal/util/types.ts");
  const { ERR_INVALID_ARG_TYPE, ERR_OUT_OF_RANGE, NodeError, NodeRangeError } = core.loadExtScript("ext:deno_node/internal/errors.ts");
  ...
  function checkPrime(candidate, options = {
    ...
    if (typeof candidate === "bigint") {
      ...
      const byteLength = isArrayBufferView(candidate) ? arrayBufferViewByteLength(candidate) : ArrayBufferPrototypeGetByteLength(candidate);
      if (byteLength > OPENSSL_BIGNUM_MAX_BYTES) {
        throw new NodeError("ERR_OSSL_BN_BIGNUM_TOO_LONG", "bignum too long");
      }
    ...
    PromisePrototypeCatch(PromisePrototypeThen(op_node_check_prime_bytes_async(candidateBytes, checks), (result)=>{
  ...
  function generatePrime(size, options = {
    __proto__: null
  }, callback) {
    ...
    validateFunction(callback, "callback");
    const { bigint, safe, add, rem } = validateRandomPrimeJob(size, options);
    PromisePrototypeThen(op_node_gen_prime_async(size, safe, add ?? null, rem ?? null), (prime)=>{
  ...
  function arrayBufferViewByteLength(view) {
    return isTypedArray(view) ? TypedArrayPrototypeGetByteLength(view) : DataViewPrototypeGetByteLength(view);
  }
  function toUint8Array(value) {
    ...
    if (isArrayBufferView(value)) {
      const buffer = isTypedArray(value) ? TypedArrayPrototypeGetBuffer(value) : DataViewPrototypeGetBuffer(value);
      const byteOffset = isTypedArray(value) ? TypedArrayPrototypeGetByteOffset(value) : DataViewPrototypeGetByteOffset(value);
      return new Uint8Array(buffer, byteOffset, arrayBufferViewByteLength(value));
    }
    return new Uint8Array(value);
  }
  function validateRandomPrimeJob(size, options) {
    validateInt32(size, "size", 1);
    validateObject(options, "options");
    let { safe = false, bigint = false, add, rem } = options;
  ...
  function bigintToBytes(n) {
    ...
    for(let i = 0; i < bytes.length; i++){
      bytes[i] = NumberParseInt(StringPrototypeSubstring(padded, i * 2, i * 2 + 2), 16);
    }
    return bytes;
  }
  ...
  function arrayBufferToUnsignedBigInt(buf) {
    ...
    const chars = new Array(length * 2);
