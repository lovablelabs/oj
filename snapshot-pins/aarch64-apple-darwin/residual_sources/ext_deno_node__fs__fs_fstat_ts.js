"use strict"; return ((function() {
  ...
  const { Date, Error, Promise, PromisePrototypeThen } = primordials;
  const lazyStatUtils = core.createLazyLoader("ext:deno_node/internal/fs/stat_utils.ts");
  ...
  function fstat(fd, optionsOrCallback, maybeCallback) {
    ...
    PromisePrototypeThen(op_node_fs_fstat(fd), (stat)=>callback(null, lazyStatUtils().CFISBIS(nodeFsStatToFileInfo(stat), options.bigint)), (err)=>callback(denoErrorToNodeError(err, {
        syscall: "fstat"
      })));
  }
  function fstatSync(fd, options) {
    ...
    try {
      ...
      return lazyStatUtils().CFISBIS(nodeFsStatToFileInfo(stat), options?.bigint || false);
    } catch (err) {
  ...
  function fstatPromise(fd, options) {
