...
function createRepl(env, opts, cb) {
  ...
  if (env && env.NODE_REPL_MODE) {
    ...
    opts.replMode = lazyREPL().default[mode === "strict" ? "REPL_MODE_STRICT" : "REPL_MODE_SLOPPY"];
  }
  if (opts.replMode === undefined) {
    opts.replMode = lazyREPL().default.REPL_MODE_SLOPPY;
  }
  ...
  const term = "terminal" in opts ? opts.terminal : lazyProcess().default.stdout?.isTTY;
  opts.filePath = term ? env?.NODE_REPL_HISTORY : "";
  const repl = lazyREPL().default.start(opts);
...
const exported = Object.create(lazyREPL().default);
