"use strict"; return ((function() {
  ...
  const { Error, ObjectGetOwnPropertyNames, ObjectHasOwn, String, ReflectOwnKeys, ArrayPrototypeIncludes, Object, Proxy, ObjectPrototype, ObjectPrototypeIsPrototypeOf, ReflectDefineProperty, ReflectHas, TypeError, TypeErrorPrototype } = primordials;
  const { build, createLazyLoader } = core;
  const { nextTick: _nextTick } = core.loadExtScript("ext:deno_node/_next_tick.ts");
  const { _exiting } = core.loadExtScript("ext:deno_node/_process/exiting.ts");
  const fs = core.loadExtScript("ext:deno_fs/30_fs.js");
  const { denoErrorToNodeError, ERR_INVALID_ARG_TYPE, ERR_INVALID_OBJECT_DEFINE_PROPERTY } = core.loadExtScript("ext:deno_node/internal/errors.ts");
  const loadProcess = createLazyLoader("node:process");
  let nodeProcess;
  /** Returns the operating system CPU architecture for which the Deno binary was compiled */ function arch() {
  ...
  /** https://nodejs.org/api/process.html#process_process_chdir_directory */ function chdir(directory) {
  ...
  /** https://nodejs.org/api/process.html#process_process_cwd */ const cwd = fs.cwd;
  /** https://nodejs.org/api/process.html#process_process_nexttick_callback_args */ const nextTick = _nextTick;
 ...
 */ const env = new Proxy(Object(), {
    ...
    set (_target, prop, value) {
      ...
      if (typeof value !== "string") {
        ...
        nodeProcess.emitWarning("Assigning any value other than a string, number, or boolean to a " + "process.env property is deprecated. Please make sure to convert the value " + "to a string before setting process.env with it.", "DeprecationWarning", "DEP0104");
      }
    ...
    defineProperty (target, property, attributes) {
      if (ObjectHasOwn(attributes, "get") || ObjectHasOwn(attributes, "set")) {
        throw new ERR_INVALID_OBJECT_DEFINE_PROPERTY("'process.env' does not accept an " + "accessor(getter/setter) descriptor");
      }
      if (!attributes?.configurable || !attributes?.enumerable || !attributes?.writable) {
        throw new ERR_INVALID_OBJECT_DEFINE_PROPERTY("'process.env' only accepts a " + "configurable, writable," + " and enumerable data descriptor");
      }
 ...
 *
 * The `26.5.1` token is substituted at snapshot build time with
