"use strict"; return ((function() {
  ...
  const { op_base64url_decode } = core.ops;
  const { forgivingBase64Decode } = core.loadExtScript("ext:deno_web/00_infra.js");
  const { DataView, DataViewPrototypeSetUint16, Error, Int8Array, MathMin, NumberPOSITIVE_INFINITY, NumberPrototypeToString, SafeRegExp, StringPrototypeCharCodeAt, StringPrototypeIndexOf, StringPrototypeReplace, StringPrototypeReplaceAll, StringPrototypeSubstring, StringPrototypeTrim, StringPrototypeTrimStart, TypedArrayPrototypeGetBuffer, TypedArrayPrototypeGetLength, TypedArrayPrototypeSubarray, Uint8Array } = primordials;
  ...
  function base64clean(str) {
    ...
    str = eqIndex !== -1 ? StringPrototypeTrimStart(StringPrototypeSubstring(str, 0, eqIndex)) : StringPrototypeTrim(str);
  ...
  function hexToBytes(str) {
    ...
    return i === length ? byteArray : TypedArrayPrototypeSubarray(byteArray, 0, i);
  }
  ...
  function utf16leToBytes(str, units) {
    ...
    const length = MathMin(str.length * 2, units ? (units >>> 1) * 2 : NumberPOSITIVE_INFINITY);
    const byteArray = new Uint8Array(length);
    const view = new DataView(TypedArrayPrototypeGetBuffer(byteArray));
    let i;
    for(i = 0; i * 2 < length; i++){
      DataViewPrototypeSetUint16(view, i * 2, StringPrototypeCharCodeAt(str, i), true);
    }
    // Returning a buffer subarray is okay: This API's return value
    // is never exposed to users and is only ever used for its length
    // and the data within the subarray.
    return i * 2 === length ? byteArray : TypedArrayPrototypeSubarray(byteArray, 0, i * 2);
  }
  return {
    asciiToBytes,
    base64CleanToBytes,
