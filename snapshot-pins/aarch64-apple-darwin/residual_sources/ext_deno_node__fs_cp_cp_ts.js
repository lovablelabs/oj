"use strict"; return ((function() {
  ...
  const lazyFsPromises = core.createLazyLoader("ext:deno_node/internal/fs/promises.ts");
  const lazyConstants = core.createLazyLoader("node:constants");
  const { denoErrorToNodeError, ERR_FS_CP_DIR_TO_NON_DIR, ERR_FS_CP_EEXIST, ERR_FS_CP_EINVAL, ERR_FS_CP_FIFO_PIPE, ERR_FS_CP_NON_DIR_TO_DIR, ERR_FS_CP_SOCKET, ERR_FS_CP_SYMLINK_TO_SUBDIRECTORY, ERR_FS_CP_UNKNOWN, ERR_FS_EISDIR } = core.loadExtScript("ext:deno_node/internal/errors.ts");
  const { op_node_cp_check_paths_recursive, op_node_cp_fast, op_node_cp_on_file, op_node_cp_on_link, op_node_cp_validate_and_prepare } = core.ops;
  /**
  Layout (bigint):
...
*/ const CpEntryFlags = {
  ...
  const { Number, PromiseResolve, SymbolAsyncIterator } = primordials;
  ...
  async function cpFn(src, dest, opts) {
    try {
      ...
      const statInfo = await op_node_cp_validate_and_prepare(src, dest, opts.dereference);
  ...
  function canUseNativeFastPath(opts) {
    return opts.filter === undefined && opts.dereference === false && opts.force === true && opts.errorOnExist === false && opts.preserveTimestamps === false && opts.verbatimSymlinks === false && opts.mode === 0;
  }
  function getStatsForCopy(statInfo, src, dest, opts) {
    ...
    } else if (statInfo & CpEntryFlags.IsSrcFile || statInfo & CpEntryFlags.IsSrcCharDevice || statInfo & CpEntryFlags.IsSrcBlockDevice) {
  ...
  async function onFile(statInfo, src, dest, opts) {
    const srcMode = Number(statInfo & CpEntryFlags.SrcModeMask);
    const isDestExists = !!(statInfo & CpEntryFlags.IsDestExists);
    await op_node_cp_on_file(src, dest, srcMode, isDestExists, opts.force, opts.errorOnExist, opts.preserveTimestamps, opts.mode ?? 0);
  }
  function setDestMode(dest, srcMode) {
    if (!srcMode) return PromiseResolve();
    return Deno.chmod(dest, srcMode);
  }
  function onDir(statInfo, src, dest, opts) {
  ...
  async function copyDir(src, dest, opts) {
    ...
    while(true){
      ...
      const statInfo = await op_node_cp_check_paths_recursive(srcItem, destItem, opts.dereference);
      await getStatsForCopy(statInfo, srcItem, destItem, opts);
    }
  }
  async function onLink(destExists, src, dest, opts) {
    await op_node_cp_on_link(src, dest, destExists, opts.verbatimSymlinks);
  }
  return {
    throwCpError,
    cpFn
