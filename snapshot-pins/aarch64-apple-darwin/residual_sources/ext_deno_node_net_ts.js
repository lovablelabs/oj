"use strict"; return ((function() {
  ...
  const { BlockList, SocketAddress } = core.loadExtScript("ext:deno_node/internal/blocklist.mjs");
  const { EventEmitter } = core.loadExtScript("ext:deno_node/_events.mjs");
  const { isIP, isIPv4, isIPv6, kReinitializeHandle, kSetKeepAlive, kSetKeepAliveInitialDelay, kSetNoDelay, normalizedArgsSymbol } = core.loadExtScript("ext:deno_node/internal/net.ts");
  const { Duplex } = core.createLazyLoader("node:stream")();
  const { asyncIdSymbol, defaultTriggerAsyncIdScope, emitDestroy, emitInit, executionAsyncId, newAsyncId, ownerSymbol } = core.loadExtScript("ext:deno_node/internal/async_hooks.ts");
  const { AbortError, ERR_INVALID_ADDRESS_FAMILY, ERR_INVALID_ARG_TYPE, ERR_INVALID_ARG_VALUE, ERR_INVALID_FD_TYPE, ERR_INVALID_IP_ADDRESS, ERR_IP_BLOCKED, ERR_MISSING_ARGS, ERR_SERVER_ALREADY_LISTEN, ERR_SERVER_NOT_RUNNING, ERR_SOCKET_CLOSED, ERR_SOCKET_CLOSED_BEFORE_CONNECTION, ERR_SOCKET_CONNECTION_TIMEOUT, errnoException, exceptionWithHostPort, genericNodeError, NodeAggregateError, uvExceptionWithHostPort } = core.loadExtScript("ext:deno_node/internal/errors.ts");
  const { kAfterAsyncWrite, kBuffer, kBufferCb, kBufferGen, kHandle, kUpdateTimer, onStreamRead, setStreamTimeout, writeGeneric, writevGeneric } = core.loadExtScript("ext:deno_node/internal/stream_base_commons.ts");
  const { kDestroy, kTimeout } = core.loadExtScript("ext:deno_node/internal/timers.mjs");
  const { nextTick } = core.loadExtScript("ext:deno_node/_next_tick.ts");
  const { DTRACE_NET_SERVER_CONNECTION, DTRACE_NET_STREAM_END } = core.loadExtScript("ext:deno_node/internal/dtrace.ts");
  const { Buffer } = core.loadExtScript("ext:deno_node/internal/buffer.mjs");
  const { constants: TCPConstants, setupListenWrap, TCP, TCPConnectWrap } = core.loadExtScript("ext:deno_node/internal_binding/tcp_wrap.ts");
  const { constants: PipeConstants, Pipe, PipeConnectWrap, setupListenWrap: setupPipeListenWrap } = core.loadExtScript("ext:deno_node/internal_binding/pipe_wrap.ts");
  const { ShutdownWrap } = core.loadExtScript("ext:deno_node/internal_binding/stream_wrap.ts");
  const { default: assert } = core.loadExtScript("ext:deno_node/assert.ts");
  const { isWindows } = core.loadExtScript("ext:deno_node/_util/os.ts");
  const { ADDRCONFIG, lookup: dnsLookup } = core.createLazyLoader("node:dns")().default;
  const { kPermTokenSink } = core.loadExtScript("ext:deno_node/internal_binding/cares_wrap.ts");
  const { codeMap, UV_ECANCELED, UV_ETIMEDOUT } = core.loadExtScript("ext:deno_node/internal_binding/uv.ts");
  ...
  const { channel, tracingChannel } = core.loadExtScript("ext:deno_node/diagnostics_channel.js");
  const { registerActiveHandle, unregisterActiveHandle } = core.loadExtScript("ext:deno_node/internal/process/active_resources.ts");
  ...
  const { isUint8Array } = core.loadExtScript("ext:deno_node/internal/util/types.ts");
  const { validateAbortSignal, validateBoolean, validateFunction, validateInt32, validateNumber, validatePort, validateString, validateStringWithoutNullBytes } = core.loadExtScript("ext:deno_node/internal/validators.mjs");
  const { ArrayIsArray, ArrayPrototypeIncludes, ArrayPrototypeIndexOf, ArrayPrototypePush, ArrayPrototypeSplice, Boolean, FunctionPrototypeBind, FunctionPrototypeCall, MathMax, Number, NumberIsNaN, NumberParseInt, ObjectDefineProperty, ObjectHasOwn, ObjectPrototypeIsPrototypeOf, ObjectSetPrototypeOf, Promise, PromiseWithResolvers, ReflectHas, SafeArrayIterator, StringPrototypeCharCodeAt, Symbol, SymbolAsyncDispose } = primordials;
  ...
  function _runInAsyncContext(snapshot, run) {
  ...
  function _emitHandleInit(handle, asyncId) {
    if (typeof handle.getProviderType !== "function") {
      return;
    }
    const providerType = handle.getProviderType();
    emitInit(asyncId, providerTypeNames[providerType] || "UNKNOWN", executionAsyncId(), handle);
  }
  ...
  function _afterConnect(status, handle, req, readable, writable) {
    _runInAsyncContext(handle?.[kAsyncContext], ()=>_afterConnectImpl(status, handle, req, readable, writable));
  }
  function _afterConnectImpl(status, handle, req, readable, writable) {
  ...
  function _afterConnectMultiple(context, current, status, handle, req, readable, writable) {
  ...
  function _internalConnect(socket, address, port, addressType, localAddress, localPort, flags = 0) {
    assert(socket.connecting);
    if (socket.blockList?.check(address, `ipv${addressType}`)) {
  ...
  function _internalConnectMultiple(context, canceled) {
    clearTimeout(context[kTimeout]);
    const self = context.socket;
    ...
    if (self.blockList?.check(address, `ipv${addressType}`)) {
      ArrayPrototypePush(context.errors, new ERR_IP_BLOCKED(address));
      _internalConnectMultiple(context);
      return;
    }
    ...
    const req = new TCPConnectWrap();
    req.oncomplete = FunctionPrototypeBind(_afterConnectMultiple, undefined, context, current);
    req.address = address;
    req.port = port;
    req.localAddress = localAddress;
    req.localPort = localPort;
    ...
    if (current < context.addresses.length - 1) {
      ...
      context[kTimeout] = setTimeout(_internalConnectMultipleTimeout, context.timeout, context, req, self._handle);
    }
  }
  ...
  function _writeAfterFIN(chunk, encoding, cb) {
    if (!this.writableEnded) {
      return FunctionPrototypeCall(Duplex.prototype.write, this, chunk, encoding, // @ts-expect-error Using `call` seem to be interfering with the overload for write
      cb);
    }
  ...
  function _lookupAndConnect(self, options) {
    ...
    if (dnsOpts.family !== 4 && dnsOpts.family !== 6 && !localAddress && autoSelectFamily) {
      ...
      defaultTriggerAsyncIdScope(self[asyncIdSymbol], function() {
        _lookupAndConnectMultiple(self, asyncIdSymbol, lookup, host, options, getLookupDnsOpts(), port, localAddress, localPort, autoSelectFamilyAttemptTimeout, usingDefaultLookup);
      });
      return;
    }
    defaultTriggerAsyncIdScope(self[asyncIdSymbol], function() {
      function emitLookup(err, ip, addressType, netPermToken) {
        ...
        if (usingDefaultLookup && netPermToken && self._handle?.setNetPermToken) {
  ...
  function _lookupAndConnectMultiple(self, asyncIdSymbol, lookup, host, options, dnsopts, port, localAddress, localPort, timeout, usingDefaultLookup) {
    defaultTriggerAsyncIdScope(self[asyncIdSymbol], function() {
      function emitLookup(err, addresses, _, netPermToken) {
        ...
        self._unrefTimer();
        defaultTriggerAsyncIdScope(self[asyncIdSymbol], _internalConnectMultiple, context);
      }
      // Tag the trusted callback so the built-in dns.lookup hands it the
      // NetPermToken; user-supplied lookups never receive it.
      if (usingDefaultLookup) {
  ...
  function _addClientAbortSignalOption(socket, signal) {
    if (signal.aborted) {
 ...
 */ function Socket(options) {
    ...
    if (options.handle) {
      ...
      this[asyncIdSymbol] = _getNewAsyncId(this._handle);
      _emitHandleInit(this._handle, this[asyncIdSymbol]);
      if ((fd === 1 || fd === 2) && ObjectPrototypeIsPrototypeOf(Pipe.prototype, this._handle) && isWindows) {
    ...
    if (onread !== null && typeof onread === "object") {
      ...
      if ((isUint8Array(onreadBuffer) || typeof onreadBuffer === "function") && typeof onread.callback === "function") {
  ...
  Socket.prototype.connect = function(...args) {
    ...
    if (pipe) {
      ...
      if (options.keepAlive !== undefined) {
        this.setKeepAlive(!!options.keepAlive, options.keepAliveInitialDelay);
      }
  ...
  Socket.prototype.setKeepAlive = function(enable, initialDelay) {
    ...
    if (newEnable !== this[kSetKeepAlive] || newDelay !== this[kSetKeepAliveInitialDelay]) {
      this[kSetKeepAlive] = newEnable;
      this[kSetKeepAliveInitialDelay] = newDelay;
      if (ReflectHas(this._handle, "setKeepAlive") && this._handle.setKeepAlive) {
  ...
  Socket.prototype.resetAndDestroy = function() {
    ...
    if (!this._handle || !ObjectPrototypeIsPrototypeOf(TCP.prototype, this._handle)) {
  ...
  ObjectDefineProperty(Socket.prototype, "bytesRead", {
    __proto__: null,
    get: function() {
      return this._handle ? this._handle.getBytesRead?.() ?? this._handle.bytesRead ?? 0 : this[kBytesRead];
    }
  });
  ObjectDefineProperty(Socket.prototype, "bytesWritten", {
    ...
    get: function() {
      ...
      for (const el of new SafeArrayIterator(writableBuffer)){
        bytes += ObjectPrototypeIsPrototypeOf(Buffer.prototype, el.chunk) ? el.chunk.length : Buffer.byteLength(el.chunk, el.encoding);
      }
      if (ArrayIsArray(data)) {
        for(let i = 0; i < data.length; i++){
          const chunk = data[i];
          if (data.allBuffers || ObjectPrototypeIsPrototypeOf(Buffer.prototype, chunk)) {
            bytes += chunk.length;
          } else {
  ...
  Socket.prototype._destroy = function(exception, cb) {
    ...
    if (this._handle) {
      ...
      this[kBytesRead] = this._handle.getBytesRead?.() ?? this._handle.bytesRead ?? 0;
      this[kBytesWritten] = this._handle.getBytesWritten?.() ?? this._handle.bytesWritten ?? 0;
    ...
    if (this._server) {
      ...
      if (server._emitCloseIfDrained) {
        if (!server._handle && server._connections === 0 && this._httpMessageDetached === true && !this._httpMessage) {
          defaultTriggerAsyncIdScope(server[asyncIdSymbol], nextTick, _emitCloseNT, server);
        } else {
  ...
  Socket.prototype._getpeername = function() {
    if (!this._handle || !ReflectHas(this._handle, "getpeername") || this.connecting) {
  ...
  ObjectDefineProperty(Socket.prototype, "_bytesDispatched", {
    __proto__: null,
    get: function() {
      return this._handle ? this._handle.getBytesWritten?.() ?? this._handle.bytesWritten ?? 0 : this[kBytesWritten];
    }
  });
  ObjectDefineProperty(Socket.prototype, "_handle", {
    ...
    set: function(v) {
      if (this[kHandle] && !v) {
        ...
        registerActiveHandle(this, ObjectPrototypeIsPrototypeOf(TCP.prototype, v) ? "TCPSocketWrap" : "PipeWrap");
      }
      this[kHandle] = v;
    }
  });
  ...
  /** https://docs.deno.com/api/node/net/#namespace_getdefaultautoselectfamily */ function getDefaultAutoSelectFamily() {
    return autoSelectFamilyDefault;
  }
  /** https://docs.deno.com/api/node/net/#namespace_setdefaultautoselectfamily */ function setDefaultAutoSelectFamily(value) {
    validateBoolean(value, "value");
    autoSelectFamilyDefault = value;
  }
  /** https://docs.deno.com/api/node/net/#namespace_getdefaultautoselectfamilyattempttimeout */ function getDefaultAutoSelectFamilyAttemptTimeout() {
    return autoSelectFamilyAttemptTimeoutDefault;
  }
  /** https://docs.deno.com/api/node/net/#namespace_setdefaultautoselectfamilyattempttimeout */ function setDefaultAutoSelectFamilyAttemptTimeout(value) {
  ...
  function _getFlags(ipv6Only, reusePort) {
    let flags = 0;
    if (ipv6Only === true) {
  ...
  function _lookupAndListen(server, port, address, backlog, exclusive, flags) {
    ...
    dnsLookup(address, {
      port
    }, function doListen(err, ip, addressType) {
  ...
  function _createServerHandle(address, port, addressType, fd, flags) {
    ...
    if (typeof fd === "number" && fd >= 0) {
      ...
      if (isWindows) {
        const instances = NumberParseInt(Deno.env.get("NODE_PENDING_PIPE_INSTANCES") ?? "");
    ...
    if (address || port || isTCP) {
      ...
      if (!address) {
        ...
        err = handle.bindWithFlags(address, port ?? 0, flags ?? 0);
      } else {
        // deno-lint-ignore deno-internal/prefer-primordials -- libuv handle.bind(), not Function.prototype.bind
  ...
  function _onconnection(err, clientHandle) {
    // deno-lint-ignore no-this-alias
    const handle = this;
    _runInAsyncContext(handle[kAsyncContext], ()=>FunctionPrototypeCall(_onconnectionImpl, handle, err, clientHandle));
  }
  function _onconnectionImpl(err, clientHandle) {
    // deno-lint-ignore no-this-alias
    const handle = this;
    ...
    if (self.blockList && clientHandle && typeof clientHandle.getpeername === "function") {
      const out = {};
      if (clientHandle.getpeername(out) === 0) {
        const { address, family } = out;
  ...
  function _serverWrapName(handle) {
    return ObjectPrototypeIsPrototypeOf(TCP.prototype, handle) ? "TCPServerWrap" : "PipeWrap";
  }
  function _setupListenHandle(address, port, addressType, backlog, fd, flags) {
    ...
    if (this._handle) {
      ...
      if (typeof rval === "number") {
        ...
        if (netServerListenChannel.hasSubscribers) {
          netServerListenChannel.error.publish({
            server: this,
            error
          });
        }
    ...
    if (netServerListenChannel.hasSubscribers) {
      netServerListenChannel.asyncEnd.publish({
        server: this
      });
    }
 ...
 */ Server.prototype.listen = function(...args) {
    ...
    if (netServerListenChannel.hasSubscribers) {
      netServerListenChannel.asyncStart.publish({
        server: this,
        options
      });
    }
    ...
    if (ObjectPrototypeIsPrototypeOf(TCP.prototype, options) || ObjectPrototypeIsPrototypeOf(Pipe.prototype, options)) {
    ...
    if (args.length === 0 || typeof args[0] === "function" || typeof options.port === "undefined" && ObjectHasOwn(options, "port") || options.port === null) {
      options.port = 0;
    }
    ...
    if (typeof options.port === "number" || typeof options.port === "string") {
      ...
      if (options.host) {
        ...
        _listenInCluster(this, null, options.port | 0, 4, backlog, undefined, options.exclusive, flags);
      }
      return this;
    }
    ...
    if (options.path && _isPipeName(options.path)) {
      ...
      if ((options.readableAll === true || options.writableAll === true) && StringPrototypeCharCodeAt(pipeName, 0) === 0) {
        throw new ERR_INVALID_ARG_VALUE("options", options, "can not set readableAll or writableAllt to true when path is abstract unix socket");
      }
