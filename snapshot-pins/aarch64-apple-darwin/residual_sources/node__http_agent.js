...
const { emitDestroy, emitInit, executionAsyncId, symbols } = core.loadExtScript("ext:deno_node/internal/async_hooks.ts");
const { async_id_symbol } = symbols;
const { once } = core.loadExtScript("ext:deno_node/internal/util.mjs");
const { validateNumber, validateOneOf, validateString } = core.loadExtScript("ext:deno_node/internal/validators.mjs");
const { ArrayPrototypeIncludes, ArrayPrototypeIndexOf, ArrayPrototypePop, ArrayPrototypePush, ArrayPrototypeShift, ArrayPrototypeSome, ArrayPrototypeSplice, FunctionPrototypeCall, NumberIsFinite, NumberParseInt, ObjectCreate, ObjectKeys, ObjectPrototypeIsPrototypeOf, ObjectSetPrototypeOf, ObjectValues, RegExpPrototypeExec, SafeRegExp, StringPrototypeIndexOf, StringPrototypeSlice, StringPrototypeSplit, StringPrototypeStartsWith, Symbol } = primordials;
...
export function Agent(options) {
  ...
  this.agentKeepAliveTimeoutBuffer = typeof this.options.agentKeepAliveTimeoutBuffer === "number" && this.options.agentKeepAliveTimeoutBuffer >= 0 && NumberIsFinite(this.options.agentKeepAliveTimeoutBuffer) ? this.options.agentKeepAliveTimeoutBuffer : 1000;
...
function maybeEnableKeylog(eventName) {
  if (eventName === "keylog") {
    ...
    this[kOnKeylog] = function onkeylog(keylog) {
      agent1.emit("keylog", keylog, this);
    };
...
Agent.prototype.createConnection = function createConnection(options, cb) {
  ...
  if (proxy && options._proxyProtocol === "http:") {
    ...
    if (proxy.protocol === "https:") {
      ...
      if (connectOpts.ca === undefined && op_get_env_no_permission_check("NODE_EXTRA_CA_CERTS")) {
...
Agent.prototype.addRequest = function addRequest(req, options, port, /* legacy */ localAddress) {
...
function installListeners(agent1, s, options) {
  function onFree() {
    debug("CLIENT socket onFree");
    agent1.emit("free", s, options);
  }
  s.on("free", onFree);
  function onClose(_err) {
    ...
    agent1.totalSocketCount--;
    agent1.removeSocket(s, options);
  }
  s.on("close", onClose);
  function onTimeout() {
    ...
    const sockets = agent1.freeSockets;
...
Agent.prototype.keepSocketAlive = function keepSocketAlive(socket) {
  ...
  if (socket._httpMessage?.res) {
    ...
    if (keepAliveHint) {
      const hint = RegExpPrototypeExec(KEEP_ALIVE_TIMEOUT_RE, keepAliveHint)?.[1];
      if (hint) {
...
function setRequestSocket(agent1, req, socket) {
  req.onSocket(socket);
  const agentTimeout = agent1.options.timeout || 0;
...
function asyncResetHandle(socket) {
  ...
  if (handle && typeof handle.asyncReset === "function") {
    ...
    emitInit(socket[async_id_symbol], providerTypeNames[providerType] || "UNKNOWN", executionAsyncId(), reusedHandle);
  }
}
...
export function setGlobalAgent(agent1) {
  globalAgent = agent1;
}
export default {
  Agent,
