 ...
 */ function HTTPParser(type) {
  ...
  HTTPParser.prototype.initialize = function(type, asyncResource, maxHeaderSize, lenientFlags) {
    ...
    if (asyncResource) {
      this._asyncResource = new AsyncResource(asyncResource.type || "HTTPPARSER");
    }
    this._native.initialize(type, effectiveMaxHeaderSize, lenientFlags ?? 0);
  };
  HTTPParser.prototype.execute = function(buffer, offset, length) {
    // Node.js calls execute(buffer) or execute(buffer, offset, length)
    let data;
    if (offset !== undefined && length !== undefined && (offset !== 0 || length !== buffer.length)) {
    ...
    if (origOnBody) {
      this[kOnBody] = function(buf) {
        return origOnBody.call(parser, Buffer.from(buf.buffer, buf.byteOffset, buf.byteLength));
      };
    }
    ...
    const doExecute = ()=>this._native.execute(this, new Uint8Array(data.buffer, data.byteOffset, data.byteLength));
    const result = this._asyncResource ? this._asyncResource.runInAsyncScope(doExecute) : doExecute();
