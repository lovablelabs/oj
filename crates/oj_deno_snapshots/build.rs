...
fn main() {
  ...
  #[allow(clippy::print_stdout, reason = "build script output")]
  {
    ...
    println!("cargo:rerun-if-env-changed=OJ_SNAPSHOT_ARCHIVE");
  }
  #[cfg(not(feature = "disable"))]
  {
    ...
    // The oj fork change: V8 snapshot CREATION is not run-deterministic
    // (lovablelabs/oj#211 — rusty_v8's SerializeInternalFields memcpys live
    // embedder memory into the blob), which breaks bit-reproducible builds.
    // OJ_SNAPSHOT_ARCHIVE points at a prebuilt bundle of this build script's
    // outputs (one per target, generated by tools/gen-snapshot-pin.sh); using
    // it takes the nondeterministic step out of the build entirely. Unset,
    // the vanilla path runs and additionally writes OJ_SNAPSHOT_MANIFEST so
    // the OUT_DIR is a ready-to-harvest bundle.
    if let Some(archive) = std::env::var_os("OJ_SNAPSHOT_ARCHIVE") {
      use_snapshot_archive(std::path::Path::new(&archive), &o);
    } else {
...
#[cfg(not(feature = "disable"))]
fn write_manifest(out_dir: &std::path::Path) {
  std::fs::write(out_dir.join("OJ_SNAPSHOT_MANIFEST"), manifest_contents())
    .unwrap();
}
...
fn use_snapshot_archive(archive: &std::path::Path, out_dir: &std::path::Path) {
  ...
  let manifest = std::fs::read_to_string(&manifest_path).unwrap_or_else(|e| {
    panic!(
      "OJ_SNAPSHOT_ARCHIVE is set but {} is unreadable: {e}",
  ...
  assert!(
    manifest == expected,
    "the pinned snapshot bundle at {} does not match this build:\n\
     bundle:\n{manifest}\nexpected:\n{expected}\n\
     Regenerate it with tools/gen-snapshot-pin.sh (a deno_runtime / deno_core \
  ...
  for entry in std::fs::read_dir(&residual_src).unwrap_or_else(|e| {
    panic!(
      "the snapshot bundle at {} has no residual_sources dir: {e}",
      archive.display()
    )
  }) {
