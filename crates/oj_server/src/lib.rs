...
async fn ensure_module(
    ...
    let compiled = tokio::task::spawn_blocking(move || -> Result<CachedModule, String> {
        ...
        if bundle {
            ...
            // value (module.exports) instead of linking as ESM bindings.
            ...
            let output = if is_dep {
                if oj_compiler::cjs::has_module_syntax_pub(&file_owned, &source) {
                    // ESM dep importing an unbundled CJS/UMD dep (proj4 ->
                    // geographiclib-geodesic) gets the same interop as app source.
                    let dep_interop = oj_compiler::interop::rewrite_cjs_interop(
                        &source,
                        &file_owned,
                        &cjs_interop_url,
                    );
                ...
                let interopped = oj_compiler::interop::rewrite_cjs_interop(
                    &source,
                    &file_owned,
                    &cjs_interop_url,
                );
