...
fn expand_import_meta(id: &str, code: &str) -> Option<String> {
    ...
    let mut expanded: Cow<'_, str> = Cow::Borrowed(code);
    if has_glob {
        expanded = Cow::Owned(oj_compiler::glob::expand_source(&expanded, path));
    }
    if has_dynamic {
        expanded = Cow::Owned(oj_compiler::glob::expand_dynamic_import_vars_source(
            &expanded, path,
        ));
    }
    if has_new_url {
        expanded = Cow::Owned(oj_compiler::glob::expand_new_url_asset_source(
            &expanded, path,
        ));
    }
    match expanded {
        Cow::Owned(out) if out != code => Some(out),
        _ => None,
    }
}
