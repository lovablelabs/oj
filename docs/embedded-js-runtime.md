...
byonm has one deliberate exception: rolldown itself. A Start app on a vite that predates rolldown (vite <= 7) resolves no rolldown from anywhere in its tree, so the Start bundles would fail outright — the Lovable corpus measured ~12% of TanStack Start projects in this class. The nix package vendors the pinned rolldown next to the binary (`OJ_VENDORED_ROLLDOWN`, embedded at compile time and part of the runtime closure), and `resolve-pkg.mjs` falls back to it only after the app graph fails to resolve one; an app that ships its own rolldown keeps it, so vite-8 apps see no change.
