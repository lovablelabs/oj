...
byonm has one deliberate exception: rolldown itself. A Start app on a vite that predates rolldown (vite <= 7) resolves no rolldown from anywhere in its tree, so the Start bundles would fail outright. The nix package vendors the pinned rolldown next to the binary (`OJ_VENDORED_ROLLDOWN`, embedded at compile time and part of the runtime closure), and `resolve-pkg.mjs` prefers it: the Start bundles are oj's own code, written and tested against that version, and old app pins carry bindings that cannot survive re-registration (napi-rs < 3.10). The app's own rolldown remains the fallback for oj builds that vendor nothing.
